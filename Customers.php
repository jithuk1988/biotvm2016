...
$Errors = array();
 ...
 while($row1=DB_fetch_array($result1) )      {  
  ...
  $district=$row1['district']; 
     ...
     }elseif($_GET['bussid']!=null and $_GET['loccode']!=null)   
     { 
            $locode=$_GET['loccode'];
            $bsid=$_GET['bussid'];

            $sqltr="UPDATE bio_dealerterritory set locationcode='$locode' where buss_id=$bsid";
            $result = DB_query($sqltr, $db); 
            
     $sql1="SELECT bio_businessassodetails_enq.custname,
                   bio_businessassodetails_enq.custphone,bio_businessassodetails_enq.custmob,bio_businessassodetails_enq.custmail,
                   bio_businessassodetails_enq.houseno,bio_businessassodetails_enq.housename,bio_businessassodetails_enq.area1,bio_businessassodetails_enq.area2,bio_businessassodetails_enq.pin,
                   bio_businessassodetails_enq.nationality,bio_businessassodetails_enq.state,bio_businessassodetails_enq.district,
                   bio_businessassodetails_enq.LSG_type,bio_businessassodetails_enq.LSG_name,bio_businessassodetails_enq.block_name,
                   bio_businessassodetails_enq.taluk,bio_businessassodetails_enq.LSG_ward,bio_businessassodetails_enq.village  
            FROM   bio_businessassociates_enq,bio_businessassodetails_enq 
...
if (isset($_POST['submit'])) {
    ...
    elseif (strlen($_POST['ebno']) <3) {
        $InputError = 1;
        prnMsg( _('Please enter a valid Electricity Board No to Continue.'),'error');
        $Errors[$i] = 'ebno';
        $i++;
    }                                                                                        */
...
//	}
	...
	if ($InputError !=1){
		...
		if (!isset($_POST['New'])) {
           ...
           /*$sql1 = "UPDATE custbranch SET phoneno='".$_POST['Phone']."',faxno='".$fx."',email='".$_POST['Email']."' WHERE debtorno='" . $_POST['DebtorNo'] . "'";    
            DB_query($sql1,$db);  */       
...
//echo $sql;
		//
       $sql1 = "UPDATE custbranch SET 
       brname='" . $_POST['CustName'] . "',
       braddress1='" . $_POST['Address1'] . "',
               braddress2='" . $_POST['Address2'] . "',
             braddress3='" . $_POST['Address3'] ."',
             braddress4='" . $_POST['Address4'] . "',
          braddress5='" . $_POST['Address5'] . "',
      braddress6='" . $_POST['Address6'] . "',
       phoneno='".$_POST['Phone']."',
       faxno='".$fx."',email='".$_POST['Email']."', 
       cid= '" . $_POST['country'] . "',
       stateid='" . $_POST['State'] . "',
       did='" . $_POST['District'] . "',
       taluk='" . $_POST['taluk'] . "',
...
$_SESSION['Email']=$_POST['Email'];            
            ...
            if($_POST['bussid']!=""){
                $sql_cust="UPDATE bio_dealerterritory SET debtorno='" . $_POST['DebtorNo'] ."' WHERE buss_id=" . $_POST['bussid'] . " ";
                $result_cust=DB_query($sql_cust,$db); 
            }
...
/* First check that all the necessary items have been setup */
    ...
    if($bsid!=""){
    echo '<input type="hidden" name="bussid" value="'.$bsid.'">';     
    }
...
echo '<table align=left >';//    echo'<table id=autocode >';  

    echo'<tr id=autocode><td width=237px>' . _('Customer Code') . ':</td><td><input tabindex=1 type="text" name="DebtorNo" id="DebtorNo" size=11 maxlength=10 style="width:100px"></td></tr>';
...
echo'</td></tr>';
    ...
    echo '<tr><td>' . _('Fathers/Husbands Name') . ':</td>
        <td><input tabindex=2 type="Text" name="DepName" id="DepName" size=42 maxlength=100 value="'.$depname.'" onchange=duplicatename();></td></tr>';    
...
// Show Sales Type drop down list
	...
	if (DB_num_rows($result)==0){
		...
		echo '<tr ><td>' . _('Sales Type/Price List') . ':</td>              
			   <td><select tabindex=9 name="SalesType">';                                               //display none     style="display:none"
	...
	$DateString = Date($_SESSION['DefaultDateFormat']);
         if(!isset($ticketid))  {
             ...
             $sqyr="SELECT DATE_FORMAT(CURDATE(),'%Y') as yr";
             $mr=DB_query($sqyr,$db);
             $yr=DB_fetch_array($mr);
             $year=$yr[0];
	...
	echo '<tr><td>' . _('Customer Since') . ' (' . $_SESSION['DefaultDateFormat'] . '):</td>
         ...
         echo '<tr><td>EBNO(Electricity Board)</td><td><input type="text" name="ebno" id="ebno" value="'.$_POST['ebno'].'" ></td></tr>';
...
//DebtorNo exists - either passed when calling the form or from the form itself
	...
	if (!isset($_POST['New'])) {
        $sq="select * from bio_fileno where debtorno='".$DebtorNo."'";
         $result_FILE=DB_query($sq,$db);
          $row_FILE=DB_fetch_array($result_FILE);
	...
	echo'<tr><td width=237px>' . _('File No') . ':</td><td><b>'.$row_FILE['fileno'].'</b></td></tr>';
		echo '<tr><td>' . _('Customer Name') . ':</td>
        ...
        echo '<tr><td>' . _('Fathers/Husbands Name') . ':</td>
            <td><input ' . (in_array('DepName',$Errors) ?  'class="inputerror"' : '' ) .' type="Text" name="DepName" value="' . $_POST['DepName'] . '" size=42 maxlength=40></td></tr>';
   ...
   echo '</select></td></tr>';
    ...
    if($LSGtypeid==1) 
      ...
      echo'</tr>';
            ...
            echo '<tr id="showpanchayath"><td>' . _('Panchayat Name') . ':</td>';         //grama panchayath
         
         $sql="SELECT * FROM bio_panchayat WHERE country='".$nationality."' AND state='".$state."' AND district='".$district."'";
         $result=DB_query($sql,$db);
         
         echo '<td><select name="gramaPanchayath" id="gramaPanchayath" style="width:190px" onchange="showblocks(this.value)" tabindex=11>';
         $f=0;
         while($myrow1=DB_fetch_array($result))
         {
