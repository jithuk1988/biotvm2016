...
if(!isset($_GET['stockid'])) 
{
 ...
 $sql_totprice="UPDATE bio_lsgplant SET totprice='".$myrow[0]."'                                       
                                      WHERE propid='".$propid."'
                                      AND leadid= $leadid";
...
/*     $sql="DELETE FROM bio_lsgplant WHERE leadid=$leadid AND propid='".$_POST['flag']."'"; 
    ...
    echo  $sql_plantdel="DELETE FROM bio_lsgplantdetails WHERE  propid='".$propid."'"; 
     $result_plantdel=DB_query($sql_plantdel,$db);
     
    echo $sql_funddel="DELETE FROM bio_fundingagency WHERE leadid=$leadid AND propid='".$propid."'"; 
    ...
    while($myrow_temp=DB_fetch_array($result_temp))
    {
     ...
     $sql_temp1="INSERT INTO bio_fundingagency (leadid,propid,funding_agency,amount) VALUES($leadid,$propid,'".$myrow_temp['scheme']."','".$myrow_temp['amount']."')"; 
...
if($_POST['submit'])
 {
    ...
    $result_plant=DB_query($sql_plant,$db);
     ...
     $myrow=DB_fetch_row($result); 
       ...
       $sql_plant1="INSERT INTO bio_lsgplant(propdate,leadid,totprice,status) VALUES('".date("Y-m-d")."',$leadid,'".$myrow[0]."',0)";
...
echo"<table style='width:90%'>";
   ...
   $sql="SELECT `bio_lsgreg`.`project_name`,
  ...
  $cph=$custphone.",".$custmob;

       

  
echo"<tr><td width=50%>Project Name:</td>";
...
echo"<tr></tr>";
echo"<tr><td>Project Cost</td>";
echo"<td><input type='hidden' name='email' id='email' value='$total_projectcost'>$total_projectcost</td></tr>";
echo"<tr></tr>";
echo"<tr><td>Number of Beneficiaries:</td>";
echo"<td><input type='hidden' name='custplace' id='custplace' value='$num_beneficiaries'>$num_beneficiaries</td></tr>";
echo"<tr></tr>";
...
{               
   ...
   myRef = window.open("bio_lsgplantselect.php?categoryid="+str1+"&flag="+str,"lsgplant","toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=no,width=700,height=400");
