  ...
  date_default_timezone_set ("Asia/Calcutta"); 
   $curdate = date("Y-m-d  H:i:s", time()) ;
    ...
    $sql_add="INSERT INTO bio_cstask(cstype,orderno,despatch_id,taskno,taskdescription,daystocomplete,prevtask,scheduleddate,team,status,cid,stateid,did,LSG_type,LSG_name,block_name)
             ...
             //Task Transaction  
            $sql_get_maxid="SELECT max(id) AS maxid from bio_cstask WHERE cstype=3 AND orderno='".$orderno."' AND  despatch_id='".$desid."' ";
   ...
   // $plant=$_POST['plant'];
    ...
    $sql_add="INSERT INTO bio_cstask(cstype,orderno,despatch_id,taskno,taskdescription,daystocomplete,prevtask,scheduleddate,team,status,cid,stateid,did,LSG_type,LSG_name,block_name)
            ...
            //Task Transaction        
            $sql_get_maxid="SELECT max(id) AS maxid from bio_cstask WHERE cstype=3 AND orderno='".$orderno."' AND  despatch_id='".$_POST['amcno']."' ";
            $result_get_maxid=DB_query($sql_get_maxid,$db);
            $max_id=DB_fetch_array($result_get_maxid);
            $maxid=$max_id['maxid'];
    ...
    $team=$_POST['team'.$i];
          ...
          AND orderno='".$orderno."' AND taskno='".$_POST['task_no'.$i]."' ";
              ...
              //Task transaction
                ...
                $sql_get_maxid="SELECT id from bio_cstask WHERE cstype=3 AND orderno='".$orderno."' AND  despatch_id='".$desid."' AND taskno='".$_POST['task_no'.$i]."'";
            $result_get_maxid=DB_query($sql_get_maxid,$db);
            $max_id=DB_fetch_array($result_get_maxid);
            $maxid=$max_id['id'];
 ...
 if($orderno!='')
 {
    ...
    $sql1="SELECT custbranch.brname,
     ...
     $row1=DB_fetch_array($result1,$db);   */
         ...
         $sql1="SELECT custbranch.brname,
              custbranch.phoneno,custbranch.did,custbranch.stateid,custbranch.cid,bio_district.district,custbranch.LSG_type,custbranch.LSG_name,custbranch.block_name,
              bio_amc.start_date,bio_amc.end_date,bio_amc.plant,bio_amc.serialno,bio_amc.plant_description 
              from custbranch,salesorders,bio_district,bio_amc 
              where 
              salesorders.debtorno=custbranch.debtorno
              AND salesorders.orderno=bio_amc.new_orderno
    ...
    if($row1['plant']!=NULL)
    {   $result_plnt=DB_query("SELECT stockmaster.description FROM stockmaster WHERE stockmaster.stockid='".$row1['plant']."'",$db);
    $row_plnt=DB_fetch_array($result_plnt);
        $plnt=$row_plnt['description'];
    }
    else
    {
        $plnt=$row1['plant'];
...
echo '<table class="selection">
            ...
            <tr>
                <th> ' . _('Serial No') . ' :<b> ' . $row1['serialno'].'</b></th>
                <th colspan ="2">' . _('Plant') . ' :<b> ' . $plnt. '</b></th>
            </tr>
