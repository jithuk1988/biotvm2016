...
if ($WORow['closed']==1){
  ...
  $a=0;
    ...
    echo '<input type="hidden" name="serno" id="serno" value=' .$a. '>';
    //  echo"<input type='hidden' name='serno' id='serno' value='".$a."'"; 
    }
/*    if(isset($_POST['Process'])){
	...
	if ($WORow['serialised']==1){ //serialised
		...
		if ($_SESSION['DefineControlledOnWOEntry']==1){ //then potentially serial numbers already set up
			...
			if (DB_num_rows($WOSNResult)==0){
				...
				while ($WOSNRow = DB_fetch_row($WOSNResult)){
					...
					echo '<td><input type="checkbox" name="CheckItem' . $i . '">'. $WOSNRow[0] .'<input type="hidden" name="SerialNo' . $i . '"  id="SerialNo' . $i . '" value="' . $WOSNRow[0] . '"><input type="hidden" name="QualityText' . $i . '" value="' . $WOSNRow[1] . '"></td>';
					$i++;
				}
			}
        ...
        $tb=number_format($WORow['qtyreqd'],$WORow['decimalplaces'])-number_format($WORow['qtyrecd'],$WORow['decimalplaces']);
			for ($i=0;$i<$tb;$i++){
				...
				echo '" onchange="duplicate(this.id)"> </td>';
...
while($row_getwo=DB_fetch_array($result_getwoitems))
{
    if ($j==1) {
           ...
           $rr= $row_getwo['qtyrecd'];
            ...
            else if($con<1 ){ echo "<td><input type=text  name='comp_name".$s."' value=0></td>"; }
...
function log_in()
{   //
//var sno=document.getElementById('SerialNo'+1).value; alert(sno);   
 
var f=0;
  

if(f==0){ f=common_error('SerialNo0','Enter the SerialNo');  if(f==1){return f; } } 
/*for(var i=0;i<=sno;i++)
{
 
}*/ 
}
function duplicate(serid)
{
  var serno=document.getElementById('serid').value; alert("serno") ; 
if (str=="")
  {
  document.getElementById("serid").innerHTML="";
  return;
  }
...
xmlhttp.open("GET","bio_dup_oder_serial_ajx.php?serial=" + serno,true);
xmlhttp.send(); 
