...
echo '<center><font style="color: #333;
    ...
    font-size:16px;
    font-family:Georgia;
    text-shadow: 1px 1px 1px #666;">Sales Invoice</font></center>';
...
echo"<legend><h3>Select an order</h3></legend>";
   echo"<div id=grid>"; 
        ...
        echo '<tr>  <th>' . _('Slno') . '</th>
             ...
             GROUP BY salesorders.orderno having count(salesorderdetails.completed)>0";  
       ...
       $sql_inst .=" AND custbranch.did IN (6,11,12)  ";
                      ...
                      if($_POST['submit'])
              {
                  if($_POST['custname']!=NULL) {
                   ...
                   if($_POST['ordno']!=NULL){
                     $sql_inst.=" AND salesorders.orderno LIKE '".$_POST['ordno']."%'";   
                  }
...
while($row_so=DB_fetch_array($result_so))
{
    ...
    $plant=$row_so['stkcode'];
     $despatch_id=$row_so['despatch_id'];
          if ($k==1)
          {
            echo '<tr  id="link" class="EvenTableRows" >';
            $k=0;
          }else 
          {
            ...
            $k=1;     
               ...
               $sql_getitems="SELECT  salesorderdetails.stkcode,stockmaster.description FROM salesorderdetails,stockmaster 
               WHERE stockmaster.stockid=salesorderdetails.stkcode
               AND salesorderdetails.completed=0
               AND salesorderdetails.orderno=".$row_so['orderno']." 
               ";
...
{      //alert(str2); //  alert(str1);    
    //myRef = window.open("bio_lsgstatuschange.php?leadid=" + str1 + "&id=" + str2,"droplead","toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=no,width=700,height=400");
                       // alert(str2);
location.href="ConfirmDispatch_Invoice.php?OrderNumber=" + str1;   
