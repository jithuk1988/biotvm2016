...
//      $('#dinhide').show(); 
  ...
  //alert(str);
xmlhttp.open("GET","bio_maincatsubcat.php?maincatid="+str,true);
xmlhttp.send(); 
...
//      $('#dinhide').show(); 
  ...
  //alert(str);
xmlhttp.open("GET","bio_maincatsubcat.php?maincatid="+str+"&subcatid="+str1,true);
xmlhttp.send(); 
...
/* $Id: Stocks.php 4581 2011-05-29 09:38:46Z daintree $ */
...
/*If the page was called without $StockID passed to page then assume a new stock item is to be entered show a form with a part Code field other wise the form showing the fields with the existing entries against the part will show for editing with only a hidden StockID field. New is set to flag that the page may have called itself and still be entering a new part, in which case the page needs to know not to go looking up details for an existing part*/
	...
	if ($New==1) {
		echo '<tr><td>'. _('Item Code'). ':</td><td><input ' . (in_array('StockID',$Errors) ?  'class="inputerror"' : '' ) .'  type="text"
			value="'.$StockID.'" name="StockID" size=30 maxlength=20 /></td></tr>'. "\n";
	} else {
...
echo '<tr><td>' . _('Part Description') . ' (' . _('short') . '):</td><td><input ' . (in_array('Description',$Errors) ?  'class="inputerror"' : '' ) .' type="Text" name="Description" size=30 maxlength=50 value="' . $Description . '"></td></tr>'."\n";
...
echo '<tr><td>' . _('Part Description') . ' (' . _('long') . '):</td><td><textarea ' . (in_array('LongDescription',$Errors) ?  'class="texterror"' : '' ) .'  name="LongDescription" cols=27 rows=4>' . stripslashes($LongDescription) . '</textarea></td></tr>'."\n";
...
function select_files($dir, $label = '', $select_name = 'ItemPDF', $curr_val = '', $char_length = 60) {
	...
	if ($handle = opendir($dir)) {
		$mydir = "<select style='width:200px' name=".$select_name.">\n";
...
echo '<tr style="display:none"><td>' . _('PDF attachment (.pdf)') . ':' . "\n</td><td>" . select_files('companies/' . $_SESSION['DatabaseName'] .'/pdf_append//','' , 'ItemPDF', $_POST['ItemPDF'], '60') . '</td></tr>'. "\n";

// Add image upload for New Item  - by Ori
echo '<tr style="display:none"><td>'. _('Image File (.jpg)') . ':</td><td><input type="file" id="ItemPicture" name="ItemPicture"></td>';
...
if(isset($_POST['CategoryID']))     {
    ...
    $sql7="SELECT maincatid,subcatid
           FROM bio_maincat_subcat
           WHERE subsubcatid='".$_POST['CategoryID']."'";
    $result7=DB_query($sql7,$db);
    $myrow7=DB_fetch_array($result7); 
     $_POST['SubCategoryID']=$myrow7[1];
    $_POST['MainCategoryID']=$myrow7[0];    
}

 echo '<tr><td>' . _('Main Category') . ':</td><td><select style="width:200px" name="MainCategoryID" id="maincatid" onblur="Filtercategory()" onchange="Filtercategory()" >';
...
echo '</select></td></tr>';
 ...
 echo '<tr><td >' . _('Sub Category') . ':</td><td id="subcat"><select name="SubCategoryID" id="SubCategoryID"  style="width:200px" onblur="Filtercategory2()"  onchange="Filtercategory2()"';
...
echo '<tr><td>' . _('Sub Sub Category') . ':</td><td id="subsubcat"><select name="CategoryID" style="width:200px" >';
...
echo '</select><a target="_blank" href="'. $rootpath . '/StockCategories.php?' . SID . '">' . _('Sub Sub-Categories Master') . '</a></td></tr>';
...
echo '<tr><td>' . _('Economic Order Quantity') . ':</td><td><input ' . (in_array('EOQ',$Errors) ?  'class="inputerror"' : '' ) .'   type="Text" class="number" name="EOQ" size=30 maxlength=10 value="' . $_POST['EOQ'] . '"></td></tr>';

echo '<tr><td>' . _('Packaged Volume (metres cubed)') . ':</td><td><input ' . (in_array('Volume',$Errors) ?  'class="inputerror"' : '' ) .'   type="Text" class="number" name="Volume" size=30 maxlength=10 value="' . $_POST['Volume'] . '"></td></tr>';

echo '<tr><td>' . _('Packaged Weight (KGs)') . ':</td><td><input ' . (in_array('KGS',$Errors) ?  'class="inputerror"' : '' ) .'   type="Text" class="number" name="KGS" size=30 maxlength=10 value="' . $_POST['KGS'] . '"></td></tr>';

echo '<tr><td>' . _('Units of Measure') . ':</td><td><select style="width:200px" ' . (in_array('Description',$Errors) ?  'class="selecterror"' : '' ) .'  name="Units">';
...
echo '<tr><td>' . _('Assembly, Kit, Manufactured or Service/Labour') . ':</td><td><select name="MBFlag" style="width:200px">';
/*if ($_POST['MBFlag']=='A'){
...
echo '<tr><td>' . _('Current or Obsolete') . ':</td><td><select style="width:200px" name="Discontinued">';
...
echo '<tr><td>' . _('Batch, Serial or Lot Control') . ':</td><td><select style="width:200px" name="Controlled">';
...
echo '<tr><td>' . _('Serialised') . ':</td><td><select style="width:200px" ' . (in_array('Serialised',$Errors) ?  'class="selecterror"' : '' ) .'  name="Serialised">';
...
if ($_POST['Serialised']==1 AND $_POST['MBFlag']=='M'){
	echo '<tr><td>' . _('Next Serial No (>0 for auto numbering)') . ':</td><td><input ' . (in_array('NextSerialNo',$Errors) ?  'class="inputerror"' : '' ) .' type="text" name="NextSerialNo" size=30 maxlength=15 value="' . $_POST['NextSerialNo'] . '"><td></tr>';
} else {
	echo '<input type="hidden" name="NextSerialNo" value="0">';
}

echo '<tr><td>' . _('Perishable') . ':</td><td><select style="width:200px" name="Perishable">';
...
echo '<tr><td>' . _('Decimal Places for display Quantity') . ':</td><td><input type="text" class="number" name="DecimalPlaces" size=30 maxlength=1 value="' . $_POST['DecimalPlaces'] . '"><td></tr>';
...
echo '<tr><td>' . _('Bar Code') . ':</td><td><input ' . (in_array('BarCode',$Errors) ?  'class="inputerror"' : '' ) .'  type="Text" name="BarCode" size=30 maxlength=20 value="' . $BarCode . '"></td></tr>';
...
echo '<tr><td>' . _('Discount Category') . ':</td><td><input type="Text" name="DiscountCategory" size=30 maxlength=2 value="' . $DiscountCategory . '"></td></tr>';

echo '<tr><td>' . _('Tax Category') . ':</td><td><select name="TaxCat" style="width:200px">';
...
echo '<tr>
		<td>' . _('Pan Size') . ':</td>
		<td><input type="Text" class="number" name="Pansize" size="30" maxlength="6" value=' . $_POST['Pansize'] . '></td>
	</tr>
	 <tr>
		<td>' . _('Shrinkage Factor') . ':</td>
		<td><input type="Text" class="number" name="ShrinkFactor" size="30" maxlength="6" value=' . $_POST['ShrinkFactor'] . '></td>
	</tr>';
...
echo '<br /><table class=selection style="display:none">';
	...
	switch ($PropertyRow['controltype']) {
	 	case 0; //textbox
	 		if ($PropertyRow['numericvalue']==1) {
				echo '<input type="textbox" class="number" name="PropValue' . $PropertyCounter . '" size="30" maxlength="100" value="' . $PropertyValue . '">';
				echo _('A number between') . ' ' . $PropertyRow['minimumvalue'] . ' ' . _('and') . ' ' . $PropertyRow['maximumvalue'] . ' ' . _('is expected');
			} else {
				echo '<input type="textbox" name="PropValue' . $PropertyCounter . '" size="30" maxlength="100" value="' . $PropertyValue . '">';
			}
	 		break;
	 	case 1; //select box
