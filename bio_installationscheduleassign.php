...
echo '<center><font style="color: #333;
  ...
  date_default_timezone_set ("Asia/Calcutta"); 
   $curdate = date("Y-m-d  H:i:s", time()) ;
    ...
    if($_POST['prevno']!=0){
     $prev=$_POST['prevno'];
    $sql_newdate="SELECT daystocomplete,scheduleddate from bio_cstask WHERE cstype=1 AND despatch_id='".$_POST['desid']."' AND taskno='$prev'";
    ...
    $newdate2 = date ( 'Y-m-d' , $newdate2 );
     ...
     $sql_add="INSERT INTO bio_cstask(cstype,orderno,despatch_id,taskno,taskdescription,daystocomplete,prevtask,scheduleddate,team,status,cid,stateid,did,LSG_type,LSG_name,block_name)
                    values (1,'$orderno','".$_POST['desid']."',$taskno,'$des','$days','$prev','$newdate2',$team,0,'".$_POST['cid']."',
                    '".$_POST['stateid']."' , '".$_POST['did']."', '".$_POST['block_name']."' , '".$_POST['LSG_name']."' , '".$_POST['LSG_type']."')";
                    $result_add=DB_query($sql_add,$db);                                 
       
      
            $sql_get_maxid="SELECT max(id) AS maxid from bio_cstask WHERE cstype=1 AND orderno='".$orderno."' AND  despatch_id='".$_POST['desid']."' ";
    ...
    $team=$_POST['team'.$i];
          ...
          $sql_add="INSERT INTO bio_cstask(cstype,orderno,despatch_id,taskno,taskdescription,daystocomplete,prevtask,scheduleddate,team,status,cid,stateid,did,LSG_type,LSG_name,block_name)
                    values (1,'$orderno','".$_POST['desid']."',$i,'$des','$days','$prev','$start','$team',0,'".$_POST['cid']."',
                    '".$_POST['stateid']."' , '".$_POST['did']."', '".$_POST['block_name']."' , '".$_POST['LSG_name']."' , '".$_POST['LSG_type']."')";
                   $result_add=DB_query($sql_add,$db);
    ...
    $team=$_POST['team'.$i];
         ...
         $sql_edit="UPDATE bio_cstask SET  taskdescription='".$des."' , daystocomplete='".$days."' ,
           scheduleddate='".$start."' , team='".$team."' WHERE cstype=1    AND despatch_id='".$_POST['desid']."'
 ...
 if($orderno!='')
 {
             ...
             $sql1="SELECT custbranch.brname,custbranch.LSG_type,custbranch.LSG_name,custbranch.block_name,
              ...
              AND  bio_deliverynote.despatch_id='".$desid."'
              AND   salesorderdetails.orderno=".$orderno;
  ...
  $get_serial=DB_query("SELECT serialno FROM bio_despatchClearence WHERE id='".$desid."'",$db);
    $rowserial=DB_fetch_array($get_serial);       
...
echo '<table class="selection">
            ...
            <br />';
                      ...
                      /*  <tr>
                <th colspan ="3"><b>' . _('Plant Serial No:') . ' ' . $rowserial['serialno']. '</b></th>
            </tr>       */
            
            
            $sql_new="SELECT count(id) as count from bio_cstask where orderno=$orderno AND despatch_id='$desid' AND cstype=1";
            ...
            $sql_min="SELECT min(prevtask) as recent_task from bio_cstask where orderno='$orderno' AND despatch_id='$desid'   
...
//-------------- Viwe Assigned Tasks-----------------------
    ...
    //        $row_team=DB_fetch_array($result_team);u
         ...
         //----------For existing entry, to update status of table
                                                    ...
                                                    where orderno='$orderno' AND cstype=1 AND despatch_id='$desid'
                                  ...
                                  if($row_assigned['prevtask']==0){   
                                    echo" <td><input type=text readonly value='No Previous Task' name='prev".$i."' id='prev".$i."'></td>";
                                      ...
                                      $sql_prevtask="SELECT taskdescription from bio_cstask where cstype=1 AND orderno='$orderno' 
                                       AND taskno='".$row_assigned['prevtask']."'";
                                       $result_prevtask=DB_query($sql_prevtask,$db);
                                      $row_prevtask=DB_fetch_array($result_prevtask);
    ...
    echo"<td><select name='statuschange' id='".$row_assigned['taskno']."' onchange=changestatus(this.id,$orderno,'$desid',$i,this.value,$desid);>";
...
$sql="SELECT *FROM bio_cstask where despatch_id='$desid' AND cstype=1 AND orderno='".$orderno."' ";
   $result=DB_query($sql,$db);
  ...
  echo '<td>Team: </td>';
   ...
   $sql_team="SELECT
    `bio_leadteams`.`teamname`,`bio_leadteams`.`teamid`
        ...
        AND cid=".$row1['cid']."  )
        AND bio_emp.deptid IN (5,6)";
        $result_team=DB_query($sql_team,$db);   // echo $sql_team;
    //        $row_team=DB_fetch_array($result_team);u
...
function changestatus(str,str1,str5,str3,str6,str13)
{
          //alert(str5);
...
//      $('#dinhide').show(); 
  ...
  //alert(str);
xmlhttp.open("GET","bio_installation_statuschange.php?taskno="+str+"&orderno="+str1+"&desid="+str5+"&no="+str3+ "&date=" + str4 + "&status=" + str6
+ "&delay=" + x + "&desid=" + str13,true);
xmlhttp.send();
//alert(str);
}
