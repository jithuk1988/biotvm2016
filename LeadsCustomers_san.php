...
$_SESSION['Email']=$_POST['Email']; 
            ...
            $sql_cust="UPDATE bio_cust SET debtorno='" . $_POST['DebtorNo'] ."' WHERE cust_id=" . $_POST['custid'] . " ";
			...
			echo '<meta http-equiv="Refresh" content="0; url=' . $rootpath .'/CustomerBranches.php?DebtorNo=' . $_POST['DebtorNo'] . '&flag=' .$_POST['flag'] . '&leadid=' .$_POST['leadid'] . '">';        

			echo '<div class="centre">' . _('You should automatically be forwarded to the entry of a new Customer Branch page') .
			'. ' . _('If this does not happen') .' (' . _('if the browser does not support META Refresh') . ') ' .
			'<a href="' . $rootpath . '/CustomerBranches.php?DebtorNo=' . $_POST['DebtorNo']  . '&flag=' .$_POST['flag'] . '&leadid=' .$_POST['leadid'] . '"></a></div>';
...
//$sql_autofill="SELECT ";              
	...
	//if ($_SESSION['AutoDebtorNo']==0)  {
	echo '<tr><td>' . _('Customer Code') . ':</td><td><input tabindex=1 type="text" required name="DebtorNo" value="'.$DebtorNo.'" size=11 maxlength=10></td></tr>';
	//}

	echo '<tr><td>' . _('Customer Name') . ':</td>
		<td><input tabindex=2 type="Text" name="CustName" required value="'.$row_select['custname'].'" size=42 maxlength=100></td></tr>';
	echo '<tr><td>' . _('Phone Number') . ':</td>
		<td><input tabindex=2 type="Text" name="Phone" id="Phone" value="'.$row_select['custphone'].'" size=30 maxlength=40></td></tr>';
	echo '<tr><td>' . _('Mobile Number') . ':</td>
		<td><input tabindex=2 type="Text" name="Fax" id="Fax" value="'.$row_select['custmob'].'" size=30 maxlength=40></td></tr>';
	...
	echo '<tr><td width=200px>' . _('House/ Building No, House/Org Name') . ':</td>
		<td><input tabindex=3 type="Text" name="Address1" id="Address1" value="'.$row_select['houseno'].','.$row_select['housename'].'" size=42 maxlength=200></td></tr>';
...
if($LSGtypeid!=0){    
 ...
 //    style="align:left"                    
        ...
        $district=$row['district'];
          ...
          if($country==1 && $state==14)  
          {
              ...
              if($district==8){$distname='Kozhikode';} 
                    ...
                    echo '<table id="block" align=left ><tr><td width=200px>' . _('Corporation Name') . ':</td>';
        ...
        {
            //echo"2222222";
        echo '<table id="block" align=left ><tr> <td width=200px>' . _('Municipality Name') . ':</td>';    
      ...
      echo'</td></tr></table>'; 
        ...
        }elseif($LSGtypeid==3) 
        {
         ...
         echo '<table id="block" align=left ><tr> <td width=200px>' . _('Block Name') . ':</td>';    
...
// Show Customer Type drop down list
	...
	if ($DataError ==0){
		echo '<br /><div class="centre"><input tabindex=20 type="Submit" name="submit" value="' . _('Add New Customer') . '" onmouseover=tested(); onclick="if(validation()==1)return false; ">&nbsp;<input tabindex=21 type="submit" action="Reset" value="' . _('Reset') . '"></div>';
...
// Select Customer types for drop down list for SELECT/UPDATE
	...
	if (isset($_POST['New']) and $_POST['New']) {
		echo '<div class="centre"><input type="submit" name="submit" value="' . _('Add New Customer') .
			'"  onclick="if(validation()==1)return false; ">&nbsp;<input type=submit name="Reset" value="' . _('Reset') . '"></div></form>';
	} else if (!isset($_GET['Modify'])){
		echo '<br /><div class="centre"><input type="submit" name="submit" value="' . _('Update Customer') . '"  onclick="if(validation()==1)return false; ">';
...
function validation()
 { 
    str6=document.getElementById('lsgType').value; 
      str4=document.getElementById('Phone').value;
       str5=document.getElementById('Fax').value;
     ...
     str8=document.getElementById('Address1').value; 
 ...
 var f=0;
       ...
       if(str8==" , " || str8=="," || str8==" ,"  || str8=="" || str8==", " )
      {
          ...
          return f=1;
      }      /*if(f==0){f=common_error('Phone','Please Select phone number');  if(f==1) { return f; }} 
      if(f==0){f=common_error('Fax','Please Select mobile number');  if(f==1) { return f; }} */
     ...
     // alert("kjkj");
      ...
      if(str1==1 && (str3==1 || str3==3 || str3==4 || str3==5 || str3==7 || str3==9 || str3==10 || str3==10 || str3==14)){
...
//     if(f==0){f=common_error('enqid','Please Select a Customer Type');  if(f==1) { return f; }} 
 ...
 if (str6==1)
     { 
     if(f==0){f=common_error('lsgName','Please Select your Corporation');  if(f==1) { return f; }} 
     }
     if (str6==2)
     { 
     if(f==0){f=common_error('lsgName','Please Select your Municipality');  if(f==1) { return f; }} 
     }
       if (str6==3)
     { 
     if(f==0){f=common_error('lsgName','Please Select your block name');  if(f==1) { return f; }} 
     if(f==0){f=common_error('gramaPanchayath','Please Select your panchayath name');  if(f==1) { return f; }} 
     }
          ...
          if(y==2)
          {
          alert("HOUSE NUMBER ALREADY ENTERED") ;
          document.getElementById('Address1').focus();
              return 1;
 ...
 function tested()
  { 
      ...
      str1=document.getElementById('Address1').value; 
     str2=document.getElementById('country').value;
     str3=document.getElementById('state').value;
     str4=document.getElementById('Districts').value;
      str5=document.getElementById('lsgType').value;
      str6=document.getElementById('lsgName').value; 
      //alert(str6);
...
var id="&str1="+str1+"&str2="+str2+"&str3="+str3+"&str4="+str4+"&str5="+str5+"&str6="+str6;  

}
      if(str5==3)
 {
         str7=document.getElementById('gramaPanchayath').value;   
     if(str7!=""){
        
         id+="&Pancha="+str7;// alert(id); //alert(id); 
     }
 }
 if (str1==null)
  {
...
xmlhttp.onreadystatechange=function()
  ...
  if (xmlhttp.readyState==4 && xmlhttp.status==200)
    ...
    //document.getElementById("mobile").focus();+"&str7="+str7
    }
  }
xmlhttp.open("GET","bio_customer_valid.php?id="+id,true);//+"&dist="+dist+"&lsgt="+lsgt+"&lsgn="+lsgn+"&pan="+pan
xmlhttp.send();  
 ...
 function showblock(str){   
  ...
  //alert(str);  alert(str1);   alert(str2);       alert(str3);   
     if(str==1 && (str3==1 || str3==3 || str3==4 || str3==5 || str3==7 || str3==9 || str3==10 || str3==11 || str3==14)){
         alert("No Corporation for this district");
          document.getElementById("block").innerHTML="";
         document.getElementById("lsgType").focus();
      document.getElementById("lsgType").value=0;
         return;
     }
...
xmlhttp.send(); 
 ...
 function showtaluk(str){   
     ...
     str3=document.getElementById('Districts').value;
//alert(str1);   alert(str2);       alert(str3);
var str4=document.getElementById('lsgType').value;
    
//alert(str4);

   //str6=document.getElementById('gramaPanchayath').value; */
   if (str4!=null || str4!="")
  {
      document.getElementById("lsgType").focus();
      document.getElementById("lsgType").value=0;
