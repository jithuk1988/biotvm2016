         ...
         if($row['maxid']==NULL){
                    ...
                    // echo$row4['room'];
                     if($row4['room']==NULL)
                     {
                            ...
                            echo"<div class=warn>No physical location specified for this LSG , So cannot create file number.<a style='cursor:pointer;' onclick='docarch($did,$lsgtype,$lsgname);'>" . _('Click ') . "</a>   Here to add file location</div>";
...
echo'<input type=hidden name=debtorno id=debtorno value='.$row1['debtorno'].'>';
...
function change_inast_date()
{
    ...
    if(r==true){
    controlWindow=window.open("bio_installationdate.php?orderno="+order,"datechange1","toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=no,resizable=no,width=700,height=400");
    }
} 
...
xmlhttp.open("GET","bio_docarchieve.php?order=" + order + "&debtor=" + debtor + "&maxid="+ maxid + "&fileno=" +fileno,true); 
xmlhttp.send();
    //alert(fileno);   
  ...
  function docarch(str1,str2,str3)
  {    var order=document.getElementById("orderno").value; 
       controlWindow=window.open("addlocation.php?Dist="+str1+"&lsg="+str2+ "&lsgname="+str3+"&orderno="+order,"Archieving", "toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=no,width=800,height=600");
