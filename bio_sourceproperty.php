...
if (! isset($_GET['delete'])) {
   ...
   $_POST['SourceProperty']='';
    ...
    $sql="SELECT MAX(sourcepropertyid) FROM bio_sourceproperty";
            $result = DB_query($sql,$db);    
 $myrow = DB_fetch_row($result);  $_POST['PropertyID']=$myrow[0]+1;     }
	echo '<tr><td>' . _('Lead Source Property ID') . ':</td>

	                <!-- <td>' . $_POST['PropertyID'] . '</td> -->
			<td><input type="text" name="PropertyID" value="' . $_POST['PropertyID'] . '"></td>
		</tr>';
   ...
   //$c++;
	...
	echo '<tr><td>' . _('Lead Source Property') . ':</td>
			<td><input type="text" name="SourceProperty" value="' . $_POST['SourceProperty'] . '"></td>
		</tr>';
