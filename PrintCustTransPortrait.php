...
// gather the invoice data
		...
		if ($InvOrCredit=='Invoice') {
			$sql = "SELECT debtortrans.trandate,
				...
				AND bio_leadteams.teamid=salesorders.team_id
	...
	/* Print out the payment terms */
		...
		$LeftOvers = $pdf->addTextWrap($Left_Margin+450,$YPos-$line_height+9,72,$FontSize,$DisplayFreight, 'right');
        
        $pdf->addText($Page_Width-$Right_Margin-220, ($YPos+12)-(2*$line_height)+10,$FontSize, _('Subsidy'));
        $LeftOvers = $pdf->addTextWrap($Left_Margin+450,$YPos-(2*$line_height)+14,72, $FontSize,$DisplaySubsidy, 'right');

		$pdf->addText($Page_Width-$Right_Margin-220, ($YPos+12)-(2*$line_height),$FontSize, _('Tax'));
		$LeftOvers = $pdf->addTextWrap($Left_Margin+450,$YPos-(2*$line_height)+2,72, $FontSize,$DisplayTax, 'right');
