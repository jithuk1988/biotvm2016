...
echo"</td>";  
 ...
 echo '<td  class="bydistrict"><input type="text" name="byplace" id="byplace" value="'.$_POST['byplace'].'" style="width:120px"></td>';
   ...
   echo'<td class="leadstatus"><select name="LeadStatus" id="leadstatus" style="width:150px">'; 
...
echo'</select></td>';
   ...
   echo '<tr>';
   echo '<td>Grade<select name=grade_filter>';
   echo '<option value=""></option>'; 
   if($_POST['grade_filter']){
       echo'<option selected value="'.$_POST[grade_filter].'">';
   }
   echo '<option value="A">Grade A</option>';
   echo '<option value="B">Grade B</option>';
   echo '<option value="C">Grade C</option>';
   echo '<option value="D">Grade D</option>';
   echo '<option value="E">Grade E</option>';     
   echo '</td>';
   echo  '<td>Lead<input type="text" name="lead" id="lead" value='.$_POST['lead'].'></td>';
   ...
   echo '</tr>';
      ...
      if($_POST['datef']!=NULL){ $title.='Date from <b><i>'.$_POST['datef'].'</i></b>';}
      if($_POST['datet']!=NULL){$title.=' to <b><i>'.$_POST['datet'].'</i></b>';}
      if($_POST['enquiry']!=0){
      $sql="SELECT * FROM bio_enquirytypes where enqtypeid=".$_POST['enquiry'];
      $result=DB_query($sql,$db);
      $myrow1=DB_fetch_array($result);
      $title.=' Enquiry Type:<b><i>'.$myrow1['enquirytype'].'</i></b>';
      }
      
if($_POST['byplace']!=NULL){ $title.=' District <b><i>'.$_POST['byplace'].'</i></b>';}          
      
 if($_POST['off']!=0){
      $sql="SELECT * FROM bio_office where id=".$_POST['off'];
      $result=DB_query($sql,$db);
      $myrow1=DB_fetch_array($result);
 ...
 if($_POST['team']!=0){
      $sql="SELECT * FROM bio_leadteams where teamid=".$_POST['team'];
      $result=DB_query($sql,$db);
      $myrow1=DB_fetch_array($result);
      $title.=' Team:<b><i>'.$myrow1['teamname'].'</i></b>';
      }
      
 if($_POST['LeadStatus']==4){$title.=' Status:<b><i> Concept Proposal Submitted</i></b>';}
 if($_POST['LeadStatus']==5){$title.=' Status:<b><i> DPR Submitted</i></b>';} 
 if($_POST['LeadStatus']==0){$title.=' Status:<b><i> Enquriy Registered</i></b>';} 
 if($_POST['LeadStatus']==26){$title.=' Status:<b><i> Feasibility Request Submitted</i></b>';}
 if($_POST['LeadStatus']==10){$title.=' Status:<b><i> Feasibility Study</i></b>';}
 if($_POST['LeadStatus']==6){$title.=' Status:<b><i> Sale Order Registered</i></b>';}
      ...
      echo "<thead>
         ...
         <th width='4%'>" . _('Sl no') . "</th>  
                <th width='4%'>" . _('Leadno') . "</th>  
         <th width='14%'>" . _('Institution Name/<br>Customer Name') . "</th>
          <th width='14%'>" . _('Lead Date') . "</th>
         <th width='13%'>" . _('Contact No') . "</th>
         <th width='14%' class='dist'>" . _('District') . "</th> ";
...
/*  $sql="SELECT bio_cust.cust_id AS custid,  
 ...
 $sql="
SELECT DISTINCT  bio_leads.leadid AS leadid, date_format(bio_leads.leaddate,'%d-%m-%Y') as 'leaddate',bio_cust.cust_id AS custid, bio_cust.custname AS custname, 
    bio_status.biostatus, bio_cust.houseno AS houseno, bio_cust.housename AS housename, bio_cust.area1 AS place, lead_team.teamid,
    ...
    bio_enquirytypes.enquirytype AS enqtype, bio_leads.outputtypeid AS outputtypeid,  
    bio_cust.district AS districtid,bio_leads.grade, 
    bio_leadteams.teamname AS teamname, bio_leadsources.sourcename AS sourcename, bio_cust.custmail AS custmail, 
     bio_cust.state AS state, bio_district.district AS district, 
    bio_office.id AS officeid, bio_office.office AS office, LEFT(bio_leads.remarks,53) AS remarks, bio_leads.created_by, bio_emailstatus.emailstatus
FROM bio_leads
left join lead_team on bio_leads.leadid=lead_team.leadid
LEFT JOIN bio_enquirytypes ON bio_leads.enqtypeid=bio_enquirytypes.enqtypeid 
LEFT JOIN bio_leadteams ON bio_leadteams.teamid=lead_team.teamid
...
WHERE 
lead_team.teamid IN ($team_array)  
AND bio_leads.leadstatus not in (20,21,22,23,24)
...
/*     if (isset($_POST['off']))    {
     ...
     if (($_POST['off']!='')&&($_POST['off']!='0') && (($_POST['team']=='')||($_POST['team']=='0')))
    {
               ...
               $sql.=" AND lead_team.teamid in (Select teamid from bio_leadteams where office_id=".$_POST['off'].") ";    

        
    }
    else      if (($_POST['off']!='')&&($_POST['off']!='0') && ($_POST['team']!='')||($_POST['team']!='0'))
                     {
                                     $sql.=" AND lead_team.teamid in ($team_array)"; 
    ...
    $sql.=" AND lead_team.teamid='".$_POST['team']."'";    
    }
    ...
    if (isset($_POST['grade_filter'])) {   
    if ($_POST['grade_filter']!="") {        
    $sql.=" AND bio_leads.grade='".$_POST['grade_filter']."'";      
    }
    }
      if($_POST['lead'])
      {
            $sql.=" AND bio_leads.leadid='".$_POST['lead']."'";     
      }
...
//             
//}
     $sql.=" And bio_leads.leadstatus not in(20,21,22,23,24) ORDER by bio_leads.leadid desc";
  // echo $sql;       
...
*/
      ...
      {
      $remarks=substr($remarks,0,50);
       $remarks= $remarks."....";
      }
      
      echo "<input type='hidden' id='leadid' name='leadid' value='$leadid'>";    
echo "<td width=4%'>$slno</td>";
echo "<td width=4%'>$leadid</td>";

        echo "<td width='15%'>$custname</td>";                 echo "<td width='15%'>".$myrow['leaddate']."</td>";    
        echo"<td width='14%'>$custmob</td>";
        echo"<td width='15%' class='dist'>".$myrow['district']."</td>";
    if(!isset($_GET['team']))
  {      
        echo"<td width='10%' class='team'>$teamname</td>";  
  }
        echo"<td width='10%' class='leadstatus'>".$myrow['biostatus']."</td>";  
        
        $gra=$myrow[grade];
if($gra==A)
{
echo "<td  bgcolor='#339900' width='10%'><b>".$myrow[grade]."</b>&nbsp; &nbsp; 
             <select name=grade onchange=setgrade(this.value,'$leadid')>
                <option selected value=0>".$myrow[grade]."</option>
...
{
 echo '<td  bgcolor=orange width="10%"><b>'.$myrow[grade].'</b>&nbsp; &nbsp; 
             <select name=grade onchange=setgrade(this.value,'.$leadid.')>
                <option selected value=0>'.$myrow[grade].'</option>
                <option value="A">A</option>
                <option value="B">B</option>   
...
{
 echo '<td  bgcolor="#0099CC" width="10%"><b>'.$myrow[grade].'</b>&nbsp; &nbsp; 
             <select name=grade onchange=setgrade(this.value,'.$leadid.')>
                <option selected value=0>'.$myrow[grade].'</option>
                <option value="A">A</option>
                <option value="B">B</option>   
...
{
echo '<td  bgcolor="#CCFF33" width="10%"><b>'.$myrow[grade].'</b>&nbsp; &nbsp; 
             <select name=grade onchange=setgrade(this.value,'.$leadid.')>
                <option selected value=0>'.$myrow[grade].'</option>
                <option value="A">A</option>
                <option value="B">B</option>   
...
{
 echo '<td  bgcolor=Pink width="10%"><b>'.$myrow[grade].'</b>&nbsp; &nbsp; 
             <select name=grade onchange=setgrade(this.value,'.$leadid.')>
                <option selected value=0>'.$myrow[grade].'</option>
                <option value="A">A</option>
                <option value="B">B</option>   
...
/*      echo"<td width='10%'><b>".$myrow[grade]."</b>&nbsp; &nbsp; 
             <select name=grade onchange=setgrade(this.value,'$leadid')>
                <option selected value=0>".$myrow[grade]."</option>
                <option value='A'>A</option>
                <option value='B'>B</option>   
...
var dist=document.getElementById('byplace').value; 
...
xmlhttp.open("GET","bio_setgrade.php?grade=" +str+ "&leadid=" +str1 ,true);
xmlhttp.send();
