<?php
$PageSecurity = 80;
/* $Id: Customers.php 5063 2012-03-11 19:41:09Z vvs2012 $ */
...
include('includes/SQL_CommonFunctions1.inc');
...
if (isset($_POST['submit'])) {
	...
	if ($myrow[0]>0 and isset($_POST['New'])) {
		...
		$Errors[$i] = 'Address6';
		$i++;
	}/* elseif (!is_numeric(number_format($_POST['CreditLimit']))) {
		$InputError = 1;
		prnMsg( _('The credit limit must be numeric'),'error');
		$Errors[$i] = 'CreditLimit';
		$i++;
	} elseif (!is_numeric(number_format($_POST['PymtDiscount']))) {
		$InputError = 1;
		...
		$i++;
	}/* elseif (!is_numeric(number_format($_POST['Discount']))) {
		$InputError = 1;
		prnMsg( _('The discount percentage must be numeric'),'error');
		$Errors[$i] = 'Discount';
		$i++;
	} elseif (number_format($_POST['CreditLimit']) <0) {
		$InputError = 1;
		prnMsg( _('The credit limit must be a positive number'),'error');
		$Errors[$i] = 'CreditLimit';
		$i++;
	} elseif ((number_format($_POST['PymtDiscount'])> 10) OR (number_format($_POST['PymtDiscount']) <0)) {
		$InputError = 1;
		prnMsg( _('The payment discount is expected to be less than 10% and greater than or equal to 0'),'error');
		$Errors[$i] = 'PymtDiscount';
		$i++;
	} elseif ((number_format($_POST['Discount'])> 100) OR (number_format($_POST['Discount']) <0)) {
		$InputError = 1;
		...
		if (!isset($_POST['New'])) {
			...
			if ($myrow[0] == 0) {
			  $sql = "UPDATE debtorsmaster SET
					...
					discount='" . number_format($_POST['Discount'])/100 . "',
					discountcode='" . $_POST['DiscountCode'] . "',
					pymtdiscount='" . number_format($_POST['PymtDiscount'])/100 . "',
					creditlimit='" . number_format($_POST['CreditLimit']) . "',
			...
			$sql = "INSERT INTO debtorsmaster (
				...
				VALUES ('" . $_POST['DebtorNo'] ."',
					...
					'" . $_POST['HoldReason'] . "',
					'" . $_POST['PaymentTerms'] . "',
					'" . number_format($_POST['Discount'])/100 . "',
					'" . $_POST['DiscountCode'] . "',
					'" . number_format($_POST['PymtDiscount'])/100 . "',
					'" . number_format($_POST['CreditLimit']) . "',
					'" . $_POST['SalesType'] . "',
					'" . $_POST['AddrInvBranch'] . "',
					'" . $_POST['TaxRef'] . "',
	...
	echo '</table></td>
			<td><table class="selection">
				...
				<tr>
					...
					<td><input tabindex="14" type="text" class="number" name="CreditLimit" value="" size="16" maxlength="14" /></td>
				</tr>
				<tr>
