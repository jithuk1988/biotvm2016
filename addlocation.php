...
if($_POST['reload_flag']==1)
{
    ?>
    <script>  window.close();  
          window.opener.location='bio_documentmanagement.php?orderno='+<?php echo $_POST['order'];?>;  
...
//  echo '</select>';
  ...
  if($_GET['lsg'])
  {
   ...
   echo"<td><input type='submit' name='submit1' id='submit1' value='submit' onclick=' if(validate()==1)return false;'></td></tr>";
      ...
      $flag=1;
            echo "<input type='hidden' name='reload_flag' id='reload_flag' value='".$flag."'>"; 
            echo "<input type='hidden' name='order' id='order' value='".$_GET['orderno']."'>";    
  }else{
