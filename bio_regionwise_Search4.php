...
{            
   if($str=='new'){
      ...
      {  
         if ( $_POST['enq']==1){                                   
           $sql_function .= " AND salesorders.debtorno LIKE 'D%'";  
         }else if ( $_POST['enq']==2){                                   
           $sql_function .= " AND salesorders.debtorno LIKE 'C%'";                 
         }else if ( $_POST['enq']==3){                                   
           $sql_function .= " AND salesorders.debtorno LIKE 'L%'";                 
         }
      }        
   }elseif($str=='old'){
      if ($_POST['enq']!=0)
      {  
         if ( $_POST['enq']==1){                                   
           $sql_function .= " AND bio_oldorders.debtorno LIKE 'D%'";  
         }else if ( $_POST['enq']==2){                                   
           $sql_function .= " AND bio_oldorders.debtorno LIKE 'C%'";                 
         }else if ( $_POST['enq']==3){                                   
           $sql_function .= " AND bio_oldorders.debtorno LIKE 'L%'";                 
         }
      }   
   }
    ...
    if (isset($_POST['country']))    {            
     if($_POST['country']!=0)   {                              
     $sql_function .=" AND custbranch.cid=".$_POST['country'];    }
     }
                                                                                
    if (isset($_POST['state']))    {
     if($_POST['state']!=0)   {
     $sql_function .=" AND custbranch.stateid=".$_POST['state'];    }
     }
     
    if (isset($_POST['district']))    {
     if($_POST['district']!=0)   {
     $sql_function .=" AND custbranch.did=".$_POST['district'];
      
    if (isset($_POST['lsgType']))    {
     if($_POST['lsgType']!=0)   {
       if($_POST['lsgType']!=NULL)   {     
         
     $sql_function .=" AND custbranch.LSG_type=".$_POST['lsgType'];    
       } 
    if (isset($_POST['lsgName']) && $_POST['lsgName']!="")    {
     if($_POST['lsgType']==1 OR $_POST['lsgType']==2)   {
     $sql_function .=" AND custbranch.LSG_name=".$_POST['lsgName'];    }
    ...
    if (isset($_POST['gramaPanchayath']))    {  
      ...
      $sql_function .=" AND custbranch.block_name=".$_POST['gramaPanchayath'];    }       
     }
     }
     }
     }   
...
//       if($str2=='sqlview_pending_old') {
//                 $sqlview_pending=$sql_function;
//                 return $sqlview_pending; 
//       }
...
if(isset($_POST['filter'])){
   ...
   if(isset($_POST['lsgType']) && $_POST['lsgType']==3) {
        ...
        $groupby=" GROUP BY custbranch.block_name,";                                      $lsg="panchayath";     $having=" HAVING (custbranch.block_name!=0 AND custbranch.block_name is not null)";
        }
    }elseif(isset($_POST['lsgType']) && $_POST['lsgType']==2 && $_POST['lsgName']=="") {
        $groupby=" GROUP BY custbranch.LSG_name,";                                        $lsg="municipality";   $having=" HAVING (custbranch.LSG_name!=0 AND custbranch.LSG_name is not null)";
    }elseif(isset($_POST['lsgType']) && $_POST['lsgType']==1) {
        $groupby=" GROUP BY ";                                                            $lsg="corporation";
    }elseif(isset($_POST['state']))  {
        $groupby=" GROUP BY custbranch.did,";                                             $lsg="district";       $having=" HAVING (custbranch.did!=0 AND custbranch.did is not null)";
    }elseif(isset($_POST['country']) && $_POST['country']!=0)  {
        $groupby=" GROUP BY custbranch.stateid,";                                         $lsg="state";
    }    
...
SELECT docno,bio_document_master.document_name,COUNT( * ) AS c1,0 as c2,bio_district.district,bio_municipality.municipality,bio_panchayat.name AS panchayath,bio_block.block,bio_corporation.corporation,
       debtorsmaster.did,debtorsmaster.LSG_name,debtorsmaster.block_name
...
/*      if ($_POST['enq']!=0)
    ...
    if (isset($_POST['country']))    {
     if($_POST['country']!=0)   {
     $sqlview_pending .=" AND custbranch.cid=".$_POST['country'];    }
     }
                                                                                
    if (isset($_POST['state']))    {
     if($_POST['state']!=0)   {
     $sqlview_pending .=" AND custbranch.stateid=".$_POST['state'];    }
     }
     
    if (isset($_POST['district']))    {
     if($_POST['district']!=0)   {
     $sqlview_pending .=" AND custbranch.did=".$_POST['district'];
      
    if (isset($_POST['lsgType']))    {
     if($_POST['lsgType']!=0)   {
       if($_POST['lsgType']!=NULL)   {  
         
     $sqlview_pending .=" AND custbranch.LSG_type=".$_POST['lsgType'];    
       } 
    if (isset($_POST['lsgName']) && $_POST['lsgName']!="")    {
     if($_POST['lsgType']==1 OR $_POST['lsgType']==2)   {
     $sqlview_pending .=" AND custbranch.LSG_name=".$_POST['lsgName'];    }
    ...
    if (isset($_POST['gramaPanchayath']))    {  
      if($_POST['gramaPanchayath']!=0 OR $_POST['gramaPanchayath']!=NULL)   {
      $sqlview_pending .=" AND custbranch.block_name=".$_POST['gramaPanchayath'];    }       
     }
     }
     }
     }   
...
$sqlview_pending .=" $groupby bio_documentlist.docno $having";     
...
$sqlview_pending_old .= filtering("old");          
$sqlview_pending_old .=" $groupby bio_oldorderdoclist.docno $having";     
...
$sqlview_received .= filtering("new");         
$sqlview_received .=" $groupby bio_documentlist.docno $having";      
...
$sqlview_received_old .= filtering("old");                                 
$sqlview_received_old .=" $groupby bio_oldorderdoclist.docno $having";    
...
elseif($_POST['enq']==2){ $docs=16; }
    ...
    $pan=$myrow_viewdocs['block_name'];
            ...
            if( $i%$docs==0 )  {
            echo"<tr><td width=100px><a style='cursor:pointer;' onclick='passid($did,$nam,$pan)'><b>$regionName:</b>&nbsp;</a><i>$nooforders</i></td></tr>";     
            }
