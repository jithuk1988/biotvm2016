...
$newdate = date ( 'Y-m-j' , $newdate );
 ...
 if($_SESSION['UserStockLocation']==1){
   $sql="SELECT bio_leads.leadid,
           ...
           AND bio_leads.leaddate BETWEEN '$newdate' AND '$date' ";
...
if($_GET['service']==2){
    echo"<td>Services</td><td><select name='productservices' id='productservices'><option value='FEA'>Feasibility study</option></select></td>";
}


if($_GET['service']==1||$_GET['service']==3){
echo'<td>Product Services</td><td>';
if($_GET['service']==1){  $sql1="SELECT * FROM stockcategory WHERE stockcategory.categoryid='DOMP'";
$s=0;}else{$s=1;
  $sql1="SELECT * FROM stockcategory";
  }
  $result1=DB_query($sql1, $db);
  echo '<select name="productservices" id="productservices" style="width:190px" onchange=output(this.value)>';
  $f=0;
...
if($_GET['output']!=''){
    echo'<td>Output Type</td><td>';
    if(($_GET['output']==1)&&($_GET['plant']=='DOMP')){$sql1="SELECT * FROM bio_outputtypes WHERE outputtypeid=1";
      }
      else{ $sql1="SELECT * FROM bio_outputtypes";}
  $result1=DB_query($sql1, $db);
