...
if(isset($_POST['close_incentive']))
{
     ...
     for($i=1;$i<=$count;$i++)
     {
         $sql_insert="INSERT INTO bio_incentive_leadsource(type,enqtype,sourceid,month,year,target,achieved,eligible,incentive)VALUES(3,'".$_POST['enq']."',
   ...
   $sql_closingdate="INSERT INTO bio_closingdate (type,enqtype,closedate) VALUES (5,'".$enq."','".$_POST['close_date']."')";
...
echo '<input type=hidden name=enq value="'.$enq.'">';
    ...
    $closedate=DB_query("SELECT MAX(closedate) as lastclose,month(MAX(closedate)) as lastmonth,year(MAX(closedate)) as lastyear FROM  bio_closingdate WHERE type=5 AND  enqtype='".$enq."' ",$db);
...
echo "<fieldset style='float:center;width:50%;overflow:scroll;'>";
       ...
       if($enq==1){ $d="<legend><h3>NETWORK/DOMESTIC INCENTIVES</h3>";} 
      elseif($enq==2){ $d="<legend><h3>NETWORK/INSTITUTION INCENTIVES</h3>";}
       elseif($enq==3){ $d="<legend><h3>NETWORK/LSGD INCENTIVES</h3>";} 
   echo $d;
       if($_POST['closing']!="")
    {
 ...
 $sql="SELECT bio_network_cust.cust_name,bio_network_cust.cust_code,SUM(salesorderdetails.quantity * stockitemproperties.value),bio_network_cust.id
FROM bio_network_cust
INNER JOIN bio_leadsource_dealstaff ON bio_leadsource_dealstaff.sourceid=bio_network_cust.id
...
WHERE bio_leadsource_dealstaff.sourcetype=16
...
while($row=DB_fetch_array($result)){
    ...
    $result_target=DB_query("SELECT target FROM  bio_target_leadsources WHERE leadsource=3 AND   enq_type='".$enq."'   AND   month='".$closedate_row['lastmonth']."'      
...
AND bio_achievement_outpolicy.leadsource=3 
...
WHERE bio_achievement_outpolicy.enq_type='".$enq."' AND bio_achievement_outpolicy.leadsource=3
AND bio_achievement_outpolicy.from< '".$row[2]."'
...
$incentive=$incentive_policy[0]*$row[2];
    ...
    echo"<input type=hidden name='incentive".$i."' id='incentive".$i."' value=".$incentive.">";
          ...
          echo"<input type=hidden name='sourceid".$i."' id='sourceid".$i."' value=".$row['id'].">";
}
...
echo '</form>' ;
     ...
     <td style="border-bottom:1px solid #000000">Contact No.</td>
                ...
                $sql_iinnce="SELECT bio_incentive_leadsource.sourceid,bio_incentive_leadsource.month,bio_incentive_leadsource.year,bio_incentive_leadsource.target, bio_incentive_leadsource.achieved,bio_incentive_leadsource.eligible,bio_incentive_leadsource.incentive,bio_network_cust.cust_name,bio_network_cust.phoneno,
                bio_network_cust.mobileno
                from  bio_incentive_leadsource,bio_network_cust
                WHERE bio_incentive_leadsource.enqtype='".$enq."'
                AND bio_incentive_leadsource.type=3
                AND bio_network_cust.id=bio_incentive_leadsource.sourceid
