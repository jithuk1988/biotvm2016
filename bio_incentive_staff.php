...
if(isset($_POST['close_incentive']))
{
     ...
     for($i=1;$i<=$count;$i++)
     {
         $sql_insert="INSERT INTO bio_incentive_leadsource(type,enqtype,sourceid,month,year,target,achieved,eligible,incentive)VALUES(1,'".$_POST['enq']."',
   ...
   $sql_closingdate="INSERT INTO bio_closingdate (type,enqtype,closedate) VALUES (4,'".$enq."','".$_POST['close_date']."')";
...
echo '<input type=hidden name=enq value="'.$enq.'">';
    ...
    $closedate=DB_query("SELECT MAX(closedate) as lastclose,month(MAX(closedate)) as lastmonth,year(MAX(closedate)) as lastyear FROM  bio_closingdate WHERE type=4 AND  enqtype='".$enq."' ",$db);
...
echo "<fieldset style='float:center;width:50%;overflow:scroll;'>";
       ...
       if($enq==1){ $d="<legend><h3>STAFF/DOMESTIC INCENTIVES</h3>";} 
      elseif($enq==2){ $d="<legend><h3>STAFF/INSTITUTION INCENTIVES</h3>";}
       elseif($enq==3){ $d="<legend><h3>STAFF/LSGD INCENTIVES</h3>";} 
   echo $d;
       if($_POST['closing']!="")
    {
 ...
 $sql="SELECT bio_emp.empname,bio_emp.empid,SUM(salesorderdetails.quantity * stockitemproperties.value) 
FROM bio_emp
 INNER JOIN bio_leadsource_dealstaff ON bio_leadsource_dealstaff.sourceid=bio_emp.empid
...
WHERE bio_leadsource_dealstaff.sourcetype=14
...
while($row=DB_fetch_array($result)){
    ...
    $result_target=DB_query("SELECT target FROM  bio_target_leadsources WHERE leadsource=1 AND   enq_type='".$enq."'   AND   month='".$closedate_row['lastmonth']."'      
...
AND bio_achievement_outpolicy.leadsource=1 
...
WHERE bio_achievement_outpolicy.enq_type='".$enq."' AND bio_achievement_outpolicy.leadsource=1
AND bio_achievement_outpolicy.from< '".$row[2]."'
...
$incentive=$incentive_policy[0]*$row[2];
    ...
    echo"<input type=hidden name='incentive".$i."' id='incentive".$i."' value=".$incentive.">";
          ...
          echo"<input type=hidden name='sourceid".$i."' id='sourceid".$i."' value=".$row['empid'].">";
}
...
echo '</form>' ;
     ...
     echo '<tr>
     <td style="border-bottom:1px solid #000000">Employee Name</td>                   
     <td style="border-bottom:1px solid #000000">Designation/Office</td>
                ...
                $sql_iinnce="SELECT bio_incentive_leadsource.sourceid,bio_incentive_leadsource.month,bio_incentive_leadsource.year,bio_incentive_leadsource.target, bio_incentive_leadsource.achieved,bio_incentive_leadsource.eligible,bio_incentive_leadsource.incentive,bio_emp.empname, bio_designation.designation,bio_office.office
                from  bio_incentive_leadsource,bio_emp,bio_office,bio_designation
                WHERE bio_incentive_leadsource.enqtype='".$enq."'
                AND bio_incentive_leadsource.type=1
                AND bio_emp.empid=bio_incentive_leadsource.sourceid
