...
$pagetype=3;
//echo "<a href='" . $rootpath . '/StockTakinghome.php?' . SID .'&DebtorNo='.$DebtorNo."'>" . _('Back to Stock Taking') . '</a><br>';
...
Function Is_ValidAccount ($ActNo) {
	...
	echo _('The prefix') . ' - ' . $BankPrefix . ' ' . _('is not a valid New Zealand Bank') . '.<br>' .
			_('If you are using Goldengate outside New Zealand error trapping relevant to your country should be used');
	return False;
	exit;
...
/*If the page was called without $SupplierID passed to page then assume a new supplier is to be entered show a form with a Supplier Code field other wise the form showing the fields with the existing entries against the supplier will show for editing with only a hidden SupplierID field*/
      ...
      echo "<fieldset style='width:35%;'>";     
  echo"<legend><h3>Task Creation</h3>";
  echo"</legend>";
  echo "<div id=periodictaskgen>";
	...
	echo '<table><tr><td>';
    echo'<table>';
	echo '<tr><td>' . _('Supplier Code') . ":</td><td><input type='text' name='SupplierID' id='SupplierID' size=13   maxlength=10  ></td></tr>"; //onchange='savecode(this.value)'
	echo '<tr><td>' . _('Supplier Name') . ":</td><td><input type='text' name='SuppName' size=30 maxlength=40></td></tr>";
	echo '<tr><td>' . _('Address Line 1 (Street)') . ":</td><td><input type='text' name='Address1' size=30 maxlength=40></td></tr>";
	echo '<tr><td>' . _('Address Line 2 (Suburb/City)') . ":</td><td><input type='text' name='Address2' size=30 maxlength=40></td></tr>";
	echo '<tr><td>' . _('Address Line 3 (State/Province)') . ":</td><td><input type='text' name='Address3' size=30 maxlength=40></td></tr>";
	echo '<tr><td>' . _('Address Line 4 (Postal Code)') . ":</td><td><input type='text' name='Address4' size=30 maxlength=40></td></tr>";

	$DateString = Date($_SESSION['DefaultDateFormat']);
	echo '<tr><td>' . _('Supplier Since') . ' (' . $_SESSION['DefaultDateFormat'] . "):</td><td><input type='text' 
    class='date' alt='".$_SESSION['DefaultDateFormat']."' name='SupplierSince' VALUE=$DateString size=12 maxlength=10></td></tr>";
    echo'</table>';
    echo'</td>';
    echo'<td><table>';
    
	echo '<tr><td>' . _('Bank Particulars') . ":</td><td><input type='text' name='BankPartics' size=27 maxlength=12></td></tr>";
	echo '<tr><td>' . _('Bank reference') . ":</td><td><input type='text' name='BankRef' VALUE=0 size=27 maxlength=12></td></tr>";
	echo '<tr><td>' . _('Bank Account No') . ":</td><td><input type='text' name='BankAct' size=27 maxlength=30></td></tr>";
	...
	echo '<tr><td>' . _('Factor Company') . ":</td><td><select name='FactorID' style='width:180px;'>";
	...
	DB_data_seek($result, 0);
	echo '</select></td></tr>';
	echo '<tr><td>' . _('Tax Reference') . ":</td><td><input type='text' name='TaxRef' size=27 maxlength=20></td></tr>";
	...
	echo '</select></td></tr><tr><td>' . _('Remittance Advice') . ":</td><td><select name='Remittance' style='width:180px;'>";
	echo '<option VALUE=0>' . _('Not Required');
	echo '<option VALUE=1>' . _('Required');

	echo '</select></td></tr>';

	echo '<tr><td>' . _('Tax Group') . ":</td><td><select name='TaxGroup' style='width:180px;'>";
	...
	echo "</select></td></tr>";
   // echo"<div id='mag'>cdgsd</div>";
   //echo"<td align=center><a  style='cursor:pointer;'  id='".$_SESSION['supid']."' onclick='openwindo(this.id)'>" ._('Add Items') . " </a></td> ";
    

  echo"<tr><td></td>";
    echo"</table></td></table><p><div class='centre'><input type='Submit' name='submit' VALUE='" . _('Insert New Supplier') . "'>";
	echo '</div></form>';   
      echo "</div>";
    
   echo "</fieldset>";
    echo "<div id=supplierlist>"; 
   echo "<fieldset style='width:70%;'>";     
  echo"<legend><h3>Supplier list</h3>";
  echo"</legend>";

   echo'<table style="width:100%;" ><tr><th>Slno</th><th>Supplier Id</th><th>Supplier Name </th><th>Supplier City</th><th>Add Items</th></tr>';
$sql3="SELECT `supplierid`,`suppname`,`address2` FROM `suppliers`";
...
<td align=center>".$row3[supplierid]."</td>
<td align=center>".$row3[suppname]."</td>
<td align=center>".$row3[address2]."</td>
<td align=center><a  style='cursor:pointer;'  id='$SupplierID' onclick='openwindo(this.id)'>" ._('Add Items') . " </a></td> ";
...
echo"<div id=msg>fhgd</div>";
	if (!isset($_POST['New'])) {
		...
		echo "<input type=hidden name='SupplierID' id='SupplierID' VALUE='$SupplierID'>";
...
function openwindo(lead)  {
  ...
  controlWindow=window.open('bio_item_select_supplier.php?supplierid='+lead,'addsubsidy','toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width='+w+', height='+h+', top='+top+', left='+left);
}
function savecode(code)
{var sup_code=code
    alert(sup_code);
    
document.getElementById('mag').innerHTML='<?php  $_SESSION['supid']=$sup_code; echo "jjjjjjjj";  ?> ';  
