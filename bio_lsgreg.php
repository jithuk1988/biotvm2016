...
echo '<center><font style="color: #333;
      ...
      $schemeid="";
       ...
       {    
        ...
        $sql_cust = "UPDATE bio_cust SET nationality = '".$lsg_country."',
                                         ...
                                         custmob = '".$lsg_mobile."',
                                         custmail = '".$lsg_email."'
                                   WHERE cust_id='".$_POST['custid']."'";  
            DB_query($sql_cust,$db);           
                       
       $sql_leads ="UPDATE bio_leads SET cust_id=".$_POST['custid'].",            
                                         leaddate='".date("Y-m-d")."',
                                         schemeid='".$schemeid."',  
                                         remarks= '".$_POST['remarks']."',  
                                         enqtypeid=3
                                   WHERE cust_id='".$_POST['custid'] ."'";                   
            DB_query($sql_leads,$db);             
                                
                               
        $sql =  "UPDATE bio_lsgreg SET leadid = ".$_POST['leadid'].",                  
                                  ...
                                  WHERE bio_lsgreg.id=$SelectedType";
              DB_query($sql,$db);        
 ...
 if ( isset($SelectedType) && $SelectedType!="")
 {
     ...
     $sql_cust="SELECT `bio_cust`.`cust_id`,                                
     ...
     $lsg_leadid    = $myrow['leadid']; 
     $lsg_custid    = $myrow['cust_id']; 
   ...
   echo"<input type=hidden name=leadid id=leadid value=$lsg_leadid>";
   echo"<input type=hidden name=custid id=custid value=$lsg_custid>"; 
 }        
