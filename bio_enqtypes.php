...
if (isset($_POST['submit'])) {
	...
	// Fetch the default price list.
		$sql = "SELECT confvalue
					FROM config
					WHERE confname='DefaultEnquiryType'";
		$result = DB_query($sql,$db);
		$EnquiryTypeRow = DB_fetch_row($result);
		$DefaultEnquiryType = $EnquiryTypeRow[0];

	// Does it exist
		$checkSql = "SELECT count(*)
			     FROM bio_enquirytypes
			     WHERE enqtypeid = '" . $DefaultEnquiryType . "'";
		$checkresult = DB_query($checkSql,$db);
		$checkrow = DB_fetch_row($checkresult);
	...
	// If it doesnt then update config with newly created one.
		if ($checkrow[0] == 0) {
			$sql = "UPDATE config
					...
					WHERE confname='DefaultEnquiryType'";
			$result = DB_query($sql,$db);
			$_SESSION['DefaultEnquiryType'] = $_POST['TypeID'];
		}
...
echo "<table border=0 style='width:30%';><tr><td>";  
  echo "<fieldset style='width:340px;height:200px'>";
   echo "<legend><h3>Enquiry Type Master</h3></legend>";
if (! isset($_GET['delete'])) {
	...
	echo '<tr><td>' . _('Enquiry Type') . ':</td>
			<td><input type="text" name="TypeName" id="enqid" value="' . $_POST['TypeName'] . '"></td>
		</tr>';
...
echo "<tr><td>";
   ...
   echo "<legend><h3>Enquiry Type Master Created</h3></legend>";
if (!isset($SelectedType)){
...
<script language="javascript">
  document.getElementById('enqid').focus();
