...
/* $Id: SelectWorkOrder.php 4537 2011-04-03 03:24:25Z daintree $*/
$pagetype=1;
$PageSecurity = 80;
...
if (!isset($StockID)) {
	...
	if (!isset($_REQUEST['WO']) or ($_REQUEST['WO']=='')){
		...
		echo '&nbsp;&nbsp;<a href="' . $rootpath . '/WorkOrderEntry_new2.php?' . SID . '">' . _('New Work Order') . '</a></td></tr></table><br />';
	}
	...
	if (isset($_REQUEST['WO']) && $_REQUEST['WO'] !='') {
			$SQL = "SELECT workorders.wo,
					...
					AND workorders.wo='". $_REQUEST['WO'] ."'
                     ...
                     AND  woitems.stockid  IN(SELECT stockid FROM stockmaster WHERE categoryid IN (SELECT subcatid FROM bio_maincat_subcat WHERE maincatid =1))
                     UNION 
SELECT workorders.wo, woitems.stockid, stockmaster.description, woitems.qtyreqd, woitems.qtyrecd, workorders.requiredby 
FROM workorders 
INNER JOIN woitems ON workorders.wo=woitems.wo 
INNER JOIN stockmaster ON woitems.stockid=stockmaster.stockid
 WHERE workorders.closed='" . $ClosedOrOpen . "'
 AND workorders.wo='". $_REQUEST['WO'] ."'
 AND (woitems.qtyreqd-woitems.qtyrecd !=0)
 AND workorders.WO NOT IN (SELECT workorders.wo
FROM workorders 
INNER JOIN woitems ON workorders.wo=woitems.wo 
INNER JOIN stockmaster ON woitems.stockid=stockmaster.stockid
WHERE workorders.closed='" . $ClosedOrOpen . "'
...
GROUP BY workorders.wo";//group BY workorders.wo
			}
	...
	while ($myrow=DB_fetch_array($WorkOrdersResult)) {
		...
		$ModifyPage = $rootpath . '/WorkOrderEntry_new2.php?WO=' . $myrow['wo'];
		$Status_WO = $rootpath . '/WorkOrderStatus.php?WO=' .$myrow['wo'] . '&StockID=' . $myrow['stockid'];
		$Receive_WO = $rootpath . '/WorkOrderReceive_new.php?WO=' .$myrow['wo'] . '&StockID=' . $myrow['stockid'];
		...
		printf("<td><a href='%s'>%s</a></td>
				...
				$FormatedRequiredByDate);// <td><a href='%s'>" . _('Issue To') . "</a></td>         //$Issue_WO,
