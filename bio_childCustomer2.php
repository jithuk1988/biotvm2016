...
echo '<center><font style="color: #333;
 ...
 if ((isset($_GET['dealerdebtor'])) && ($_GET['dealerdebtor']!="")){
    $dealerdebtor = strtoupper($_GET['dealerdebtor']);  
} 
...
//       $enqtype=$_POST['enqtype'];
 ...
 ?>
 <script>
     location.href ="bio_childcustomer1.php";
</script>

<?php
}    
 
if(isset($dealerdebtor)){
       $sql_cust="SELECT `debtorsmaster`.`cid`,
                  ...
                  WHERE debtorsmaster.debtorno='".$_GET['dealerdebtor']."'";
} 
...
echo"<input type=hidden name=dealercode id=dealercode value='".$_GET['dealerdebtor']."'>";     
  ...
  echo "<fieldset style='float:right;width:90%;height:420px'>";         
  echo "<legend><h3>Plant Selection</h3>";
  echo "</legend>";
  
  
  if(isset($SelectedType)){    $dealerdebtor=$dealercode; 
                        }else{ $dealerdebtor=$_GET['dealerdebtor'];  }
  
  echo "<table style='margin-left:5%' >"; 
  
       $sql_dealers=" SELECT debtorsmaster.debtorno,debtorsmaster.name
                      FROM   debtorsmaster
                      WHERE  debtorno='".$dealerdebtor."'";     
       $result_dealers=DB_query($sql_dealers,$db);
   
  echo '<td>Dealer</td><td> <select name="dealer" id="dealer" style="width:200px" >';
  echo '<option value="">Select Dealer</option>';
   $f=0;
  while($myrow_dealers=DB_fetch_array($result_dealers))
  {         
  if ($myrow_dealers['debtorno']==$dealerdebtor) 
    {
...
echo"<div id=dealerdetail>";
                ...
                $sql_order="SELECT orderno,orddate FROM salesorders WHERE debtorno='".$dealerdebtor."'";
                $result_order=DB_query($sql_order,$db);
                
                echo '<tr><td>Dealer Orders</td><td> <select name="order" id="order" style="width:200px" onchange=selectplant(this.value)>'; 
            if(!isset($SelectedType))   {    echo '<option value=""></option>';     }
                while($myrow_order=DB_fetch_array($result_order))
                {
                  if($myrow_order['orderno']==$orderno){
                  echo "<option selected value=".$myrow_order['orderno'].">".$myrow_order['orderno']." - ".ConvertSQLDate($myrow_order['orddate'])."</option>";     
                  }else{
...
echo"<div id=plantselection>"; 
            ...
            if(isset($SelectedType)){ 
            echo"<br />"; 
            echo"<tr style='background:#D50000;color:white'><td>Slno</td><td>Plant</td><td>Quantity Ordered</td></tr>";  

                   $sql_plant="SELECT stockmaster.description,bio_childcustomerdetails.quantity
                                 FROM  bio_childcustomer,bio_childcustomerdetails,stockmaster
                                WHERE  bio_childcustomer.id=bio_childcustomerdetails.childid
                                  AND  bio_childcustomer.stockid=stockmaster.stockid
                                  AND  bio_childcustomer.id=".$SelectedType; 
                 ...
                 {  
                    ...
                    echo "<tr style='background:#000080;color:white'><td>".++$slno."
                                                                     ...
                                                                     <td><input type=text size=3px name=qtyedit id=qtyedit value=".$myrow_plant['quantity']."></td>
                                                              </tr>"; 
                 }    
...
echo"</td></tr>"; 
 ...
 echo '<select name="District" id="District" style="width:200px" tabindex=3  onchange="showtaluk(this.value)">';
  ...
  echo'</tr>'; 
           ...
           //LSG Type
if((isset($SelectedType)) || (isset($dealerdebtor))) {                
...
echo "<td><input type=text name='building_name' id='building_name' value='$building' tabindex=10 style='width:98%' ></td></tr>";      
 ...
 echo" <tr><td>Pin:</td><td><input type='text' name='Pin' id='Pin' value='$Pin' tabindex=13 onkeyup='' style='width:99%'></td></tr>";  
  ...
  echo "<td><input type=text name=mobile id=mobile  style='width:98%' value='$mobile' tabindex=16 ></td></tr>"; 
...
//  echo "<table style='border:1px solid #F0F0F0;width:830px;'>";              
 ...
 if(isset($dealerdebtor)) {          
       $sql.=" WHERE bio_childcustomer.dealercode='".$dealerdebtor."'";  
       }
       $sql.=" ORDER BY  bio_childcustomer.dealercode,bio_childcustomer.orderno";
...
xmlhttp.open("GET","bio_dealerdetails1.php?dealersorder=" + str ,true);
xmlhttp.send(); 
}   
