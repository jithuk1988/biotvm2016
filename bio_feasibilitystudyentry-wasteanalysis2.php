...
echo"<div id='feedstockdiv'>";
 ...
 if($myrow_dup[0]==0){
 $sql_fs_fd="SELECT * FROM bio_fs_feedstockdetails WHERE leadid=$lead_id";
 $result_fs_fd=DB_query($sql_fs_fd,$db);
 $result_count=DB_num_rows($result_fs_fd);

 if($result_count>0){
 while($myrow_fs_fd=DB_fetch_array($result_fs_fd)){
    
           $sql_1="INSERT INTO bio_feeddetailstemp(feedstockid,
                                                   weight,
                                                   feedstocksourceid,
                                                   number_source,
                                                   gas_out) 
                                           VALUES ('".$myrow_fs_fd['feedstockid']."',
                                                   '".$myrow_fs_fd['weight']."',
                                                   '".$myrow_fs_fd['feedstocksourceid']."',
                                                   '".$myrow_fs_fd['number_source']."',
                                                   '".$myrow_fs_fd['gasout']."')";
          $result_1= DB_query($sql_1,$db,$ErrMsg,$DbgMsg);       
 }    
 }
 }
