...
echo '<center><font style="color: #333;
    ...
    font-size:16px;
    font-family:Georgia;
    text-shadow: 1px 1px 1px #666;">Dealer Task View</font></center>';
...
echo "<input type=\"button\" value=\"Refresh this page\" onClick='window.location=\"bio_dealerdetail.php\";'>";
echo'</div>';
}
//echo"</div>"; 
echo"</div>";
...
/*$sql="SELECT bio_cust.cust_id,
 ...
 $sql="SELECT distinct bio_cust.cust_id,
             bio_leads.enqtypeid, 
             bio_leadtask.leadid,
             bio_cust.custname,
             bio_leads.grade,
...
if($_POST['task']==21)
{
 echo '<option selected value="21">Convert to customer</option>'; 
  echo '<option value="18">Contact Client</option>';
}else
{
  echo '<option selected value="18">Contact Client</option>';
echo '<option value="21">Convert to customer</option>';  
}                                                          
echo '</select></td>';
...
if($_SESSION['UserID']=='bdm_national' || $_SESSION['UserID']=='bde_national1' ||$_SESSION['UserID']=='bdm_international' ||$_SESSION['UserID']=='bde_international1') 
{
    ...
    if($taskid==21){
    echo"<td width='50px'><a style='cursor:pointer;' id='$leadid' onclick='selectT(this.id)'>" . _('Select') . "</a></td>";   
    }else{
    echo"<td width='50px'><a style='cursor:pointer;' id='$leadid' onclick='selectT(this.id);'>" . _('Select') . "</a></td>";       
    }
...
xmlhttp.onreadystatechange=function()
  ...
  if (xmlhttp.readyState==4 && xmlhttp.status==200)
    ...
    document.getElementById('loc').focus(); 
      ...
      if(str2==18){ 
      controlWindow=window.open("dealercontact.php?leadid=" + str1 + "&tid=" + str2 + "&enqid=" + enq,"RecreateSchedule","toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=no,resizable=no,width=980,height=500");
      }
      if(str2==20){ 
