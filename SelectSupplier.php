...
if (!isset($_POST['Search']) OR isset($_POST['Search']) or isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
...
if (isset($_SESSION['SupplierID'])) {
	...
	echo '</td><td valign=top class="select">'; /* Supplier Transactions */
        ...
        echo '<a href="' . $rootpath . '/PO_Items.php?NewOrder=Yes&SupplierID=' . $_SESSION['SupplierID'] . '">' . _('Enter a Purchase Order ') . '</a><br />';
...
if (!isset($_POST['Search']) or (isset($_POST['Search']))) {
	...
	while (($myrow = DB_fetch_array($result)) AND ($RowIndex <> $_SESSION['DisplayRecordsMax'])) {
		...
		echo '<td><input type=submit name="Select" value="'.$myrow['supplierid'].'" /></td>
            ...
            <td><a href="' . $rootpath . '/PO_Items.php?NewOrder=Yes&SupplierID=' . $myrow['supplierid'] . '">' . _('Purchase Order ') . '</a></td>
			</tr>';
		$RowIndex = $RowIndex + 1;
