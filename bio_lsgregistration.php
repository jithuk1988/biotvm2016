...
if($_GET['taluk']){  
    ...
    $result=DB_query($sql,$db);
  echo '<select name="taluk" id="taluk" style="width:200px" tabindex=12 onchange="showvillage(this.value)">';
   $f=0;
  while($myrow1=DB_fetch_array($result))
  {
...
if($_GET['village']){  
    ...
    $result=DB_query($sql,$db);
  echo '<select name="village" id="village" style="width:190px">';
   $f=0;
  while($myrow1=DB_fetch_array($result))
  {
...
if($_GET['block']){ 
        ...
        $district=$row['district'];
          ...
          if($cid==1 && $sid==14)  
          {
              ...
              if($district==8){$distname='Kozhikode';} 
                    /*echo '<table align=left ><tr><td width=115px>' . _('Corporation') . ':</td>';
                    echo '<td><select name="lsgName" readonly id="lsgName" style="width:280px" tabindex=11>';
                    echo "<option value='".$district."'>".$distname."</option>"; 
                    echo '</select></td>';   
                    echo '</tr></table>';      
                                        */
     echo '<tr><td width=30%>Corporation</td>';
     echo "<td><input type=text name='main_share' id='main_share' style='width:280px' tabindex=14></td></tr>";                              
    ...
    elseif($_GET['block']==2)        //Municipality
    {
     ...
     echo "<td><input type=text name='district_share' id='district_share' style='width:280px' tabindex=14'></td></tr>";            
        ...
        /*echo '<table align=right ><tr><td width=115px>' . _('Municipality Name') . ':</td>';    
        ...
        echo '<td><select name="lsgName" id="lsgName" style="width:280px" tabindex=11>';
         $f=0;
         while($myrow1=DB_fetch_array($result))
         {
    ...
    elseif($_GET['block']==3)          //block Panchayath
    {
     ...
     /*echo '<tr><td>Block</td>';
     echo "<td><input type=text name='block' id='block' style='width:280px' tabindex=14'></td></tr>";        */
        
        
        
         /*echo '<table align=left ><tr><td width=115px>' . _('Block Name') . ':</td>';    
         
         $sql="SELECT * FROM bio_block WHERE country=$country1 AND state=$state AND district=$district ORDER BY bio_block.block ASC";
         $result=DB_query($sql,$db);
         
         echo '<td><select name="lsgName" id="lsgName" style="width:280px" tabindex=11 onchange="showgramapanchayath(this.value)">';
         $f=0;
         while($myrow1=DB_fetch_array($result))
         {
         if ($myrow1['id']==$_POST['lsgName'])
     ...
     /*     
     echo '<tr><td>Panchayat</td>';
     echo "<td><input type=text name='panchayat' id='panchayat' style='width:280px' tabindex=14'></td></tr>"; */     
 ...
 echo "<td><input type=text name='block_share' id='block_share' style='width:280px' tabindex=14'></td></tr>";   
 ...
 echo "<td><input type=text name='district_share' id='district_share' style='width:280px' tabindex=14'></td></tr>";             
      ...
      /*echo '<tr id="showgramapanchayath"><td width=115px>' . _('Panchayat Name') . ':</td>';         
         
         $sql="SELECT * FROM bio_panchayat WHERE country=$country1 AND state=$state AND district=$district";
         $result=DB_query($sql,$db);
         
         echo '<td><select name="gramaPanchayath" id="gramaPanchayath" style="width:280px" tabindex=11>';
         $f=0;
         while($myrow1=DB_fetch_array($result))
         {
