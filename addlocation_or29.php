...
if($_POST['country']==1 && $_POST['State']==14){
    ...
    $sql13="INSERT into bio_lsg_fileno(LSG_type,LSG_id,fileno,
    room,
    rack,
    bin,
    lastfileno)
...
if (isset($_POST['submit1'] )){
    ...
    if($_POST['country']==1 && $_POST['State']==14){
     ...
     $sql="UPDATE bio_lsg_fileno SET room='".$_POST['room1']."' , rack='".$_POST['rack1']."' , bin='".$_POST['bin1']."', lastfileno=$lastfno WHERE LSG_type=$lsgtype AND LSG_id=(SELECT id FROM bio_corporation WHERE country=$cid AND state=$stateid AND district=$lsgname) AND id='".$row2['maxid']."' ";
     ...
     else 
     {
         ...
         $result2=DB_query($sql2,$db);
         $row2=DB_fetch_array($result2,$db);
          $sql="UPDATE bio_lsg_fileno SET room='".$_POST['room1']."' , rack='".$_POST['rack1']."', bin='".$_POST['bin1']."', lastfileno=$lastfno WHERE LSG_type=$lsgtype AND LSG_id=$lsgname AND id='".$row2['maxid']."' ";
     }
...
//  echo '</select>';
  ...
  if($_GET['lsg'])
  {
      ...
      echo"<legend>File Location</legend>";
        echo"<tr><th>Room</th><th>Rack</th><th>Bin</th><th>Last file no.</td></tr>";
