...
$title = _('InactiveLeads');
 

//$sql1="SELECT * FROM bio_leads
//        WHERE leadstatus=0";
//
$sql34="Select value from bio_changepolicy where policyname='No of days to became a lead inactive' ";
 ...
 $sql1="Select 
     ...
     WHERE
            ...
            bio_status.statusid=bio_leads.leadstatus AND
            bio_leads.teamid=bio_leadteams.teamid AND
            bio_leads.leaddate < (CURDATE() - INTERVAL $noday DAY) AND
            bio_leads.leadstatus=0
...
if(!isset($_POST['excelview'])){
 ...
 // include('includes/sidemenu.php');                                 
  ...
  echo '<center><font style="color: #333;
                           ...
                           font-size:16px;
                           font-family:Georgia;
                           text-shadow: 1px 1px 1px #666;">INACTIVE LEADS</font></center>';   
   ...
   echo "<legend><h3>Showing all Inactive Leads</h3>";
   echo "</legend>";   
...
//     $sql8="SELECT value from bio_changepolicy where  policyname='No of days to became a lead inactive '";
...
print'<script>
    function showactive(str1){
         ...
         window.location="bio_leads_reminderletter.php?lead=" + str1;
    }
    </script>';
 ...
 if(isset($_POST['excelview'])){ 
  ...
  while($myrow3=DB_fetch_array($result1))     {
//            
//      $date1=$myrow3['leaddate'];
//      $date2=date("Y-m-d");
...
//     $myrow4=DB_fetch_array($result4,$db);
//    
     $data= $data.$slno.",".$myrow3['leadid'].",".$myrow3['custname'].",".$myrow3['teamname'].",".ConvertSQLDate($myrow3['leaddate']).",".$myrow3['custmob'].",".$myrow3['custmail']."\n";    
     $slno++;    
      }
