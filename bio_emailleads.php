...
/* echo"<td>From Date:<input type='text' name='fromdate' id='fromdate' class=date alt=".$_SESSION['DefaultDateFormat']."></td>"; 
  echo"<td>To Date:<input type='text' name='todate' id='todate' class=date alt=".$_SESSION['DefaultDateFormat']."></td>";  */
...
echo '<select name="bycategory" id="bycategory" style="width:190px" tabindex=7 onchange="showsubmailcategory(this.value)">';
  ...
  echo '<option>';
   ...
   echo '</select></td>'; 
  echo"<td id='fromdateid'>From Date:<input type='text' name='fromdate' id='fromdate' class=date alt=".$_SESSION['DefaultDateFormat']."></td>"; 
  echo"<td id='todateid'>To Date:<input type='text' name='todate' id='todate' class=date alt=".$_SESSION['DefaultDateFormat']."></td>"; 
   echo"<td id='fromleaddateid'>Lead Date From:<input type='text' name='fromdate1' id='fromdate1' class=date alt=".$_SESSION['DefaultDateFormat']."></td>"; 
  echo"<td id='toleaddateid'>Lead Date To:<input type='text' name='todate1' id='todate1' class=date alt=".$_SESSION['DefaultDateFormat']."></td>";   
   
   echo"<td colspan=2 width=100%>"; 
   echo"<table id='showcatagry'>";
   echo"</table>";
   echo"</td>";
   echo"<td><input type='submit' name='filterbut' id='filterbut' value='search' onclick='viewdatefield();'></td>";  
...
if (isset($_POST['filterbut'])){    
 ...
 ";
  ...
  if ($_POST['bycategory']==1 ){    
   echo "<font size='+1' style='margin-left:25%'>Email Lead report of trade enquiry  </font> ";
  echo '<tr>  <th>' . _('Slno') . '</th> <th>' . _('Institution Name/ Customer Name') . '</th> <th>' . _('Contact No') . '</th>
   ...
   $sql="SELECT bio_cust.cust_id,
        ...
        FROM   bio_cust,
         ...
         WHERE bio_leads.leadid=bio_incidents.leadid
...
if($_POST['mailstatus']!=0){            
 $sql .= " AND bio_incidents.emailstatus=".$_POST['mailstatus']." ";  
}
if($_POST['leadstatus']!=0){            
 $sql .= " AND bio_leads.leadstatus=".$_POST['leadstatus']." ";  
}
if($_POST['email']!=''){
 $sql .= " AND bio_cust.custmail='".$_POST['email']."'";
}
if($_POST['fromdate1']!='' AND $_POST['todate1']!=''){
      $sql .= " AND bio_leads.leaddate BETWEEN '".FormatDateForSQL($_POST['fromdate1'])."' AND '".FormatDateForSQL($_POST['todate1'])."' ";
}


}elseif ($_POST['bycategory']==2){ 
echo "<font size='+1' style='margin-left:25%'>Email Lead report of technical collaboration </font> ";
...
if($_POST['email']!=''){
 ...
 echo "<font size='+1' style='margin-left:25%'>Email Lead report of careers</font> "; 
echo '<tr> <th>' . _('Slno') . '</th> <th>' . _('Customer Name') . '</th> <th>' . _('Contact No') . '</th><th>' . _('Email Type') . '</th>  <th>' . _ ('Category') . '</th></tr>';
...
if($_POST['email']!=''){
 ...
 echo "<font size='+1' style='margin-left:25%'>Email Lead report of complaints</font> ";                       
...
if($_POST['fromdate']!='' AND $_POST['todate']!=''){
      $sql .= " AND bio_incidents.createdon BETWEEN '".FormatDateForSQL($_POST['fromdate'])."' AND '".FormatDateForSQL($_POST['todate'])."' ";
}
...
?>
  ...
  function viewdatefield()
  {
      var str=document.getElementById("bycategory").value;  
                                             //  alert(str);
  ...
  function showMailAware(str1,str2){
        ...
        myRef = window.open("bio_viewmaildetails.php?id=" + str1 +"&category="+ str2,true);
             ...
             $('#fromdateid').fadeOut(1000);    
       $('#todateid').fadeOut(1000);
       $('#fromleaddateid').fadeOut(1000);     
       $('#toleaddateid').fadeOut(1000);
  ...
  function showsubmailcategory(str){
    ...
    if(str==1){
       $('#fromdateid').fadeOut(1000);
       $('#todateid').fadeOut(1000);
       $('#fromleaddateid').fadeIn(1000);     
       $('#toleaddateid').fadeIn(1000); 
    }else if(str==5){
       $('#fromdateid').fadeIn(1000); 
       $('#todateid').fadeIn(1000); 
              $('#fromleaddateid').fadeOut(1000);     
       $('#toleaddateid').fadeOut(1000);  
    }
    else{
...
xmlhttp.open("GET","bio_showcat.php?category=" + str,true);
xmlhttp.send();
}
