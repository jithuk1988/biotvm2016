...
/* $Id: WWW_Users.php 4591 2011-06-09 10:33:38Z daintree $*/
               $PageSecurity = 15;
...
echo '<tr><td>' . _('Password') . ':</td>
    <td><input type="password" name="Password" size="22" maxlength="20" value="' . $_POST['Password'] . '"></td></tr>';
    
echo'<tr>';
echo'<td>' . _('Office') . ':</td>
<td><select name=office id=office onchange="dept(this.value)">';
...
echo'</tr>';    
//-------------------------------------Ajax---------------------------------------------------------------------------- 
//    echo'<tr colspan=2><div id=contact1>';
echo'<tr><td colspan=2><div id=contact1>';
...
/* If its the first time the page has been displayed with no parameters then none of the above are true and the list of Users will be displayed with links to delete or edit each. These will call the same page again and allow update/input or deletion of the records*/
  ...
  $sql = "SELECT
            www_users.userid as userid,
            www_users.realname as realname,
            www_users.phone as phone,
            www_users.email as email,
            www_users.blocked,
            CASE WHEN DATE_FORMAT(www_users.lastvisitdate,'%d-%m-%Y %r') IS NULL THEN 'No Visit' ELSE DATE_FORMAT(www_users.lastvisitdate,'%d-%m-%Y %r') END AS 'lastvisitdate' ,
CASE WHEN www_users.blocked=1 THEN 'Blocked' ELSE 'Open' END AS 'blockstatus' ,            
            securityroles.secrolename
        FROM www_users,securityroles
WHERE www_users.fullaccess=securityroles.secroleid order by blocked,userid asc";
    $result = DB_query($sql,$db);
    ...
    while ($myrow = DB_fetch_row($result)) {
     ...
     else
     {
        if ($k==1){
           ...
           echo "<td>".$myrow[0]."</td><td>".$myrow[1]."</td><td>".$myrow[2]."</td><td>".$myrow[3]."</td><td>".$myrow[5]."</td><td>".$myrow[6]."</td><td>".$myrow[7]."</td><td><a href=".$_SERVER['PHP_SELF']."?&SelectedUser=".$myrow[0].">" . _('Edit') . "</a></td></tr>";
...
xmlhttp.onreadystatechange=function()
  ...
  if (xmlhttp.readyState==4 && xmlhttp.status==200)
    {
    document.getElementById("contact1").innerHTML=xmlhttp.responseText;

    }
  } 
xmlhttp.open("GET","bio_contact.php?con=" + str2,true);
xmlhttp.send();    
...
if (str=="")
  {
  document.getElementById("empcombo").innerHTML="";
  return;
  }
if (window.XMLHttpRequest)
  {// code for IE7+, Firefox, Chrome, Opera, Safari
...
xmlhttp.onreadystatechange=function()
  ...
  if (xmlhttp.readyState==4 && xmlhttp.status==200)
    {
    document.getElementById("empcombo").innerHTML=xmlhttp.responseText;

    }
  } 
xmlhttp.open("GET","bio_empdept.php?off=" + str,true);
xmlhttp.send();    
}
