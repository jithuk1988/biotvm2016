...
$title = _('Periodic Maintenance Task Scheduling');  
...
echo '<center><font style="color: #333;
    ...
    font-size:16px;
    font-family:Georgia;
    text-shadow: 1px 1px 1px #666;">Periodic Maintenance Task Scheduling</font></center>';
  echo'<a href="bio_pm_cust_list.php"><h4>BACK TO LIST</h4></a>';
  ...
  date_default_timezone_set ("Asia/Calcutta"); 
   $curdate = date("Y-m-d  H:i:s", time()) ;
    ...
    $sql_add="INSERT INTO bio_cstask(cstype,orderno,despatch_id,taskno,taskdescription,daystocomplete,prevtask,scheduleddate,team,status,cid,stateid,did,LSG_type,LSG_name,block_name)
                    values (2,'$orderno','$desid',$taskno,'$des','$days','".$_POST['no']."','$newdate2',$team,0,'".$_POST['cid']."',
                    '".$_POST['stateid']."' , '".$_POST['did']."', '".$_POST['LSG_type']."' , '".$_POST['LSG_name']."' , '".$_POST['block_name']."')";
                    $result_add=DB_query($sql_add,$db);                                 
       
      
                   $sql_get_maxid="SELECT max(id) AS maxid from bio_cstask WHERE cstype=2 AND orderno='".$orderno."' AND  despatch_id='".$_POST['desid']."' ";
            $result_get_maxid=DB_query($sql_get_maxid,$db);
            $max_id=DB_fetch_array($result_get_maxid);
            $maxid=$max_id['maxid'];
    
    $sql_trans="INSERT INTO bio_task_transaction (task_id,modified,date,user) VALUES ('".$maxid."',0,'".$curdate."','".$user."')" ;
   ...
   // $plant=$_POST['plant'];
    ...
    $sql_add="INSERT INTO bio_cstask(cstype,orderno,despatch_id,taskno,taskdescription,daystocomplete,prevtask,scheduleddate,team,status,cid,stateid,did,LSG_type,LSG_name,block_name)
                    values (2,'$orderno','".$_POST['desid']."',$i,'$des','$days','$prev','$start','$team',0,'".$_POST['cid']."',
                    '".$_POST['stateid']."' , '".$_POST['did']."', '".$_POST['LSG_type']."' , '".$_POST['LSG_name']."' , '".$_POST['block_name']."')";
    ...
    $newdate2 = date ( 'Y-m-d' , $newdate2 );
               $sql_add1="INSERT INTO bio_cstask(cstype,orderno,despatch_id,taskno,taskdescription,daystocomplete,prevtask,scheduleddate,team,status,cid,stateid,did,LSG_type,LSG_name,block_name)
                    values (2,'$orderno','".$_POST['desid']."',$prop_tno,'AMC Proposal',335,'".$_POST['no']."','$newdate2',''$team'',0,'".$_POST['cid']."',
                    '".$_POST['stateid']."' , '".$_POST['did']."', '".$_POST['LSG_type']."' , '".$_POST['LSG_name']."' , '".$_POST['block_name']."')";
    ...
    $team=$_POST['team'.$i];
          ...
          $sql_edit="UPDATE bio_cstask SET  taskdescription='".$des."' , daystocomplete='".$days."' ,
           scheduleddate='".$start."' , team='".$team."'
           WHERE cstype=2 AND despatch_id='".$desid."' 
          AND orderno='".$orderno."' AND taskno='".$_POST['task_no'.$i]."' ";
                   $result_adit=DB_query($sql_edit,$db);
         
                //Tasr transaction
                
                $sql_get_maxid="SELECT id from bio_cstask WHERE cstype=1 AND orderno='".$orderno."' AND  despatch_id='".$_POST['desid']."' AND taskno='".$_POST['task_no'.$i]."'";
            $result_get_maxid=DB_query($sql_get_maxid,$db);
            $max_id=DB_fetch_array($result_get_maxid);
            $maxid=$max_id['id'];
 ...
 if($orderno!='')
 {
    ...
    $sql1="SELECT custbranch.brname,
              ...
              bio_installationstatus.installation_date 
              from custbranch,salesorders,bio_district,bio_installationstatus 
              where 
              salesorders.debtorno=custbranch.debtorno
              AND salesorders.orderno=bio_installationstatus.orderno
              AND bio_district.did=custbranch.did
              AND    bio_district.stateid=custbranch.stateid
              AND    bio_district.cid=custbranch.cid
              AND bio_installationstatus.orderno=".$orderno;
    ...
    $expare = strtotime ( '+ 365 day' , strtotime ( $row1['installation_date'] ));
...
echo '<table class="selection">
            ...
            <tr>
                <th colspan ="3"><b>' . _('Plant installed date:') . ' ' . convertSQLDate($row1['installation_date']) . '</b></th>
            </tr>
            <tr>
                <th colspan ="3"><b>' . _('Warranty expiry date:') . ' ' . convertSQLDate($expare) . '</b></th>
            </tr>
            </table>
            <br />';
            ...
            $sql_new="SELECT count(id) as count from bio_cstask where orderno=$orderno AND despatch_id='$desid' AND cstype=2";
            ...
            $sql_new1="SELECT count(id) as count1 from bio_def_cstask where stockcode='$planttype' AND cstype=2";
...
//-------------- Viwe Assigned Tasks-----------------------
    ...
    echo"<tr><th>Slno</td>
         ...
         if($row_new['count']==0){
             ...
             $sql_assigned="SELECT * FROM bio_def_cstask 
                                                    where bio_def_cstask.stockcode='$planttype' AND cstype=2
                                                    ORDER BY bio_def_cstask.daystocomplete";
                        $result_assigned=DB_query($sql_assigned,$db);              
                        $k=0;
                        $slno=1;
                         $newdate=$row1['installation_date'];
                        ...
                        while($row_assigned=DB_fetch_array($result_assigned))
                        {
                          $days=$row_assigned['daystocomplete'];
                                    $newdate2 = strtotime ( '+'.$days.' day' , strtotime ($row1['installation_date']) );
     // echo $newdate = date ( 'Y-m-d' , $newdate );
    ...
    //        $row_team=DB_fetch_array($result_team);u
         ...
         //----------For existing entry, to update status of table
                                                    ...
                                                    where bio_cstask.despatch_id='$desid' 
                                                    AND orderno='$orderno' AND cstype=2
                                                    ORDER BY daystocomplete";
                        $result_assigned=DB_query($sql_assigned,$db);              
                        $k=0;
                        $slno=1;
    ...
    echo"<td><select name='statuschange' id='".$row_assigned['taskno']."' onchange=changestatus(this.id,$orderno,'$desid',$i,this.value);>";
...
//------------------------------------------------------ 
 ...
 echo'<input type=hidden name=desid id=desid value='.$desid.'>';  
echo'<input type=hidden name=new_year id=new_year value='.$row1['installation_date'].'>';
...
echo'<input type=hidden name=block_name id=block_name value='.$row1['block_name'].'>';  
   ...
   $sql_checkbox="SELECT count(id) as count from bio_cstask where despatch_id='$desid' AND orderno=$orderno AND taskdescription='AMC Proposal' 
AND cstype=2";
...
//      $('#dinhide').show(); 
  ...
  //alert(str);
xmlhttp.open("GET","bio_pm_statuschange.php?taskno="+str+"&orderno="+str1+"&desid="+str5+"&no="+str3+ "&date=" + str4 + "&status=" + str6,true);
xmlhttp.send();
//alert(str);
}
function validate()
...
{ 
 ...
 if(f==0){f=common_error('team','Please select team');  if(f==1) { return f; }}
