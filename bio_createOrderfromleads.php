...
///*     
     ...
     $_SESSION['enqid']=$_POST['enquiryid'];
     $propid=$_POST[propid];
     ...
     if($stage==1)
     {
         $sql_plant="SELECT bio_proposaldetails.propid,
                            bio_proposaldetails.stockid AS stockid,
                            bio_proposaldetails.qty AS quantity,
                            bio_proposaldetails.price,
                            stockmaster.longdescription AS description,
                            stockmaster.units
                       FROM bio_proposaldetails,stockmaster 
                      WHERE stockmaster.stockid=bio_proposaldetails.stockid
                        AND propid=$propid";
        //$result_plant=DB_query($sql_plant,$db);
                       
     }elseif($stage==2)
     {
        $sql_plant="SELECT bio_conceptproposal.cp_id,
                           bio_conceptproposaldetails.stockid AS stockid,
                           bio_conceptproposaldetails.qty AS quantity,
                           bio_conceptproposaldetails.price,
                           bio_conceptproposal.created_on, 
                           stockmaster.longdescription AS description,
                           stockmaster.units 
                      FROM bio_conceptproposal,bio_conceptproposaldetails,stockmaster
                     WHERE bio_conceptproposaldetails.cp_id=bio_conceptproposal.cp_id 
                       AND stockmaster.stockid=bio_conceptproposaldetails.stockid 
                       AND bio_conceptproposal.cp_id=$propid";  

        //$result_plant=DB_query($sql_plant,$db);                    
     }elseif($stage==3)
     {
        $sql_plant="SELECT bio_dpr.dpr_id,
                           bio_dprsubassemblies.subassembly AS stockid,
                           bio_dprsubassemblies.quantity,
                           bio_dprsubassemblies.price,     
                           stockmaster.longdescription AS description,
                           stockmaster.units
                      FROM bio_dpr,bio_dprsubassemblies,stockmaster 
                     WHERE bio_dprsubassemblies.dpr_id=bio_dpr.dpr_id
                       AND stockmaster.stockid=bio_dprsubassemblies.subassembly
                       AND bio_dpr.dpr_id=$propid";

         //$result_plant=DB_query($sql_plant,$db);               
     }elseif($stage==4)
     {
        $sql_plant="SELECT bio_lsgplant.propid,
                            bio_lsgplantdetails.stockid AS stockid,
                            bio_lsgplantdetails.qty AS quantity,
                            bio_lsgplantdetails.price,
                           bio_lsgplant.propdate, 
                           stockmaster.longdescription AS description,
                           stockmaster.units 
                     ...
                     WHERE bio_lsgplantdetails.propid=bio_lsgplant.propid 
                       ...
                       AND bio_lsgplant.propid=$propid";  
...
//		$sql_plant = $sql_plant . " LIMIT " . $_SESSION['DefaultDisplayRecordsMax'] . " OFFSET " . number_format($_SESSION['DefaultDisplayRecordsMax']*$Offset);

//		$ErrMsg = _('There is a problem selecting the part records to display because');
//		$DbgMsg = _('The SQL used to get the part selection was');
//		$SearchResult = DB_query($SQL,$db,$ErrMsg, $DbgMsg);

        $SearchResult = DB_query($sql_plant,$db);
...
//Get The exchange rate used for GPPercent calculations on adding or amending items
	...
	if (isset($_POST['DeliveryDetails'])){
		echo '<meta http-equiv="Refresh" content="0; url=' . $rootpath . '/bio_DeliveryDetails.php?' . SID .'identifier='.$identifier . '">';
		prnMsg(_('You should automatically be forwarded to the entry of the delivery details page') . '. ' . _('if this does not happen') . ' (' . _('if the browser does not support META Refresh') . ') ' .
		   '<a href="' . $rootpath . '/bio_DeliveryDetails.php?' . SID .'identifier='.$identifier . '">' . _('click here') . '</a> ' . _('to continue'), 'info');
	   	exit;
	}
...
/*Now figure out if the item is a kit set - the field MBFlag='K'*/
	...
	foreach ($_SESSION['Items'.$identifier]->LineItems as $OrderLine) {
        ...
        $OrderLine->Price=$NewPrice_array[$OrderLine->LineNumber];
		...
		foreach ($_SESSION['Items'.$identifier]->LineItems as $OrderLine) {
			...
			if (in_array(2,$_SESSION['AllowedPageSecurityTokens'])){
				/*OK to display with discount if it is an internal user with appropriate permissions */
				    echo '<td><input class="number" type=hidden name="Price_' . $OrderLine->LineNumber . '" size=16 maxlength=16 value=' . $OrderLine->Price . '>'. $OrderLine->Price .'</td>';
					echo'<td><input class="number" type=text name="Discount_' . $OrderLine->LineNumber . '" size=5 maxlength=4 value=' . ($OrderLine->DiscountPercent * 100) . '></td>
...
//		echo '<br /><div class="centre"><input type=submit name="Recalculate" Value="' . _('Re-Calculate') . '">';
				echo'<br /><div class="centre"><input type=submit name="DeliveryDetails" value="' . _('Enter Delivery Details and Confirm Order') . '"></div><hr>';
	} # end of if lines
...
//=====================================	
  ...
  $sql_lead="SELECT bio_leads.leadid,
                       bio_leads.enqtypeid,
                       bio_enquirytypes.enquirytype,
                       bio_leads.leadstatus,
                       bio_status.biostatus        
                  FROM bio_cust,bio_leads,bio_enquirytypes,bio_status 
                 WHERE bio_enquirytypes.enqtypeid=bio_leads.enqtypeid     
                   AND bio_cust.cust_id=bio_leads.cust_id
                   AND bio_status.statusid=bio_leads.leadstatus 
                   AND bio_cust.debtorno='". $_SESSION['Items'.$identifier]->DebtorNo ."'";
               
     $result_lead=DB_query($sql_lead,$db);
     $row_lead=DB_fetch_array($result_lead);
     $leadid=$row_lead['leadid'];
     $enqid=$row_lead['enqtypeid'];
     $status=$row_lead['leadstatus']; 
     
     if($row_lead['leadstatus']==25)
     { 
                       $stage=1;                                                  //domestic
                   $sql_plant="SELECT bio_proposal.propid AS propid,
                                          bio_proposal.propdate AS propdate,
                                          bio_proposaldetails.stockid AS stockid,
                                          stockmaster.longdescription AS longdescription,
                                          COUNT(bio_proposal.propid) AS count                                
                                     FROM bio_proposal,bio_proposaldetails,stockmaster
                                    WHERE bio_proposaldetails.propid=bio_proposal.propid
                                      AND stockmaster.stockid=bio_proposaldetails.stockid
                                      AND bio_proposal.leadid=$leadid
                                 GROUP BY bio_proposal.propid HAVING count>0";
                        $result_plant=DB_query($sql_plant,$db);
                        $result_count=DB_num_rows($result_plant);
                        
 
             if($result_count<=0)
             {                 
                        $stage=3;                                                 //dpr
                        $sql_plant="SELECT bio_dpr.dpr_id AS propid,
                                           bio_dpr.plant AS stockid,
                                           bio_dpr.createdon AS propdate,
                                           stockmaster.longdescription AS longdescription 
                                      FROM bio_dpr,stockmaster 
                                     WHERE stockmaster.stockid=bio_dpr.plant
                                       AND bio_dpr.leadid=$leadid";
                        $result_plant=DB_query($sql_plant,$db);             
                        $result_count=DB_num_rows($result_plant);  
            ...
            { 
                        $stage=2;                                               //cp
                        $sql_plant="SELECT bio_conceptproposal.cp_id AS propid,                                                                                                        
                                           bio_conceptproposaldetails.stockid AS stockid,  
                                           bio_conceptproposal.created_on AS propdate,
                                           stockmaster.longdescription AS longdescription,                                                                                    
                                           COUNT(bio_conceptproposal.cp_id) AS count
                                      FROM bio_conceptproposal,bio_conceptproposaldetails,stockmaster
                                     WHERE bio_conceptproposaldetails.cp_id=bio_conceptproposal.cp_id
                                       AND stockmaster.stockid=bio_conceptproposaldetails.stockid 
                                       AND bio_conceptproposal.lead_id=$leadid
                                       AND bio_conceptproposal.status=4 
                                  GROUP BY bio_conceptproposal.cp_id HAVING count>0";                     
                        $result_plant=DB_query($sql_plant,$db);
                        $result_count=DB_num_rows($result_plant); 
            }
            if($result_count<=0)
            {                              
                $result_parentlead=DB_query("SELECT parent_leadid FROM bio_leads WHERE leadid=$leadid",$db);
                $myrow_parentlead=DB_fetch_array($result_parentlead);
                        $stage=4;                                                //lsg
                        $sql_plant="SELECT bio_lsgplant.propid AS propid,                                                                                                        
                                           bio_lsgplantdetails.stockid AS stockid,  
                                     ...
                                     WHERE  bio_lsgplantdetails.propid=bio_lsgplant.propid
                                       AND stockmaster.stockid= bio_lsgplantdetails.stockid 
                                       AND bio_lsgplant.leadid='".$myrow_parentlead['parent_leadid']."'
                                       AND bio_lsgplant.status=0 
                                  GROUP BY bio_lsgplant.propid HAVING count>0";                     
                        $result_plant=DB_query($sql_plant,$db);
                        $result_count=DB_num_rows($result_plant); 
            }
     }
     if($result_count==0 && $enqid==1) 
     {
      echo '<div class="page_help_text">' . _('No proposals for this Customer. Select the link to create a Proposal') . '</div>';      
      echo "<a  style='cursor:pointer;'  id='$leadid' onclick='passid(this.id,$enqid)'>" . _('Create Standard Quatation') . "</a>";   
     }     
     elseif($result_count==0 && $enqid==2) 
     {                      
      echo '<div class="page_help_text">' . _('No proposals for this Customer. Select the link to create a Proposal') . '</div>';       
      echo "<a  style='cursor:pointer;'  id='$leadid' onclick='passid(this.id,$enqid)'>" . _('Create Concept Proposal') . "</a>";   
     }   
     else
     {
      echo"<table style='width:100%'> "; 
      echo"<tr><th>Slno</th><th>Proposal No</th><th>Date</th><th>Sock code</th><th>Plant Description</th></tr>";     
     }
     
       $no=1;        
       while($row_plant=DB_fetch_array($result_plant)) 
       {
           
           echo"<tr>"; 
           echo"<td>$no</td>";
           echo'<td><input type="submit" name="propid" value="'.$row_plant['propid'].'"></td>';
           echo"<td>".$row_plant['propdate']."</td>";
           echo"<td>".$row_plant['stockid']."</td><td>".$row_plant['longdescription']."</td>";
           echo'<input type="hidden" name="stage" value="'.$stage.'">';  
           echo"</tr>";
           $no++;            
       } 
       echo'<input type="hidden" name=leadid value='.$leadid.'>';
       echo'<input type="hidden" name=enquiryid value='.$enqid.'>';  
 //    echo  $_SESSION['propid']=$propid;      
/*		
...
//			echo '<td colspan=><input type="hidden" name="nextlist" value='.number_format($Offset+1).'><input tabindex='.number_format($j+10).' type="submit" name="Next" value="'._('Next').'"></td></tr>';
			...
			$k=0; //row colour counter
                                        $stocks=array();
                                        $i=0;
            echo '<div class="page_help_text">' . _('Selected Proposal id - '.$propid.' ') . '</div>';
            echo '<br>';                              
			...
			while ($myrow=DB_fetch_array($SearchResult)) {
				...
				printf('<td>%s</td>
						...
						<td class="number">%s</td>
						<td class="number">%s</td>
						<td><font size=2><input class="number"  tabindex='.number_format($j+7).' type="hidden" size=6 name="itm'.$myrow['stockid'].'" value='.$myrow['quantity'].'>'.$myrow['quantity'].'</td>
						<td><font size=1><input class="number"  tabindex='.number_format($j+8).' type="hidden" size=6 name="Price'.$myrow['stockid'].'" value='.$myrow['price'].'></td>
                        </tr>',
						$myrow['stockid'],
						$myrow['description'],
						$myrow['units'],
...
function passid(str,str2){
    //alert(str);
    if(str2==1)
    {
      window.location="bio_proposal.php?lead=" + str; 
    }
    if(str2==2)
    {
        window.location="bio_conceptproposal.php?lead=" + str; 
    }
