...
while($myrow=DB_fetch_array($result))
          
 {
    ...
    $letterdate=$myrow['letterdate'];
    $remarks=$myrow['remarks']; 
//    $pid= $myrow['propid']; 
    $pdate=$myrow['propdate']; 
...
while($myrow_items=DB_fetch_array($result_items))
{
   ...
   $item_all_array[]="'".$myrow_subsidy['stockid']."'";
...
//   print_r($netamount_words);
 ...
 //--------------------------------------------------------------------------------------------------------------------//   
    ...
    $pdf->addTextWrap($Left_Margin+20,$YPos-=$line_height+10,100,$FontSize,$letterno, 'left'); 
     ...
     $pdf->addTextWrap($Left_Margin+20,$YPos-=$line_height+5,200,$FontSize+1,_('Sub:   <Project name>'), 'left');
     
      
     $pdf->addTextWrap($Left_Margin+20,$YPos-=$line_height,300,$FontSize+1,_('Ref.:   '), 'left');
     ...
     $pdf->addTextWrap($Left_Margin+20,$YPos-=$line_height-5,900,$FontSize+1,_('plant to process solid waste and generate gas prompt us to offer you a cost-effective proposal for <Project name>'), 'left'); 
...
//         $pdf->addTextWrap($Left_Margin+350,$YPos-=$line_height-15,700,$FontSize+1,_('xxx CUM'),'left');
        /* $pdf->addTextWrap($Left_Margin+20,$YPos-=$line_height+10,900,$FontSize+1,_('  The cost estimation of the project is given below (For single page quotation.) '),'left');  
         ...
         $pdf->addTextWrap($Left_Margin+20,$YPos-=$line_height+5,140,$FontSize+1,$designation,'',0);
...
//         $pdf->addTextWrap($Left_Margin+20,$YPos-=$line_height-2,900,$FontSize+1,_('<Designation of authorized signatory>'), 'left');    
    ...
    $pdf->addTextWrap($Left_Margin+35,$YPos-=$line_height+5,140,$FontSize+1,$designation,'',0);
//    $pdf->addTextWrap($Left_Margin+35,$YPos-=$line_height+15,800,$FontSize+1,_('<Name and designation of authorized signatory> '),'left');
  ...
  $pdf->line($Left_Margin+195,$YPos=$YPos-2,$Left_Margin+285,$YPos); 
         ...
         $pdf->addTextWrap($Left_Margin+20,$YPos-=$line_height-5,100,$FontSize+1,$area, 'left'); 
         $pdf->addTextWrap($Left_Margin+20,$YPos-=$line_height-5,100,$FontSize+1,$housename, 'left'); 
           $pdf->line($Left_Margin+10,$YPos=$YPos-10,$Left_Margin+470,$YPos); 
