...
$sql3=$_SESSION['qry'];
    ...
    $objPHPExcel->getActiveSheet()->SetCellValue('C1', 'Address');
     $objPHPExcel->getActiveSheet()->SetCellValue('D1', 'ST');
    $objPHPExcel->getActiveSheet()->SetCellValue('E1', 'District');
        $objPHPExcel->getActiveSheet()->SetCellValue('F1', 'LSG');
    $objPHPExcel->getActiveSheet()->SetCellValue('G1', 'Contact No');
    $objPHPExcel->getActiveSheet()->SetCellValue('H1', 'Installed Date');
    $objPHPExcel->getActiveSheet()->SetCellValue('I1', 'Model');
    $objPHPExcel->getActiveSheet()->SetCellValue('J1', 'Capacity');
    $objPHPExcel->getActiveSheet()->SetCellValue('K1', 'Base');
...
while($row = DB_fetch_array($result)){
    ...
    $objPHPExcel->getActiveSheet()->SetCellValue('C'.$rowCount, str_replace('</br>',',',$row['address']));
    $objPHPExcel->getActiveSheet()->SetCellValue('D'.$rowCount, $row['code']);
    $objPHPExcel->getActiveSheet()->SetCellValue('E'.$rowCount, $row['district']);
     $objPHPExcel->getActiveSheet()->SetCellValue('F'.$rowCount, $LSG_name);
    $objPHPExcel->getActiveSheet()->SetCellValue('G'.$rowCount, str_replace('</br>',',',$row['mobile']));
    $objPHPExcel->getActiveSheet()->SetCellValue('H'.$rowCount, $row['insdate']);
    $objPHPExcel->getActiveSheet()->SetCellValue('I'.$rowCount, $row['model']);
    $objPHPExcel->getActiveSheet()->SetCellValue('J'.$rowCount, $row['capacity']);
     $objPHPExcel->getActiveSheet()->SetCellValue('K'.$rowCount, $bas);
    $objPHPExcel->getActiveSheet()->SetCellValue('L'.$rowCount, $row['plantstatus']);
