...
//      $schemeid=$myrow['schemeid'];              
     ...
     $myrow1=DB_fetch_array($result1); 
      ...
      $lsgtype=$myrow1['LSG_type'];
      $lsgid=$myrow1['LSG_name'];
      $panchayath=$myrow1['block_name'];
...
//       echo '<center><h2>Leads</h2></center>';
 ...
 //       echo "<tr><td>House No:</td><td><input type='text' name='Houseno' id='Houseno' onkeyup='' style='width:99%'></td></tr>";    
    ...
    $result=DB_query($sql,$db);
          ...
          {    //echo $myrow[0]; 
          $nid=$myrow[0]; 
          $nationality1=$myrow[1];    
  
                 }
         echo"<tr></tr>";           
        echo "<tr><td>Country:</td><td><input type='hidden' name='country' value='$nid' id='Country' onkeyup='' style='width:99%'>:$nationality1</td></tr>"; 
    ...
    $stateid1=$myrow[2];   
        ...
        echo"<tr></tr>";   
        echo "<tr><td>State:</td><td><input type='hidden' name='State' id='State' value='$sid' onkeyup='' style='width:99%'>:$state1</td></tr>"; 
    ...
    $result=DB_query($sql,$db);
        ...
        echo"<tr></tr>";             
        echo "<tr><td>District:</td><td><input type='hidden' name='District' id='District' value='$did' onkeyup='' style='width:99%'>:$district1</td>";       
    echo '</tr>';
...
//     $lsg_name="Corporation:".$lsgname;
   ...
   if($lsgtype==2) {      
   $sql="SELECT * FROM bio_municipality WHERE country=$nationality AND state=$state AND district=$district AND id=$lsgid";
        $result=DB_query($sql,$db);  
    ...
    if($lsgtype==3) { 
        ...
        $sql="SELECT * FROM bio_block WHERE country=$nationality AND state=$state AND district=$district AND id=$lsgid";
        $result=DB_query($sql,$db);
        $myrow1=DB_fetch_array($result);
        $lsgname=$myrow1['block'];
        echo '<tr><td>Block</td><td>:'.$lsgname.'</td></tr>';
        echo"<tr></tr>";
        $sql="SELECT * FROM bio_panchayat WHERE country=$nationality AND state=$state AND district=$district AND id=$panchayath";
        $result=DB_query($sql,$db);
        $myrow1=DB_fetch_array($result);
...
//---------------------Lead History-------------------------------//
  ...
  while($row_task=DB_fetch_array($result_task))  
  {
   ...
   echo "<td>$task</td><td>$team</td><td>$assigneddate</td><td>$duedate</td><td>$taskcompleteddate</td></tr>";
  }  
