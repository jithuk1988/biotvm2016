...
if($_GET['taluk']){  
    ...
    $result=DB_query($sql,$db);
  echo '<select name="Taluk" id="Taluk" style="width:190px" tabindex=12 onchange="showvillage(this.value)">';
   $f=0;
  while($myrow1=DB_fetch_array($result))
  {
...
if($_GET['block']){ 
        ...
        $district=$row['district'];
          ...
          if($cid==1 && $sid==14)  
          {
              ...
              if($district==8){$distname='Kozhikode';} 
                    ...
                    echo '<td><select name="lsgName" id="lsgName" style="width:200px"  onblur="showamt(1);">';
    ...
    elseif($_GET['block']==2)        //Municipality
    {
        ...
        echo '<td><select name="lsgName" id="lsgName" style="width:200px"  onchange="showamt(2);">';
         $f=0;
         while($myrow1=DB_fetch_array($result))
         {
    ...
    elseif($_GET['block']==3)          //block Panchayath
    {
         ...
         echo '<td><select name="lsgName" id="lsgName" style="width:200px"  onchange="showgramapanchayath(this.value)">';
         $f=0;
         while($myrow1=DB_fetch_array($result))
         {
         if ($myrow1['id']==$_POST['lsgName'])
      ...
      echo '<tr id="showgramapanchayath"><td width=100px>' . _('Panchayat Name') . ':</td>';         
         
         $sql="SELECT * FROM bio_panchayat WHERE country=$country1 AND state=$state AND district=$district";
         $result=DB_query($sql,$db);
         
         echo '<td><select name="gramaPanchayath" id="gramaPanchayath" style="width:200px" onchange="showamt(3);" >';
         $f=0;
         while($myrow1=DB_fetch_array($result))
         {
...
if($_GET['grama']){                  //grama panchayath 
      ...
      echo '<tr><td width=100px>' . _('Panchayat Name') . ':</td>';         
