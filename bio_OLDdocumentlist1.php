...
echo"<tr><th>No:of orders</th><th>Document Name</th><th>Recieved Documents</th><th>Pending Documents</th></tr>";     
 ...
 $sql_old="SELECT DISTINCT bio_oldorderdoclist.orderno AS oldorders,
...
//$sql_old .=" order by debtorsmaster.clientsince asc ";
 ...
 $sql_so="SELECT DISTINCT  salesorders.orderno AS neworders,
                  debtorsmaster.debtorno,
                  debtorsmaster.name,
                  bio_district.district,
                  debtorsmaster.clientsince,
...
$sql.=" AND docno IN (SELECT doc_no
    ...
    WHERE STATUS =0";
if($neworders_arr!=''){
$sql.=" AND bio_oldorderdoclist.orderno IN ($neworders_arr)";  
}    
    $sql.=" AND docno IN (SELECT doc_no
    FROM bio_document_master
    WHERE enqtypeid ='".$_POST['enq']."')
...
// echo $sql;  
  ...
  {
      echo "<td></td>
             <td>".$myrow['docno']."</td><td>".$myrow[count_not_recd]."</td><td>".$myrow[count_recd]."</td>
             </tr>";
  }  
