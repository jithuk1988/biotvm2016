...
xmlhttp.open("GET","bio_taskcustdetails.php?p=" + str1 + "&q=" +str2,true);
xmlhttp.send(); 
}
...
if (isset($_POST['submit'])) {
    ...
    if($status_count>0){
        $sql9="SELECT task_master_id,actual_task_day
         ...
         AND schedule_id>=(SELECT schedule_id FROM bio_schedule WHERE task_master_id=$task_ID AND schedule_master_id=$scheduleid
  ...
  //  echo $officeid;
    ...
    if (isset($_POST['Taskstatus']))  {        
        if ($_POST['Taskstatus']!='')    {  
            ...
            if ($_POST['Taskstatus']==2) {
                ...
                $sql.=" AND bio_leadtask.leadid IN ($schedule_arr)";    
    ...
    if (isset($_POST['enq']))    {
        if (($_POST['enq']!='')&&($_POST['enq']!='0'))
            $sql .=" AND bio_enquirytypes.enqtypeid='".$_POST['enq']."'";    
    }
...
$sql .=" GROUP BY leadid ORDER BY leadid DESC";     // echo $sql;
$result=DB_query($sql,$db);
...
echo '<option selected='.$_POST['Taskstatus'].'>'.$taskstatusdesc.'</option>';                                                           
echo '</select></td>';
...
echo '<td class="byoffice"><select name="off1" id="off1" style="width:100px">';
echo '<option value=0></option>'; 
...
echo '</select></td>';
echo '<td class="bycusttype"><select name="enq" id="enq" style="width:100px">';
echo '<option value=0></option>';   
...
while($myrow=DB_fetch_array($result))
{
    ...
    if($myrow_1[0]>0){
        echo"<td><a  style='cursor:pointer;'  id='$leadid' onclick='passid(this.id,$taskid)'>" . _('Reassign') . "</a></td>";
    }
    else{
       echo"<td><a  style='cursor:pointer;'  id='$leadid' onclick='passid(this.id,$taskid)'>" . _('Assign') . "</a></td>"; 
    }
