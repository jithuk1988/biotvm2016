...
echo"<legend><h3>Dispatch Cleared Items</h3></legend>";
    ...
    echo"<td><input type='submit' name='submit' id='submit' value=search></td></tr></table>";
        ...
        echo '<tr>  <th>' . _('Slno') . '</th>
              ...
              $sql_inst="SELECT salesorderdetails.orderno,bio_despatchClearence.id,bio_despatchClearence.despatch_id,
              custbranch.brname,
              ...
              ";                            
              if($_POST['submit'])                       //   AND salesorders.inst_completed=0 
              {
          ...
          }else 
          {
            ...
            $k=1;     
                       ...
                       <td>".convertSQLDate($row_so['sched_instlln_date'])."</td> 
                                     ...
                                     <td>".$row_so['district']."</td> 
                                     <td width='50px'><a style='cursor:pointer;' id=$plant onclick='viewdocs(this.id,$orderno,$id);'>" . _('Select ') . "</a></td>"; 
                 if ($row_checkrec['count']!=0)
                 {
                     echo"<td width='50px'><a style='cursor:pointer;' id=$plant onclick='delet_dn($orderno,$id);'>" . _('Delete') . "</a></td>";
                 }                     
                                echo"</tr>";
...
{           //alert(str2);     // 
 ...
 //alert(str);
location.href="bio_deliverynote.php?orderno=" + str1 + "&plant=" + str + "&id=" +str2;   

}

function delet_dn(str1,str2)
{          // alert(str2);     // 
    //myRef = window.open("bio_lsgstatuschange.php?leadid=" + str1 + "&id=" + str2,"droplead","toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=no,width=700,height=400");
 //alert(str);
...
if (r==true)
  {
location.href="?orderno=" + str1 + "&id=" +str2;   
