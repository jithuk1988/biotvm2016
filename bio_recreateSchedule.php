...
{
    $sqltim="select pref_time from bio_leads where leadid=".$_GET['leadid'];
...
$sql_mail="SELECT bio_cust.custmail FROM bio_cust,bio_leads
  ...
  if(isset($_POST['submit']))
  {
      ...
      if($status==1)
      {
          $sql1="UPDATE bio_leadtask SET taskcompletedstatus=1,taskcompleteddate='$crdt' WHERE leadid=$leadid AND taskid=$taskid and viewstatus=1 and taskcompletedstatus=0 ";      DB_query($sql1,$db); 
      }
      elseif($status==2)
      { 
          $sql2="UPDATE bio_leadtask SET taskcompletedstatus=2 WHERE leadid=$leadid AND taskcompletedstatus!=1 ";    
          DB_query($sql2,$db);     
          $sql1="UPDATE bio_leadtask SET taskcompletedstatus=1,taskcompleteddate='$crdt' WHERE leadid=$leadid AND taskid=$taskid and viewstatus=1 ";     DB_query($sql1,$db);          
          $sql3="UPDATE bio_leads SET leadstatus=20 WHERE leadid=$leadid";         DB_query($sql3,$db);          
      }
      elseif($status==3)
      {     
          $sql2="UPDATE bio_leadtask SET taskcompletedstatus=2 WHERE leadid=$leadid AND taskid !=18 and taskcompletedstatus=0 ";    DB_query($sql2,$db); 
          $sql1="UPDATE bio_leadtask SET taskcompletedstatus=1,taskcompleteddate='$crdt' WHERE leadid=$leadid AND taskid=$taskid AND viewstatus=1 and taskcompletedstatus !=2 ";     DB_query($sql1,$db);
