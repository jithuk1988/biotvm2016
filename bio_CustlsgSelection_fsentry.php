...
if($_GET['state']){      
    ...
    $result=DB_query($sql,$db);
  echo '<select name="District" id="Districts" style="width:100%>';
   $f=0;
  while($myrow1=DB_fetch_array($result))
  {
...
echo '<select name="village" id="village" tabindex=12 style="width:100%">';
...
if($_GET['block']){ 
        ...
        $district=$row['district'];
          ...
          if($cid==1 && $sid==14)  
          {
              ...
              if($district==8){$distname='Kozhikode';} 
                    echo '<table width=100%><tr><td>' . _('Corporation Name') . ':</td>';
    ...
    {
        echo '<table width=100%><tr><td>' . _('Municipality Name') . ':</td>';    
    ...
    {
         echo '<table   width=100%><tr><td>' . _('Block Name') . ':</td>';    
         
         $sql="SELECT * FROM bio_block WHERE country=$country1 AND state=$state AND district=$district";
         $result=DB_query($sql,$db);
         
         echo '<td width=50%><select name="lsgName" id="lsgName" style="width:100%" onchange="showgramapanchayath(this.value)">';
         $f=0;
         while($myrow1=DB_fetch_array($result))
         {
         if ($myrow1['id']==$_POST['lsgName'])
