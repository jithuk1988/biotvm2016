    ...
    if($_POST['FSID']!=''){ 
        $InputError=1;
         ...
         $result_fs = DB_query($sql_fs,$db,_('The update/addition  failed because')); 
                 ...
                 $feacondate=FormatDateForSQL($_POST['ConductedDate']);  
...
$sql_fsentry="UPDATE bio_leadtask SET remarks =".$_POST['fsentry']."                                            
                   WHERE bio_leadtask.taskid=14                    
                     AND taskcompletedstatus=1";                     
         DB_query($sql_fsentry,$db); 
...
echo'<td><input type="text" style="width:120px" id="conducteddate" class=date alt='.$_SESSION['DefaultDateFormat'].' name="conducteddate" onChange="return isDate(this, this.value, '."'".$_SESSION['DefaultDateFormat']."'".')'.'"'.'></td>'; 
...
echo"<tr><th>Slno</th>
  ...
  $sql8="SELECT bio_cust.custname AS custname,               
          ...
          WHERE bio_cust.cust_id=bio_leads.cust_id 
            ...
            AND bio_fs_entrydetails.created_by IN ($user_array)
