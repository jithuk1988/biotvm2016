<?php
/* $Id: StockCategories.php 4551 2011-04-16 06:20:56Z daintree $*/
$PageSecurity = 40;
...
if (isset($_POST['submit'])) {
	...
	if ($SelectedCategory AND $InputError !=1) {
        ...
        $sql4="UPDATE bio_maincat_subcat        
               SET maincatid = " . $_POST['MainCategoryID'] . ",
               subcatid = '" . $_POST['SubCategoryID'] . "',
                subsubcatid='" . $_POST['CategoryID'] . "'
                WHERE subsubcatid= '".$SelectedCategory."';
               ";
	...
	/*Selected category is null cos no item selected on first time round so must be adding a	record must be submitting new entries in the new stock category form */
        ...
        $sql4="INSERT INTO bio_maincat_subcat(        
                maincatid,
                subcatid,subsubcatid
                )
                VALUES(
                " . $_POST['MainCategoryID'] . ",
                '" . $_POST['SubCategoryID'] . "',
                '" . $_POST['CategoryID'] . "'
                )
               ";
...
if (! isset($_GET['delete'])) {
	...
	if (isset($SelectedCategory)) {
		...
		if (!isset($_POST['UpdateTypes'])) {
            ...
            $sql5="SELECT bio_maincat_subcat.maincatid,bio_maincat_subcat.subcatid
                   FROM bio_maincat_subcat
                   WHERE bio_maincat_subcat.subsubcatid ='" . $SelectedCategory . "'  ";
            $result5=DB_query($sql5,$db);
            $myrow5=DB_fetch_array($result5);
            $_POST['MainCategoryID']=$myrow5['maincatid'];
            $_POST['SubCategoryID']=$myrow5['subcatid'];
		}
		echo '<input type=hidden name="SelectedCategory" value="' . $SelectedCategory . '">';
		echo '<input type=hidden name="CategoryID" value="' . $_POST['CategoryID'] . '">';
		echo '<table class=selection><tr><td>' . _('Sub Sub Category Code') . ':</td><td>' . $_POST['CategoryID'] . '</td></tr>';

	} else { //end of if $SelectedCategory only do the else when a new record is being entered
		if (!isset($_POST['CategoryID'])) {
			$_POST['CategoryID'] = '';
		}
		echo '<table class=selection><tr><td>' . _('Sub Sub Category Code') . ':</td>
	...
	echo '<tr><td>' . _('Sub Sub Category Description') . ':</td>
            <td><input type="Text" name="CategoryDescription" size=35 maxlength=50 value="' . $_POST['CategoryDescription'] . '"></td></tr>';

     echo '<tr><td>' . _('Main Category') . ':</td><td><select name="MainCategoryID" id="maincatid"  onchange="Filtercategory()">';

$sql = "SELECT maincatid, 
               maincatname 
        FROM bio_maincategorymaster
        ORDER BY maincatid ASC";
...
echo '</select></td></tr>';   
echo '<tr><td >' . _('Sub Category') . ':</td><td id="subcat"><select name="SubCategoryID" id="SubCategoryID"  style="width:200px" >';
...
or deletion of the records*/
    ...
    $result = DB_query("SELECT *,bio_maincategorymaster.maincatname,substockcategory.subcategorydescription
FROM stockcategory
left join bio_maincat_subcat on (bio_maincat_subcat.subsubcatid=stockcategory.categoryid)
left join substockcategory on (bio_maincat_subcat.subcatid=substockcategory.subcategoryid)
left join bio_maincategorymaster on (bio_maincategorymaster.maincatid=bio_maincat_subcat.maincatid)
 ",$db);
    ...
    echo '<tr>
            ...
            <th>' . _('WIP GL') . '</th></tr>';//<th>' . _('Main cat') . '</th>
           // <th>' . _('Sub cat') . '</th>
