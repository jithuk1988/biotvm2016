...
if($_GET['country']){
  ...
  echo '<select name="State" id="state" tabindex=5 style="width:220px" onchange="showdistrict(this.value)">';
   $f=0;
...
if($_GET['state']){      $state=$_GET['state']; $country1=$_GET['country1'];
         echo"<td>District*</td><td>";
            ...
            ORDER BY did";
    $result=DB_query($sql,$db);
  echo '<select name="District" id="Districts" style="width:220px" tabindex=6 onchange="addnew(this.value)">';
   $f=0;
  while($myrow1=DB_fetch_array($result))
  {
