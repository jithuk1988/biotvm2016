...
if(isset($_POST['close_incentive']))
{
     ...
     for($i=1;$i<=$count;$i++)
     {
         $sql_insert="INSERT INTO bio_incentive_leadsource(type,enqtype,sourceid,month,year,target,achieved,eligible,incentive)VALUES(6,'".$_POST['enq']."',
   ...
   $sql_closingdate="INSERT INTO bio_closingdate (type,enqtype,closedate) VALUES (7,'".$enq."','".$_POST['close_date']."')";
...
echo '<input type=hidden name=enq value="'.$enq.'">';
    ...
    $closedate=DB_query("SELECT MAX(closedate) as lastclose,month(MAX(closedate)) as lastmonth,year(MAX(closedate)) as lastyear FROM  bio_closingdate WHERE type=7 AND  enqtype='".$enq."' ",$db);
...
echo "<fieldset style='float:center;width:50%;overflow:scroll;'>";
       ...
       if($enq==1){ $d="<legend><h3>Customer/DOMESTIC INCENTIVES</h3>";} 
      elseif($enq==2){ $d="<legend><h3>Customer/INSTITUTION INCENTIVES</h3>";}
       elseif($enq==3){ $d="<legend><h3>Customer/LSGD INCENTIVES</h3>";} 
   echo $d;
       if($_POST['closing']!="")
    {
 ...
 $sql="SELECT debtorsmaster.name,debtorsmaster.debtorno,SUM(salesorderdetails.quantity * stockitemproperties.value) 
FROM debtorsmaster
 INNER JOIN bio_leadsource_dealstaff ON bio_leadsource_dealstaff.sourceid=debtorsmaster.debtorno
...
WHERE bio_leadsource_dealstaff.sourcetype=4
...
$result=DB_query($sql,$db);
     ...
     <td style="border-bottom:1px solid #0000FF" ><b>Customer Name</td>   
                <td style="border-bottom:1px solid #0000FF"><b>Target Month/Year</td>
               
                <td style="border-bottom:1px solid #000000"><b>No. of CUM</td>
       
                <td style="border-bottom:1px solid #000000"><b>Eligible Unit</td>
                <td style="border-bottom:1px solid #000000"><b>Incentive</td>
                </tr>';  
  $i=0;   
...
AND bio_achievement_outpolicy.leadsource=6 
...
WHERE bio_achievement_outpolicy.enq_type='".$enq."' AND bio_achievement_outpolicy.leadsource=6
AND bio_achievement_outpolicy.from< '".$row[2]."'
...
echo '</form>' ;
     ...
     <td style="border-bottom:1px solid #000000">Customer Name</td>                   
                ...
                $sql_iinnce="SELECT bio_incentive_leadsource.sourceid,bio_incentive_leadsource.month,bio_incentive_leadsource.year, bio_incentive_leadsource.achieved,bio_incentive_leadsource.eligible,bio_incentive_leadsource.incentive,debtorsmaster.name
                from  bio_incentive_leadsource,debtorsmaster
                WHERE bio_incentive_leadsource.enqtype='".$enq."'
                AND bio_incentive_leadsource.type=6
                AND debtorsmaster.debtorno=bio_incentive_leadsource.sourceid
