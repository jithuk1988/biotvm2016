...
echo '<center><font style="color: #333;
    ...
    {
        $del_file="DELETE FROM bio_fileno WHERE id='".$_GET['dellast']."'";
        $result_del=DB_query($del_file,$db);
        unset($_GET['dellast']);
    }
    
     if(isset($_POST['arch']))   
       {
/*           $sql_checkduplicte="SELECT count(id) as count from bio_fileno WHERE debtorno='".$_POST['debtor']."' AND orderno='".$_POST['order']."' 
           AND fileno='".$_POST['fileno']."' ";
           $result_checkduplicate=DB_query($sql_checkduplicte,$db);
           $dup=DB_fetch_array($result_checkduplicate);
           echo $dup['count'];                                       
           if($dup['count']==0) {*/
                  $sql3="INSERT INTO bio_fileno (debtorno,orderno,fileno) VALUES ('".$_POST['debtorno']."' , '".$_POST['orderno']."' , '".$_POST['fileno']."')";                    
                  DB_query($sql3,$db);
 ...
 if($orderno!='')
 {
     ...
     AND bio_oldorders.orderno='".$orderno."'";
...
echo '<br /><table class="selection">
            ...
            <br />';
            /*echo"<a href=>Change installation Date</a>";*/ 
     
            //Link to change installatio date
         if($insdate1!="Not entered" AND $row7['fileno']=="")
          {
              echo"<a style='cursor:pointer;'  onclick='change_inast_date();'>" . _('Change installation Date') . "</a>";  
         }  
...
//------------- Outward documents-----------------------
    ...
    echo "</fieldset>";
//------------------------------------------------------   
 echo'<input type=hidden name=instdate id=instdate value='.$row['installationdate'].'>';                                                
 echo'<input type=hidden name=enqtype id=enqtype value='.$_GET['enq'].'>';
...
echo'<input type=hidden name=fileno id=fileno value='.$fileno2.'>';
//echo'<input type=hidden name=debtorno id=debtorno value='.$debtorno.'>';       
echo'<input type=hidden name=maxid id=maxid value='.$row2['maxid'].'>';
...
echo'<tr><td colspan=2><p><div class="centre">
         ...
         <input type="button" value="Back" onClick=window.location="bio_OLDdocumentmanagement.php" >';
                  if($row7['fileno']==NULL)
      {
         echo '<input type="submit"  name="arch" id="arch" onclick="if(archieve()==1) return false;" VALUE="' . _('Archieve') . '">'; 
      } 
      
      
        echo"<fieldset style='width:90%;'>";
    echo"<legend>Last Added file</legend>";
     echo"<table style='border:1px solid #F0F0F0;width:90%'>";
      
      $sql_getfile="SELECT bio_fileno.id, 
      bio_fileno.debtorno,
      bio_fileno.fileno,
      debtorsmaster.name 
      FROM bio_fileno,debtorsmaster 
      WHERE debtorsmaster.debtorno=bio_fileno.debtorno 
      GROUP BY bio_fileno.id 
      ORDER BY id DESC LIMIT 1";
      $result_getfile=DB_query($sql_getfile,$db);
      $row_file=DB_fetch_array($result_getfile);
          echo"<tr ><th>Name</th><th>File Name</th></tr>";
          echo'<tr class="EvenTableRows"><td>'.$row_file['name'].'</td>'; 
          echo'<td>'.$row_file['fileno'].'</td>';
          echo"<td><a style='cursor:pointer;' id='".$row_file['id']."' onclick='del_lastfile(this.id);'>" . _('Delete ') . "</a></td></tr>"; 
    echo"</table>";
echo"</div>";
echo"</fieldset>";
...
function change_inast_date()
{
    var dbtor=document.getElementById("debtorno").value;
    var r=confirm("Are you sure to want to update the installation date");
    if(r==true){
    controlWindow=window.open("bio_installationdate.php?debtorno="+dbtor,"datechange","toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=no,resizable=no,width=700,height=400");
    }
} 

function del_lastfile(str)
{   var order=document.getElementById("orderno").value;
var enq=document.getElementById("enqtype").value; 
   // alert(enq);
    location.href="?orderno="+order+"&enq="+enq+"&dellast="+str;
}

 function archieve()
{
  var debtor=document.getElementById("debtorno").value;  
  var order=document.getElementById("orderno").value;     
 ...
 var f=0;
  if(fileno=="")
  {     
   alert("Cannot create fileno because of some warnings");
   f=1;
    return f;
  }
  /*else{
...
xmlhttp.open("GET","bio_docarchieve1.php?order=" + order + "&debtor=" + debtor + "&maxid="+ maxid + "&fileno=" +fileno,true); 
xmlhttp.send();  
    //alert(fileno);   
