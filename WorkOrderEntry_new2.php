	...
	If (strlen($_POST['Keywords'])>0) {
		...
		if ($_POST['StockCat']=='All'){
			$SQL = "SELECT stockmaster.stockid,
					...
					stockmaster.units
					FROM stockmaster
				INNER JOIN bio_wo_stocklocation
        ON (`bio_wo_stocklocation`.`stockid` = `stockmaster`.`stockid` AND bio_wo_stocklocation.loccode=".$_POST['StockLocation'].")
			...
			$SQL = "SELECT stockmaster.stockid,
                    ...
                    AND stockcategory.categoryid IN (SELECT `subcatid` FROM `bio_maincat_subcat` WHERE `maincatid` IN(1))
...
if (isset($NewItem) AND isset($_POST['WO'])){
      ...
      // FOR ADDING PLANT COMPONENTS Edited: Nizam
   /*   if($_GET['Qty'])
      {
          $EOQ=$_GET['Qty'];
...
function addWO_forsubassembly($db,$wo,$item,$qty,$location)
{
        $sql = "INSERT INTO woitems (wo,
                                 stockid,
                                 qtyreqd
        ...
        //Recursively insert real component requirements - see includes/SQL_CommonFunctions.in for function WoRealRequirements
        WoRealRequirements($db, $wo, $location, $item);
...
$sql_numrow="SELECT COUNT(*),qtyreqd FROM woitems WHERE wo='".$_POST['WO']."'";
$result_numrow=DB_query($sql_numrow,$db);
$row_numrow=DB_fetch_array($result_numrow);

if($row_numrow[0]==1) // IF ANY PLANT IS SELECTED SHOWING IT'S COMPONENTS
{
    $Lineno=$row_numrow[0]+1;
  $sql_getrwcount="SELECT qtyreqd,woitems.stockid FROM  woitems,stockmaster,bio_maincat_subcat WHERE wo='".$_POST['WO']."'
 ...
 $res_getrwcount=DB_query($sql_getrwcount,$db);
 $row_getrwcount=DB_fetch_array($res_getrwcount);

    $sql_getcomp="SELECT bom.component,stockmaster.description
    FROM bom,stockmaster
    WHERE bom.parent='".$row_getrwcount['stockid']."'
        AND bom.component=stockmaster.stockid 
         AND bom.component NOT IN (SELECT woitems.stockid FROM woitems WHERE woitems.wo='".$_POST['WO']."' )
        AND stockmaster.mbflag='M'
        ";
        $result_getcomp=DB_query($sql_getcomp,$db); 
        $j=0;
        while($row_getcomp=DB_fetch_array($result_getcomp))
        {
            $sql_getrqd="SELECT bom.quantity FROM bom WHERE bom.component='".$row_getcomp['component']."'
            AND bom.parent='".$row_getrwcount['stockid']."'
            AND bom.loccode='".$_POST['StockLocation']."'
           
            ";
            $result_getrqd=DB_query($sql_getrqd,$db);
            $row_get_rqd=DB_fetch_array($result_getrqd);
              $total=$row_getrwcount['qtyreqd']*$row_get_rqd['quantity'];
        
        if(isset($_POST['components']))
        {

           addWO_forsubassembly($db,$_POST['WO'],$row_getcomp['component'],$total,$_POST['StockLocation']); 
        }
        
        }

/*"-".$sql_getrwcount['qtyreqd'].*/
...
echo '<p align=center><a href="' . $rootpath . '/SelectWorkOrder.php">' . _('Select a work order') . '</a>'; 
...
$LocResult = DB_query("SELECT loccode,locationname FROM locations where loccode IN (7,8)",$db);
...
echo '<div class="centre"><br /><input type=submit name="submit" value="' . _('Confirm') . '">';

echo '<br /><p><input type=submit name="delete" VALUE="' . _('Delete This Work Order') . '" onclick="return confirm(\'' . _('Are You Sure?') . '\');">';



//CHECK WHETHER SELECT ANY PLANT

/*$sql_numrow="SELECT COUNT(*) FROM woitems WHERE wo='".$_POST['WO']."'";
$result_numrow=DB_query($sql_numrow,$db);
$row_numrow=DB_fetch_array($result_numrow);

if($row_numrow[0]!=0) // IF ANY PLANT IS SELECTED SHOWING IT'S COMPONENTS
{
    $Lineno=$row_numrow[0]+1;
  $sql_getrwcount="SELECT qtyreqd,woitems.stockid FROM  woitems,stockmaster,bio_maincat_subcat WHERE wo='".$_POST['WO']."'
 AND woitems.stockid= stockmaster.stockid
...
echo'<p><table width=85% class=selection>';
echo '<tr><th>' . _('Components') . '</th>
          <th>' . _('Unit Qty Required') . '</th>
          <th>' . _('Total') . '</th>
          <th>' . _('Create WO') . '</th>
          </tr>';
    ...
    WHERE bom.parent='".$row_getrwcount['stockid']."'
        ...
        while($row_getcomp=DB_fetch_array($result_getcomp))
        {
            ...
            echo "<td>".$row_getcomp['component']."-".$row_getcomp['description']."</td>";
              echo "<td>".$row_get_rqd['quantity']."</td>";
              $total=$row_getrwcount['qtyreqd']*$row_get_rqd['quantity'];
            echo "<td>".$total."</td>";
            echo "<td><a href='WorkOrderEntry_new2.php?WO=".$_POST['WO']."&NewItem=".$row_getcomp['component']."&Line=".$Lineno."&Qty=".$total."'>Create WO</a></td></tr>";
...
/*"-".$sql_getrwcount['qtyreqd'].*/
/*echo '</table>';  */
/*echo '<br /><p><input type=submit name="components" VALUE="' . _('Add Work Order') . '" onclick="return confirm(\'' . _('Are You Sure?') . '\');">'

;*/

if($row_numrow[0]==1 and $row_numrow['qtyreqd']!=0){
echo'<p><table width=85% class=selection>';
echo '<tr><th>' . _('Components') . '</th>
          <th>' . _('Unit Qty Required') . '</th>
          <th>' . _('Total') . '</th>
    ...
    WHERE bom.parent='".$row_getrwcount['stockid']."'
        ...
        while($row_getcomp=DB_fetch_array($result_getcomp))
        {
            ...
            echo "<td>".$row_getcomp['description']."</td>";
              echo "<td>".$row_get_rqd['quantity']."</td>";
              $total=$row_getrwcount['qtyreqd']*$row_get_rqd['quantity'];
            echo "<td>".$total."</td>";
            echo "</tr>";
...
/*"-".$sql_getrwcount['qtyreqd'].*/
echo '</table>';  
echo '<br /><p><input type=submit name="components" VALUE="' . _('Add Work Order For Assembly') . '" onclick="return confirm(\'' . _('Are You Sure?') . '\');">


 <br /></div>';
}

//if($row_numrow[0]==0){ // IF NO PLANT SELECTED SHOW PLANT SEARCH OPTION j+
    
    
/*    $SQL="SELECT categoryid, categorydescription FROM stockcategory 
WHERE stockcategory.categoryid IN (SELECT `subcatid` FROM `bio_maincat_subcat` WHERE `maincatid` IN(1))
AND  (stocktype='F' OR stocktype='D')
ORDER BY categorydescription";
    $result1 = DB_query($SQL,$db);*/

echo'<table class=selection><tr><input type="hidden" name="StockCat" value="All">';//j+
...
if (isset($SearchResult)) {
	...
	if (DB_num_rows($SearchResult)>1){
		...
		echo '<table width=85% cellpadding=2 colspan=7 class=selection>';
		...
		while ($myrow=DB_fetch_array($SearchResult)) {
			...
			if (!in_array($myrow['stockid'],$ItemCodes)){
				...
				printf('<td><font size=3>%s</font></td>
						<td><font size=2>%s</font></td>
						<td><font size=2>%s</font></td>
						<td>%s</td>
						<td><font size=2><a href="%s">'
