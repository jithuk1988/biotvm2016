...
if(isset($_POST['search']))
{
  ...
  $SQL="select mrpdemands.stockid,mrpdemands.quantity,stockmaster.description FROM  mrpdemands,stockmaster WHERE mrpdemands.duedate='".$_POST[date1]."' AND mrpdemands.location='".$location."' AND mrpdemands.workcenter='".$workcenter."' AND quantity>0 
...
echo"<tr>";
echo "<tr><th>Sl No.</th><th width=185px>Item</th><th>Pending SO</th><th>Todays Plan(All WC)</th><th>Pending WO</th><th>Qty. For WO</th></tr>";
...
$Res_PO=DB_fetch_array($Get_PO);
    ...
    echo "<td>".$Res_PO[0]."</td>";
    echo "<td><input type=text name='Qty".$j."' value=".$Row['quantity']." ></td></tr>";
    echo "<input type=hidden name='stock".$j."' value=".$Row['stockid'].">";
    $i++;
    $j++;
}
echo "</table>";
echo "<input type=hidden name=rowcount value=".$j.">";

    echo '<div class="centre"><br /><input type=submit name="submit" value="' . _('ADD WO') . '">';
...
controlWindow=window.open("bio_view_wo.php?y=" +str1 + "&m="+str2+ "&d="+str3 + "&loc=" +str4,"selplant","toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=no,width=700,height=400");
}
