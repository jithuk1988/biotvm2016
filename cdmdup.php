...
function exc()
{
      controlWindow=window.open("excelccr1.php","Excel","toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=no,width=100,height=100");
...
<?php
 echo '<center><font style="color: #333;
    ...
    font-size:16px;
    font-family:Georgia;
    text-shadow: 1px 1px 1px #666;"><br>BIOTECH IORA LIST</br></font></center>';
...
ifnull(date_format(bio_installation_status.installed_date,'%d-%m-%Y'),'NA') as insdate,
LEFT(stockitemproperty.model,1) as model,stockitemproperty.capacity,
 bio_plant_status.code as plantstatus ,orderplant.stkcode as 'stockid',
...
left join bio_plant_status on bio_installation_status.plant_status=bio_plant_status.p_id where salesorders.debtorno not like '0000' "; 
...
`bio_district`.code as district ,bio_state.code,`bio_oldorders`.`debtorno` as 'debtorno' ,
  ifnull(date_format( `bio_oldorders`.`installationdate` , '%d-%m-%Y' ),'NA') AS insdate,LEFT(stockitemproperty.model,1) as model 
  , stockitemproperty.capacity, bio_plant_status.code as plantstatus,bio_oldorders.plantid as 'stockid',
 bio_corporation.corporation,
bio_municipality.municipality,
custbranch.block_name ,
bio_panchayat.name as pname,
...
LEFT JOIN bio_plant_status ON bio_oldorders.plantstatus = bio_plant_status.p_id
...
"; 
            ...
            $sql2.= " order by left(bio_oldorders.debtorno,1),length(bio_oldorders.debtorno),bio_oldorders.debtorno";
               */
               ...
               $sqlall="select * from (".$sql0."union  ".$sql1.")as T order by T.insdate asc ";
  $sql33= $sql0. " union ". $sql1; 
            $sql2.= " order by left(bio_oldorders.debtorno,1),length(bio_oldorders.debtorno),bio_oldorders.debtorno";
 
$_SESSION['qry']=$sqlall;

    $result=DB_query($sqlall,$db);  
$i=1;
