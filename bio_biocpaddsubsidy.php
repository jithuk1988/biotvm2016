...
if ($_GET['ledid']!='' AND $_GET['item']!='') {
 ...
 $first=$_GET['first'];
 $add=$_GET['add'];
 $approval=$_GET['approval'];         //from approval page
...
if(isset($_GET['delete']) AND $_GET['delete']=='yes'){
  ...
  var approval='<?php echo $approval; ?>'; 
  
  if(approval!=3){
   window.opener.location='bio_conceptproposal.php?add=1&lead='+<?php echo $lead;?>+'&first=3'+'&stockid='+stock+'&Subsidy=1';
  }
  else
  {
   var sub=1;   
   window.opener.location='bio_approval.php?lead='+<?php echo $lead;?>+'&stockid1='+stock+'&subsidy='+sub;   
  }
...
//  var app=document.getElementById('approval').value;     alert(app); 
  window.opener.location='bio_conceptproposal.php?add=1&lead='+<?php echo $lead;?>+'&first=3'+'&stockid='+stock+'&Subsidy=1';
  }
  else
  {
...
echo '<table width=80%>';
    ...
    echo'<input type="hidden" name="approval" id="approval" value="' . $approval . '" style="width:200px">';  
...
if($count<=0 && $_GET['delete']!='yes')
{
echo     $sql_select="SELECT * FROM bio_cpsubsidy WHERE leadid=".$_SESSION['leadid']." AND stockid='".$_SESSION['stockid']."'";
    $result_select=DB_query($sql_select,$db);
    
    while($row_select=DB_fetch_array($result_select))
    {
        
        $sql_insert="INSERT INTO bio_temppropsubsidy (leadid,stockid,scheme,amount)
