...
echo"<tr><td>Name</td><td>Contact No</td><td>Register Type</td><td>Archieve</td></tr>";

//echo"<tr><td>CreatedON From</td><td></td><td>Name</td><td>Contact No</td><td>Client From</td><td>Client To</td><td>Register Type</td></tr>";
//echo"<tr>"; 
...
//echo "<td><input type=text id='createdto' class=date alt=".$_SESSION['DefaultDateFormat']." name='createdto' value='$_POST[createdto]' onChange='return isDate(this, this.value, ".$_SESSION['DefaultDateFormat'].")'></td>";
 ...
 echo'</select></td>';
   if($_POST[arc]==1){
       ...
       <option selected  value=1>Archieved</option>
       <option  value=0>Unarchieved</option> </select></td>'; 
   ...
   else
   {
      echo'<td><select name=arc id=arc >
       ...
       <option selected value=0>Unarchieved</option> </select></td>';    
...
if($_POST['regtype']==1){$title.=" of Consolidated ";}
elseif($_POST['regtype']==2 || $_GET['regtype']==2){$title.=" of Detailed ";}
...
if($_POST['regtype']==1)   {   
   ...
   echo"</tr>";    
    ...
    echo"<tr><th>SL No</th>
             ...
             <th>Created On</th> <th>Doc View</th><th>Reference</th></tr>"; 
...
$sql_old="SELECT DISTINCT `bio_oldorderdoclist`.`orderno` AS oldorders,0 AS neworders
    ...
    ,date_format(`debtorsmaster`.`clientsince`,'%d-%m-%Y') as 'clientsince'
    , `custbranch`.`phoneno`
    , `custbranch`.`faxno`
    , `bio_district`.`district`
...
FROM
 ...
 $sql_old .=" AND bio_oldorders.currentstatus=".$_GET['plantid'];    
     ...
     if($_POST['arc']==1)
                   {
                       $sql_old .= " AND `debtorsmaster`.`debtorno` in  (SELECT bio_fileno.debtorno FROM `bio_fileno`)  AND `bio_oldorders`.`debtorno` in  (SELECT bio_fileno.debtorno FROM `bio_fileno`) ";     
                   }   
                else    if($_POST['arc']==0)
                    {
                            $sql_old .= " AND `debtorsmaster`.`debtorno` not in (SELECT bio_fileno.debtorno FROM `bio_fileno`) AND `bio_oldorders`.`debtorno` not in  (SELECT bio_fileno.debtorno FROM `bio_fileno`)  ";                                                                                                                                                                                                
                    }
...
FROM  `bio_documentlist`        
 ...
 $sql_so .=" AND salesorders.currentstatus=".$_GET['plantid'];  
    ...
    if($_POST['arc']==1)
                   {
                       $sql_so .= " AND `debtorsmaster`.`debtorno` in  (SELECT bio_fileno.debtorno FROM `bio_fileno`) AND `salesorders`.`debtorno` in  (SELECT bio_fileno.debtorno FROM `bio_fileno`) ";     
                   }   
                else    if($_POST['arc']==0)
                    {
                            $sql_so .= " AND `debtorsmaster`.`debtorno` not in (SELECT bio_fileno.debtorno FROM `bio_fileno`) AND `salesorders`.`debtorno` not in  (SELECT bio_fileno.debtorno FROM `bio_fileno`)  ";                                                                                                                                                                                                
                    }
           // AND    bio_oldorderdoclist.status=0
           if(isset($_POST['filter']) || ($_GET['regtype']==2))
           {           
...
if($row_doc['status']<1) {
    ...
    $fontcolor='blue';
      ...
      echo"<td title='".$row_doc['document_name']."'> <font color='$fontcolor'>".$status."</font></td>";    
               ...
               echo"        
               <td><a href id='".$ordtab."-".$new."-".$old."-".$enq."'   onclick='viewdocuments(this.id);return false;'>View</a></td>
               </tr>";
...
{   
    ...
    $sqld="select debtorno from bio_fileno where debtorno='".$row_old['debtorno']."'";
      ...
      echo"
               ...
               <td>".$row_old['clientsince']."</td>
               <td><a href id='".$ordtab."-".$new."-".$old."-".$enq."' onclick='viewdocuments(this.id);return false;'>View</a></td> 
                  <td><a style='font-weight:bold' href id='".$row_old['debtorno']."' onclick='reference(this.id);return false;'>Add Ref Lead</a></td>
               </tr>
          ";
