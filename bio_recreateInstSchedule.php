...
//  $title = _('Task Re-assigning'); 
 ...
 $sql_max="SELECT max(tid) from bio_leadtask where leadid=".$leadid." AND taskid=".$taskid;
        $result_max=DB_query($sql_max,$db);
      $row_max=DB_fetch_array($result_max);
      $maxid=$row_max[0];
      $sql_remark='UPDATE bio_leadtask SET remarks="'.$remarks.'" WHERE leadid='.$leadid.' AND taskid='.$taskid.' AND tid='.$maxid;   DB_query($sql_remark,$db);    
            if($status==1)
      {
          $sql1="UPDATE bio_leadtask SET taskcompletedstatus=1,taskcompleteddate='$crdt' WHERE leadid=$leadid AND taskid=$taskid 
          AND tid=".$maxid." ";      DB_query($sql1,$db);       
      }
      elseif($status==2)
      { 
...
echo "<form method='post' action='" . $_SERVER['PHP_SELF'] . "?" . SID . "'>";  
 ...
 $sql_source="SELECT bio_leads.sourceid FROM bio_leads
            WHERE bio_leads.leadid=".$leadid;
 $result_source=DB_query($sql_source,$db);
 $myrow_source=DB_fetch_array($result_source);
  
  $sourceid=$myrow_source['sourceid'];
...
echo"<tr><td>"; 
echo"<fieldset style='width:800px'><legend>Edit Scheduled Date Schedule</legend>";
  ...
  {    
     ...
     $duedate=ConvertSQLDate($row['duedate']);
          ...
          { 
              echo"<td>$no</td>
               <td>".$row['task']."</td>        
               <td><input type='text' name='assignd' id='assignd' readonly value='$assignDate' onchange=updateDate(this.value,'$taskid','$leadid',1)></td>   
               <td><input type='text' name='dued' id='dued' value='".$duedate."' readonly onchange=updateDate(this.value,'$taskid','$leadid',2)></td>"; 
          ...
          echo"<td>$no</td>
               <td>".$row['task']."</td>        
               <td><input type='text' name='assignd' readonly id='assignd'  value='$assignDate' onchange=updateDate(this.value,'$taskid','$leadid',1)></td>   
