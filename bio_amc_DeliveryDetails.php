...
if (isset($_POST['BackToLineDetails']) and $_POST['BackToLineDetails']==_('Modify Order Lines')){
	...
	echo '<meta http-equiv="Refresh" content="0; url=' . $rootpath . '/bio_amc_order.php?identifier='.$identifier   . '">';
...
/* finally write the order header to the database and then the order line details */
	...
	$result = DB_Txn_Commit($db);
            ...
            $AMCorder = "UPDATE bio_amc SET new_orderno='". $OrderNo . "' WHERE debtorno='".$_SESSION['Items'.$identifier]->DebtorNo."' "; 

    $ErrMsg = _('The order cannot be added because');
    $UpdateAMCOrder = DB_query($AMCorder,$db,$ErrMsg);
         ?>
         <script>
            
            window.opener.location='bio_amcRegister.php';  
         </script>
