...
if (isset($_POST['submit'])) {
	...
	if ($InputError !=1) {
       ...
       if($_POST['bsid']!=null and $_POST['SalesmanCode']!=null)   
{ 
$sman=$_POST['SalesmanCode'];
$bsid=$_POST['bsid'];

  $sqlr="update bio_dealerterritory set salesmancode='".$_POST['SalesmanCode']."' where buss_id='".$_POST['bsid']."'";
                    DB_query($sqlr,$db);       
}      
...
if (! isset($_GET['delete'])) {
    ...
    if($_GET['bsid']!=null and $_GET['loccode']!=null)   
{ 
$locode=$_GET['loccode'];
$bsid=$_GET['bsid'];
...
$result = DB_query($sqltr, $db); 
   ...
   echo '<input type="hidden" name="bsid" value="' . $_GET['bsid'] . '" />';    
    //echo '<input type="hidden" name="bsid" value="' . $_GET['bsid'] . '" />';     
	if (isset($SelectedSaleperson)) {
		...
		echo '<table class=selection><tr><td>' . _('Dealer code') . ':</td>
	...
	echo '<tr><td>' . _('Dealer Name') . ':</td><td><input type="text" '. (in_array('SalesmanName',$Errors) ? 'class="inputerror"' : '' ) .' name="SalesmanName"  size=30 maxlength=30 VALUE="' . $_POST['SalesmanName'] . '"></td></tr>';
...
//    $SQL = "SELECT batype_id,businessasociatetypes FROM bio_businesasociatemaster";
//    $result = DB_query($SQL,$db);

//    while ($myrow = DB_fetch_array($result)) {
//        if ($myrow['batype_id']==$_POST['TypeAbbrev']) {
//            echo '<option selected VALUE="';
//        } else {
//            echo '<option VALUE="';
//        }
//        echo $myrow['batype_id'] . '">' . $myrow['businessasociatetypes'] . '</option>';
...
//=================================================================================================  
    ...
    echo'<tr><td> Area</td>';
    echo'<td><select id="barea" name="BArea"> ';
        $sql1="SELECT areas.areacode,areas.areadescription FROM areas";            
    $result1=DB_query($sql1,$db) ;
    $a=0;
      while ($myrow1 = DB_fetch_array($result1)) {
    if ($myrow1['areacode']==$_POST['areadescription']) 
    {
    echo '<option  selected value="';
     } 
        ...
        echo '<option value="';
        echo $myrow1['areacode'] .'">'.$myrow1['areadescription'];
        echo  '</option>';
        $a++;
    }
