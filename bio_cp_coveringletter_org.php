...
//=================================================kuiiiii====================================================================//  
  ...
  //==========================================================================================================================//
    ...
    $leadid=$_POST['LeadID'];
    $cpID=$_POST['CpID'];
    $feacondate=FormatDateForSQL($_POST['feacondate']);  
    
    
    $sql_update="UPDATE bio_fs_entrydetails SET fs_date='$feacondate' WHERE leadid=$leadid";
...
$item_price=array();
 ...
 $total_plantprice_sub=$total_plantprice;    
  ...
  $sql_plant="SELECT bio_conceptproposaldetails.stockid,bio_conceptproposaldetails.tprice AS plantprice,bio_conceptproposaldetails.qty AS quantity,stockmaster.description AS plantdesc
                FROM   bio_conceptproposaldetails,stockmaster 
                WHERE  bio_conceptproposaldetails.stockid=stockmaster.stockid
  ...
  $quantity=$row_plant['quantity'];             
                                    
 $optionalitem=array(); 
 $optionalprice=array(); 
 $sql_notplant="SELECT bio_conceptproposaldetails.stockid,bio_conceptproposaldetails.tprice,bio_conceptproposaldetails.qty AS quantity,stockmaster.description 
                FROM   bio_conceptproposaldetails,stockmaster 
                WHERE  bio_conceptproposaldetails.stockid=stockmaster.stockid
 ...
 while($row_notplant=DB_fetch_array($result_notplant))
 {
    ...
    $optionalqty[]=$row_notplant['quantity'];
 }                
...
// 
   ...
   $mnresub=$row_subsidy1['amount'];   
   $row_subsidy2=DB_fetch_array($result_subsidy2);
   $biosub=$row_subsidy2['amount'];
...
//=======================================================================================================================//
 ...
 $string='Kind Attention:'." ".$contactperson;  
  ...
  $pdf->addTextWrap($Left_Margin+30,$YPos-=$line_height,300,$FontSize,$name,'',0);    
  $pdf->addTextWrap($Left_Margin+30,$YPos-=$line_height,300,$FontSize,$place,'',0);                                                                                
  $pdf->addTextWrap($Left_Margin+30,$YPos-=$line_height,300,$FontSize,$district,'',0);    
...
// if($letterno!="NULL"){
    ...
    $pdf->addTextWrap($Left_Margin+290,$YPos,200,$FontSize+1,$_POST['feacondate']);  
...
//$pdf->addTextWrap($Left_Margin+20,$YPos-=$line_height+50,200,$FontSize+1,_('With warm regards,'), 'left');      
 $pdf->addTextWrap($Left_Margin+20,$YPos-=$line_height+40,200,$FontSize+1,_('Yours faithfully'), 'left');      
...
//       $YPos-=(17*(($i+1)-$j));
 ...
 $pdf->addJpegFromFile($img4,$Left_Margin+345,$YPos,0,9); 
 $pdf->addTextWrap($Left_Margin+430,$YPos,40,$FontSize+1,$annual_biogas_income.".00",'right',0);
 $pdf->line($Left_Margin-2,$YPos-=2,$Left_Margin+498,$YPos);
 $i++;
    ...
    $pdf->addJpegFromFile($img4,$Left_Margin+345,$YPos,0,9);
    $pdf->addTextWrap($Left_Margin+430,$YPos,40,$FontSize+1,$annual_organic_fertilizer.".00",'right',0);   
    $pdf->line($Left_Margin-2,$YPos-=2,$Left_Margin+498,$YPos);      
    $i++;                       
...
// $pdf->addTextWrap($Left_Margin+10,$YPos-=$line_height,100,$FontSize+1,$i-1,'',0);    
 ...
 $pdf->addJpegFromFile($img4,$Left_Margin+345,$YPos,0,9);                              
 $pdf->addTextWrap($Left_Margin+430,$YPos,40,$FontSize+1,$total_annual_income.".00",'right',0);
 $pdf->line($Left_Margin-2,$YPos-=2,$Left_Margin+498,$YPos);   
 $i++;
 ...
 $pdf->addJpegFromFile($img4,$Left_Margin+345,$YPos,0,9); 
 $pdf->addTextWrap($Left_Margin+430,$YPos,40,$FontSize+1,$biosub.".00",'right',0);
 $pdf->line($Left_Margin-2,$YPos-=2,$Left_Margin+498,$YPos);
 $i++;
...
$pdf->addJpegFromFile($img4,$Left_Margin+345,$YPos,0,9); 
 $pdf->addTextWrap($Left_Margin+430,$YPos,40,$FontSize+1,$mnresub.".00",'right',0);   
 $pdf->line($Left_Margin-2,$YPos-=2,$Left_Margin+498,$YPos);     
 $i++;
 ...
 $pdf->addJpegFromFile($img4,$Left_Margin+345,$YPos,0,9); 
 $pdf->addTextWrap($Left_Margin+430,$YPos,40,$FontSize+1,$othersub.".00",'right',0);   
 $pdf->line($Left_Margin-2,$YPos-=2,$Left_Margin+498,$YPos);      
 $i++;                   
 }
 else
 ...
 $pdf->addJpegFromFile($img4,$Left_Margin+345,$YPos,0,9);                               
 $pdf->addTextWrap($Left_Margin+430,$YPos,40,$FontSize+1,$total_sub.".00",'right',0);
 $pdf->line($Left_Margin-2,$YPos-=2,$Left_Margin+498,$YPos);   
 $i++;
 ...
 $pdf->addTextWrap($Left_Margin,$YPos-=$line_height,20,$FontSize+1,_('No'), 'center');
 $pdf->addTextWrap($Left_Margin+20,$YPos,200,$FontSize+1,_('Description'), 'center'); 
 $pdf->addTextWrap($Left_Margin+240,$YPos,100,$FontSize+1,_('Quantity'), 'center');  
 $pdf->addTextWrap($Left_Margin+300,$YPos,100,$FontSize+1,_('Currency'), 'center');
 $pdf->addTextWrap($Left_Margin+400,$YPos,100,$FontSize+1,_('Amount'), 'center');
 $pdf->line($Left_Margin-2,$YPos-=2,$Left_Margin+498,$YPos); 
 $i++;
 ...
 $pdf->addTextWrap($Left_Margin+290,$YPos,900,$FontSize+1,$quantity,'',0);  
 $pdf->addJpegFromFile($img4,$Left_Margin+345,$YPos,0,9);    
 $pdf->addTextWrap($Left_Margin+430,$YPos,40,$FontSize+1,number_format($plantprice,2),'right',0); 
 $pdf->line($Left_Margin-2,$YPos-=2,$Left_Margin+498,$YPos); 
 $i++;
       $m=2;
       for($j=0;$j<8;$j++)
       {  
          ...
          $tot_optprice+=$optionalprice[$j];
                ...
                if($optionalitem[$j]!="") 
                { 
                $pdf->addTextWrap($Left_Margin+10,$YPos,100,$FontSize+1,$m,'',0);  
                $pdf->addTextWrap($Left_Margin+30,$YPos,900,$FontSize+1,$optionalitem[$j],'',0); 
                $pdf->addTextWrap($Left_Margin+290,$YPos,900,$FontSize+1,$optionalqty[$j],'',0); 
                $pdf->addJpegFromFile($img4,$Left_Margin+345,$YPos,0,9); 
                $pdf->addTextWrap($Left_Margin+430,$YPos,40,$FontSize+1,number_format($optionalprice[$j],2),'right',0);
                }
                $pdf->line($Left_Margin-2,$YPos-=2,$Left_Margin+498,$YPos);
           $m++;     
       }  
       
        $total_price=$plantprice+$tot_optprice; 
         ...
         $pdf->line($Left_Margin+265,$YPos+=$line_height+2,$Left_Margin+265,$YPos+(17*9));
         
        $pdf->addTextWrap($Left_Margin+50,$YPos-=$line_height-2,200,$FontSize+1,_('Total project cost'), 'center');   
        $pdf->addJpegFromFile($img4,$Left_Margin+345,$YPos,0,9);  
        $pdf->addTextWrap($Left_Margin+430,$YPos,40,$FontSize+1,number_format($total_price,2),'right',0);  
        $i=$i+8; 
