...
echo '<center><font style="color: #333;
 ...
 if($_GET['orderno']!='')
 {
     ...
     $enqid=$_GET['enq'];
     $sql="SELECT bio_oldorders.debtorno,
     debtorsmaster.name,
     custbranch.phoneno,
     ...
     $row['debtorno'];
     $sql1="SELECT * FROM bio_fileno where debtorno='".$row['debtorno']."' AND orderno=".$orderno;
     ...
     if($row1['fileno']==NULL)
     {
         if($row['LSG_type']==1)
          ...
          $sql2="SELECT MAX(id) as maxid , LSG_type ,LSG_id FROM bio_lsg_fileno WHERE LSG_type=$lsgtype AND LSG_id=(SELECT id FROM bio_corporation WHERE country=$cid AND state=$stateid AND district=$did)";
         //echo $sql2;
         $result2=DB_query($sql2,$db);
                    ...
                    $row4=DB_fetch_array($result4,$db);
                     ...
                     $bin=$row4['bin'];
                       $fileno=explode("/",$row4['fileno']);
                        if(sizeof($fileno)==5){  $fileno1=$row4['fileno']."/L0";}
                           elseif(sizeof($fileno)==6){
                           //$lastindex=$fileno[5]+1;
                           $fileno1=$fileno[0]."/".$fileno[1]."/".$fileno[2]."/".$fileno[3]."/".$fileno[4]."/L".$fileno[5];
                            }
                     ...
                     $fileno2=$fileno1."/".$room."/".$rack."/".$bin."/".$debtorno."/".$orderno."/".$lastfileno;
                     $sql3="INSERT INTO bio_fileno (debtorno,orderno,fileno) VALUES ('".$row['debtorno']."' , $orderno , '$fileno2')";
                     DB_query($sql3,$db);
                     $sql1="SELECT * FROM bio_fileno where debtorno='".$row['debtorno']."' AND orderno=".$orderno;;
      $result1=DB_query($sql1,$db);
     $row1=DB_fetch_array($result1,$db);   
          
          $sql6="UPDATE bio_lsg_fileno set lastfileno=$lastfileno  where LSG_type=$lsgtype AND LSG_id=(SELECT id FROM bio_corporation WHERE country=$cid AND state=$stateid AND district=$lsgname) AND id='".$row2['maxid']."'";
          DB_query($sql6,$db);
                     }
          }
...
echo '<br /><table class="selection">
            ...
            <tr>
                <th colspan ="3"><b>' . _('Customer Type:') . ' ' . $row['enquirytype'] . '</b></th>
            </tr>
            <tr>
                <th colspan ="3"><b>' . _('File no:') . ' ' . $row1['fileno'] . '</b></th>
            </tr>
            <tr>
