...
echo"<legend><h3>Dispatch Cleared Items</h3></legend>";
    ...
    echo"<td><input type='submit' name='submit' id='submit' value=search></td></tr></table>";
        ...
        echo '<tr>  <th>' . _('Slno') . '</th>
              ...
              $sql_inst="SELECT salesorderdetails.orderno,bio_despatchClearence.id,
              custbranch.brname,
              custbranch.phoneno,
...
{     
       $sql_checkrec="SELECT count(id) as count from bio_deliverynote WHERE bio_deliverynote.orderno=".$row_so['orderno']." 
       AND bio_deliverynote.stockcode='".$row_so['stkcode']."' AND bio_deliverynote.despatch_id='".$row_so['id']."'"; 
          ...
          }else 
          {
            ...
            $k=1;     
                       ...
                       <td>".convertSQLDate($row_so['sched_instlln_date'])."</td> 
                                     ...
                                     <td width='75px'><a style='cursor:pointer;' id=$plant onclick='viewdocs(this.id,$orderno,$id);'>" . _('Cancel DC') . "</a></td>"; 
...
if (r==true)
  {
location.href="?orderno=" + str1 + "&plant=" + str + "&id=" +str2; 
  }
