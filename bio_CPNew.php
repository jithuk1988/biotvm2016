...
$sql3="INSERT INTO bio_temppropitems (`stockid`,`description`,`qty`,`price`,`tprice`,`leadid`,`subsidy`,`netprice`) 
       SELECT a.stockid,a.description,a.qty,a.price,a.tprice,$leadid,
       COALESCE(SUM(b.amount),0) AS subsidy,a.tprice-COALESCE(SUM(b.amount),0) AS netprice
       FROM bio_conceptproposaldetails a
       LEFT JOIN bio_cpsubsidy b ON b.cp_id=a.cp_id AND b.stockid=a.stockid
       WHERE a.cp_id=$cp_ID GROUP BY stockid";
       
$result3=DB_query($sql3,$db);
      


$sql_delsubsidy="DELETE FROM bio_temppropsubsidy WHERE leadid=$leadid";
DB_query($sql_delsubsidy,$db);
  
$sql_tempsubsidy="INSERT INTO bio_temppropsubsidy (leadid,stockid,scheme,amount) SELECT leadid,stockid,scheme,amount FROM bio_cpsubsidy WHERE cp_id=$cp_ID AND leadid=$leadid";    
DB_query($sql_tempsubsidy,$db);  
