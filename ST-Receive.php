...
//                } /*end if the transferred item is a controlled item */
    ...
    echo '<p><a href="'.$rootpath.'/PDFSrecn.php?' . SID . 'slno=' . $StkMoveNo . '">'.
        _('Print the Transfer Docket'). '</a>';
        echo "<br>";
   echo '<a href="'.$rootpath.'/StockLocTransferReceive.php.php?' . SID .'">' ._('Back to Store Receive page') . '</a><br>';
        ...
        if ($TrfLine->Controlled==1){
            if ($TrfLine->Serialised==1){
                echo '<td><a href="' . $rootpath .'/StockTransferControlled.php?' . SID . '&TransferItem=' . $i . '">' . _('Enter Serial Numbers') . '</a></td>';
                  ...
                  $sql2='SELECT  batchref,
                         qty                    
                         FROM loctransferbatch WHERE reference="'. $_SESSION['Transfer']->TrfID.'"
                         AND itemcode="'.$TrfLine->StockID.'"';
                  ...
                  $PropertyCounter=0;
                   ...
                   echo '<br><table><tr><th colspan="2">' . _('Batch/Lots Issued from the store') . '</th></tr></table>'; 
   
        while($myrow2=DB_fetch_array($result2)){
           
           
             echo '<table><tr><td><input type="textbox" name='."BatchRef".$PropertyCounter.' size="20" maxlength="100" value="' . $myrow2[0] . '">';
             echo '</td>
                     <td><input type="textbox" name='."Qty".$PropertyCounter.' value='.$myrow2[1].'>
                      </td>
                      </tr></table>';   
