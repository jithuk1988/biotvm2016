...
include('includes/header.inc');
 ...
 print'<script>
          myRef = window.open("bio_CP_autochange.php","autochange","toolbar=no,location=no,directories=no,status=yes,menubar=no,scrollbars=yes,resizable=no,width=600,height=400");
          </script> ';
}
       ...
       $sql15="Select value from bio_changepolicy where policyname='No of Days to became CP passive'";
                          $result5=DB_query($sql15,$db);   
  ...
  WHERE            bio_conceptproposal.lead_id= bio_leads.leadid 
            AND    bio_leadteams.teamid=bio_conceptproposal.team_id 
            AND    bio_leads.leadstatus=18 
...
include('includes/sidemenu.php');           
  ...
  echo '<center><font style="color: #333;
                           ...
                           font-size:16px;
                           font-family:Georgia;
                           text-shadow: 1px 1px 1px #666;">CONCEPT PROPOSAL PASSIVE LEADS</font></center>';
 ...
 echo "<legend><h3>Showing all Passive Leads of Concept Proposal</h3>";
 echo "</legend>";  
 
 echo '<form name="CPpassiveLeads"  method="post" action="' . $_SERVER['PHP_SELF'] . '?' .SID .'">';
...
//    echo'<tr><td colspan=6>Showing all Pending Leads</td>';
    ...
    $k=0;                       
      ...
      //$sql3="SELECT bio_conceptproposal.conceptproposal_id,bio_conceptproposal.createdon FROM bio_conceptproposal
...
//    echo'<td>'.$myrow3['reqno'].'</td>';
    ...
    echo "<td><a style=cursor:pointer; id=".$myrow3['lead_id']." onclick=viewDetails(this.id)>Create DPR</a></td>";
//    echo "<td><a style=cursor:pointer; id=".$row1['leadid']." onclick=select(this.id)>View</a></td>";
 ...
 ?>  
    ...
    <script>
    function viewDetails(str1){
       window.location="bio_detailedprojectproposal.php?leadid=" + str1;
    }
    </script>  
