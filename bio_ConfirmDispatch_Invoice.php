...
//--------------------------------------------------------------------------------------------------
    ...
    $sql_advance="SELECT SUM(amount) AS advance FROM bio_advance WHERE leadid=$leadid";
    $result_advance=DB_query($sql_advance,$db);
    $myrow_advance=DB_fetch_array($result_advance);
    $advance=$myrow_advance['advance'];
...
foreach ($_SESSION['Items']->LineItems as $LnItm) {
    ...
    $Displayadvance = number_format($advance,$_SESSION['Items']->CurrDecimalPlaces);
     
    
	echo '<td class="number">'.$DisplayTaxAmount.'</td><td class="number">'.$Displaysubsidy.'</td><td class="number">'.$DisplayGrossLineTotal.'</td>';
...
echo '<tr> 
     ...
     <td></td><td><td></td></td>
     <td class="number">-'.$Displayadvance.'</td>    
</tr>';   

 $total=($TaxTotal+$_SESSION['Items']->total + $_POST['ChargeFreightCost'])-$Gross_subsidy; 
 $grandtotal=$total-$advance;
 $grandtotal = number_format($grandtotal,$_SESSION['Items']->CurrDecimalPlaces);
 
 
echo '<tr> 
     <td colspan="10" class="number"><b>' . _('Grand Total'). '</b></td> 
     <td></td><td><td></td></td>
     <td class="number"><hr><b>'.$grandtotal.'</b><hr></td>    
</tr>'; 
