...
echo"</td></tr>"; 
  ...
  while($myrow_prop=DB_fetch_array($result_prop)){
      ...
      $subsidy=$myrow_sub[0];
      $netprice=$myrow_prop['tprice']-$subsidy;
      }else{
          $subsidy=0;
          $netprice=$myrow_prop['tprice'];
...
//$item=urlencode($myrow4['stockid']);
   echo "<tr><td>".$stock[$k]."</td><td>".$myrow4['description']."</td>
       ...
       <td align=center><a  style='cursor:pointer;'  id='$lead' onclick='removeitm($leadid,\"$item[$k]\",4,1)'>" . _('Remove') . "</a></td> 
       </tr>";
}
...
echo "<tr><td colspan=2>
          <input type='button' id=\"".$leadid."\" value='Add Item'  onclick='selectplant(this.id,2);'>
       </td>";
echo "<td colspan=3>
          <input type='submit' name='Editcp' id=\"".$leadid."\"  value='Save'>
       </td></tr>";
