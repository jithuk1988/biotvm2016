...
$PageSecurity=80;
/* $Id: SelectSalesOrder.php 4579 2011-05-29 04:07:16Z daintree $*/
...
$leadid=$_GET['lead'];           // from domTaskview
...
/*		echo '<tr><td>' . _('Order number') . ': </td>
		...
		echo '</select></td><td>';                         */
		echo '<tr style="display:none"><td><select name="Quotations">';                                      // display:none
...
//end if stock search results to show
    ...
    if ($_POST['Quotations']==""){ 
        ...
        $_GET['Quotations']='Orders_Only'; 
        $Quotations = 0;       
    }
	elseif (isset($_POST['Quotations']) AND $_POST['Quotations']=='Orders_Only'){
		$Quotations = 0;            
	} else {
		$Quotations =1;
	}
    ...
    if (isset($_GET['lead'])) {
     ...
     $SQL = "SELECT salesorders.orderno,
                ...
                GROUP BY salesorders.orderno,
                    ...
                    salesorders.printedpackingslip,
                    salesorders.poplaced
                ORDER BY salesorders.orderno";               // AND salesorders.orderno=". $_REQUEST['OrderNumber'] ."     AND salesorders.so_status =1 
    }                                                        
...
//		if (isset($_POST['Quotations']) AND $_POST['Quotations']=='Orders_Only'){ 
        if (isset($_GET['Quotations']) AND $_GET['Quotations']=='Orders_Only'){  
			$tableheader = '<tr>
						<th>' . _('Modify') . '</th>
						<th>' . _('Invoice') . '</th>
		...
		while ($myrow=DB_fetch_array($SalesOrdersResult)) {
			...
			$Confirm_Invoice = $rootpath . '/bio_ConfirmDispatch_Invoice.php?OrderNumber=' .$myrow['orderno'];
			...
			if (($_POST['Quotations']=='Orders_Only') OR ($_GET['Quotations']=='Orders_Only')){
