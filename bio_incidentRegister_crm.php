...
echo '<center><font style="color: #333;
    ...
    font-size:16px;
    font-family:Georgia;
    text-shadow: 1px 1px 1px #666;">INCIDENT REGISTER</font></center>';
 ...
 //   $landno_no=$phpieces[1];
   ...
   {   $order=$_GET['selectOrder'];  
       $sql="SELECT 
                   ...
                   debtorsmaster.address5,debtorsmaster.did,
                   custbranch.debtorno,
                   custbranch.phoneno,
                   custbranch.email
...
if (isset($_POST['update'])){  
    ...
    $sql_update_add="UPDATE bio_incident_cust SET 
...
Biotech Helpdesk"; 
        ...
        $sql1 = "INSERT INTO bio_incidents(cust_typ,warr_type,chargable,
                                           cust_id,
                                           type,
                                           source,
                                           enqtypeid,
                                           ...
                                           mainmailcategory,
                                           submailcategory)
                               VALUES ('".$_POST['cust_typ']."','".$_POST['warr_type']."','".$_POST['chargable']."',
                                        '". $cust_id . "',
                                       '". $_POST['type'] . "',
                                       '". $_POST['source'] . "',
                                       '". $_POST['enquiry'] . "',
...
echo '<form name="sub" method="post" action="' . $_SERVER['PHP_SELF'] . '">'; 
echo '<div id=panel1>';
echo '<table style=width:40%><tr><td>';  
     
echo"Ticket No:";
echo"<input type='text' name='ticketno' id='ticketno' tabindex=0  style='width:75px' onChange='searchCustomer()'>";
echo'&nbsp;&nbsp;<input type=button value=Search onClick="department();"></br>';
if($_GET['selectOrder'])
{
       
    echo'&nbsp;&nbsp;<input type=button value="View History" onClick="history();" >';
    //echo'&nbsp;&nbsp;<input type=button autofocus value="Check Warranty" onfocus="check_warranty();" >';   
  // echo"<input type='text' autofocus value='".$_GET['selectOrder']."'    onfocus='check_warranty()'>";                                              
    // check_warranty(); 
}
...
echo '<fieldset style="width:400px; height:auto">';  
...
echo"<td><input type='text' name='custname' id='custname' tabindex=1 value='".$custname."' style='width:220px'  onChange='searchCustomer()'></td></tr>";
...
echo "</select></td></tr>"; 
echo"<tr><td width=33%>Complaint Item</td>";  
if($_GET['selectOrder'])  
{
   $ord=$_GET['selectOrder'];
  
echo"<td><select name='complaint_item' id='complaint_item' tabindex=35 style='width:219px' >";

            $sql15="SELECT salesorderdetails.stkcode,stockmaster.stockid,stockmaster.description FROM stockmaster
            LEFT JOIN salesorderdetails ON salesorderdetails.stkcode=stockmaster.stockid WHERE salesorderdetails.orderno=$ord"; 
    $result25=DB_query($sql15,$db);
         $f=0; 
    while($myrow25=DB_fetch_array($result25))
    { 
  /* if ($myrow25['stkcode']==$myrow25['stockid']) 
    {
    ...
    echo '<option selected value="';
        ...
        $f++;
    //}
    echo $myrow25['stkcode'].'>'.$myrow25['description'].'</option>';
   // echo '</option>';
    }
...
echo "</select></td></tr>"; 
 ...
 {
     echo "<td><input type:=text name='complaint_item' id='complaint_item'  style='width:219px'></td></tr>";
 }
...
if(isset($_GET['selectOrder']))
 { 
   $flage=6;
   $ord=$_GET['selectOrder'];
   $sql="select bio_installation_status.installed_date FROM bio_installation_status WHERE bio_installation_status.orderno=$ord " ;
 $result=DB_query($sql,$db);
...
//echo"<td><input type='text' readonly name='custname' id='custname' tabindex=1 value='".$valid."' style='width:120px' ></td></tr>";   
 ...
 LEFT JOIN salesorders ON bio_amc.debtorno=salesorders.debtorno           
   WHERE salesorders.orderno=$order " ;        //  LEFT JOIN salesorders ON bio_amc.debtorno=salesorders.debtorno  
$result=DB_query($sql,$db);
 $myrow=DB_fetch_array($result);
...
echo '<legend><b>Details</b></legend>';
echo '<table width=90% >';              
  
echo '<tr><td ><b>Biotech Customer&nbsp;</td><td><b>Warranty&nbsp;</td><td><b>Chargable</td></b>';
echo '<tr>';
if(isset($_GET['selectOrder']))
{
  echo '<tr><td><Input type = "Radio" Name ="cust_typ" id="cust_typ" tabindex=30 value= "1" checked >yes</td>';
 
  if($flage==0)
  {

        echo '<td><Input type = "Radio" Name ="warr_type" id="warr_type" tabindex=31 value="1" checked >Warranty</td>'; 
            echo '<td><Input type = "Radio" Name ="chargable" id="chargable" tabindex=32  value= "1" >yes</td></tr>'; 
    echo '<tr><td><Input type = "Radio" Name ="cust_typ" id="cust_typ" tabindex=30 value= "2"  >No</td> '; 
        echo '<td><Input type = "Radio" Name ="warr_type" id="warr_type" tabindex=31 value="2"  >AMC</td> '; 
            echo ' <td><Input type = "Radio" Name ="chargable" id="chargable" tabindex=32 value= "2" checked >NO</td></tr>';  
    echo '<tr><td> <td><Input type = "Radio" Name ="warr_type" id="warr_type"  tabindex=31 value="3" >Other</td></tr>';
  ...
  {   
        echo '<td><Input type = "Radio" Name ="warr_type" id="warr_type" tabindex=31 value="1"  >Warranty</td>'; 
            echo '<td><Input type = "Radio" Name ="chargable" id="chargable" tabindex=32  value= "1" checked>yes</td></tr>'; 
    echo '<tr><td><Input type = "Radio" Name ="cust_typ" id="cust_typ" tabindex=30 value= "2"  >No</td> '; 
        echo '<td><Input type = "Radio" Name ="warr_type" id="warr_type" tabindex=31 value="2" checked >AMC</td> '; 
            echo ' <td><Input type = "Radio" Name ="chargable" id="chargable" tabindex=32 value= "2" checked >NO</td></tr>';  
    echo '<tr><td> <td><Input type = "Radio" Name ="warr_type" id="warr_type"  tabindex=31 value="3" >Other</td></tr>';
  ...
  else
  {
    ...
    echo '<tr><td><Input type = "Radio" Name ="cust_typ" id="cust_typ" tabindex=30 value= "2"  >No</td> '; 
        echo '<td><Input type = "Radio" Name ="warr_type" id="warr_type" tabindex=31 value="2"  >AMC</td> '; 
            echo ' <td><Input type = "Radio" Name ="chargable" id="chargable" tabindex=32 value= "2"  >NO</td></tr>';  
    echo '<tr><td> <td><Input type = "Radio" Name ="warr_type" id="warr_type"  tabindex=31 value="3" checked>Other</td></tr>';  
  } 
}  
else
{      
    echo '<tr><td><Input type = "Radio" Name ="cust_typ" id="cust_typ" tabindex=30 value= "1"  >yes</td>';
        echo '<td><Input type = "Radio" Name ="warr_type" id="warr_type" tabindex=31 value="1"  >Warranty</td>'; 
            echo '<td><Input type = "Radio" Name ="chargable" id="chargable" tabindex=32  value= "1" checked>yes</td></tr>'; 
    echo '<tr><td><Input type = "Radio" Name ="cust_typ" id="cust_typ" tabindex=30 value= "2" checked >No</td> '; 
        echo '<td><Input type = "Radio" Name ="warr_type" id="warr_type" tabindex=31 value="2"  >AMC</td> '; 
            echo ' <td><Input type = "Radio" Name ="chargable" id="chargable" tabindex=32 value= "2"  >NO</td></tr>';  
    echo '<tr><td> <td><Input type = "Radio" Name ="warr_type" id="warr_type"  tabindex=31 value="3" checked>Other</td></tr>';
...
if(!isset($_GET['edit']))     
{                                                             
echo'<tr><td colspan=2 align="center"> <input type="Submit" name="submit" tabindex=27  value="' . _('Add Complaint') . '" onclick=" if(validate()==1)return false">&nbsp;&nbsp;<input type="submit" tabindex=28  action="Reset" value="' . _('Clear') . '"></td></tr>'; 
}else{
echo'<tr><td colspan=2 align="center"> <input type="Submit" tabindex=27  name="update" value="' . _('Update') . '" onclick=updateincident()> </td></tr>';    
}    
...
function searchCustomer(){
    ...
    var str3=document.getElementById("ticketno").value;
    var x=str1;  
...
if (str1=="")
  ...
  return;
    ...
    xmlhttp.open("GET","bio_selectIncidents_crm.php?id="+id1+id,true);
    xmlhttp.send();   
...
window.open("bio_incidentdepartment_crm.php?ticketno="+ticket,"_blank");
}
function history()
{
var order=document.getElementById("orderid").value;   
 controlWindow=window.open("viewhistory_log.php?orderid="+order,"viewlog", 
...
{
var order=document.getElementById("orderid").value;   
 controlWindow=window.open("bio_check_warranty.php?orderid="+order,"viewlog", 
                "toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=no,width=800,height=600");   
...
xmlhttp.open("GET","bio_check_warranty.php?orderid=" +order ,true);
xmlhttp.send(); 
}            */
