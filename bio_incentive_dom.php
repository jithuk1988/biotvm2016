...
$sql_tmptable="CREATE  TABLE IF NOT EXISTS bio_temptarget_dom(teamid int,task int,month int,year int,ordervalue double,target double,achivement_p double,eligible_p double,incentive double,level int)";
...
AND orddate between '".$closedate_row['lastclose']."' AND'".$nowFormat."' 
   ...
   */
   ...
   $sql_k="SELECT SUM(stockitemproperties.value) FROM salesorderdetails,stockitemproperties WHERE stockitemproperties.stockid=salesorderdetails.stkcode AND stockitemproperties.stkcatpropid=48 AND salesorderdetails.orderno IN (SELECT orderno FROM salesorders WHERE leadid IN (SELECT DISTINCT bio_leadtask.leadid
FROM bio_leadtask,bio_leads 
WHERE bio_leadtask.teamid='".$row['teamid']."'
AND bio_leadtask.viewstatus=1
AND bio_leads.enqtypeid=1 
...
AND bio_teammembers.empid=bio_emp.empid AND bio_temptarget_dom.level=1  GROUP BY bio_emp.reportto   
";
...
AND bio_teammembers.empid=bio_emp.empid  AND  bio_temptarget_dom.level=2   GROUP BY bio_emp.reportto 
";
$result_get_ro=DB_query($sql_get_ro,$db);
while($rep_off=DB_fetch_array($result_get_ro))
