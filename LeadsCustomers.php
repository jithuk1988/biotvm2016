...
if (isset($_POST['submit'])) {
	...
	if ($myrow[0]>0 and isset($_POST['New'])) {
        ...
        $InputError = 1;
        prnMsg( _('Please enter a valid Electricity Board No to Continue. Please reload page'),'error');
        ECHO "<b><a style='text-allign:center' href=".$_POST['urls'].">RELOAD PAGE</a></b>";
        $Errors[$i] = 'ebno';
        $i++;
    }  
...
// Show Customer Type drop down list
	...
	echo '<tr><td>' . _('Customer Since') . ' (' . $_SESSION['DefaultDateFormat'] . '):</td>
				<td><input tabindex=10 type="text" class="date" alt="'.$_SESSION['DefaultDateFormat'].'" name="ClientSince" value="' . $DateString . '" size=12 maxlength=10></td></tr>';
	                            echo '<tr><td>EBNO(Electricity Board)</td><td><input type="text" name="ebno" id="ebno" value="'.$_POST['ebno'].'" required></td></tr>';
                                echo '<tr><td><input type="hidden" name="urls" id="urls" value="'.curPageURL().'"></td></tr>';
	...
	if ($DataError ==0){
		echo '<br /><div class="centre"><input tabindex=20 type="Submit" name="submit" value="' . _('Add New Customer') . '" onclick="if(validation()==1)return false;">&nbsp;<input tabindex=21 type="submit" action="Reset" value="' . _('Reset') . '"></div>';
...
<script type="text/javascript">  
                        function validation()
 {
   ...
   if(f==0){f=common_error('ebno','Please enter a valid EB No to Continue');  if(f==1) { return f; }} 
 }
