...
if($_GET['block']){ 
    ...
    {
        echo '<table align=left style="border:1px solid #F0F0F0;width:80%" ><tr><td width=40%>' . _('Municipality Name') . ':</td>';    
        
        $sql="SELECT * FROM bio_municipality WHERE country='$country1' AND state='$state' AND district='$district'";
        $result=DB_query($sql,$db);
        
        echo '<td width=60%><select name="lsgName" id="lsgName" style="width:130px" tabindex=11>';
         $f=0;
         while($myrow1=DB_fetch_array($result))
         {
    ...
    {
         echo '<table align=left style="border:1px solid #F0F0F0;width:80%"  align=left ><tr><td width=40%>' . _('Block Name') . ':</td>';    
         
         $sql="SELECT * FROM bio_block WHERE country=$country1 AND state=$state AND district=$district";
         $result=DB_query($sql,$db);
         
         echo '<td width=60%><select name="lsgName" id="lsgName" style="width:130px" tabindex=11 onchange="showgramapanchayath(this.value)">';
         $f=0;
         while($myrow1=DB_fetch_array($result))
         {
         if ($myrow1['id']==$_POST['lsgName'])
      ...
      echo '<tr id="showgramapanchayath"><td width=40%>' . _('Panchayat Name') . ':</td>';         
         
         $sql="SELECT * FROM bio_panchayat WHERE country=$country1 AND state=$state AND district=$district";
         $result=DB_query($sql,$db);
         
         echo '<td width=60%><select name="gramaPanchayath" id="gramaPanchayath" style="width:130px" tabindex=11>';
         $f=0;
         while($myrow1=DB_fetch_array($result))
         {
