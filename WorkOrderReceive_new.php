<?php
/* $Id: WorkOrderReceive.php 4550 2011-04-14 10:28:52Z daintree $*/
$PageSecurity=80;
...
// This Function   recieve_sub()  added by Nizam

function recieve_sub($db,$substock,$wono,$subqty,$locn)
  {
...
if ($WORow['closed']==1){
  ...
  $a=0;
    if(!isset($_POST['Process'])){
        for($k=0;$k<$_POST['CountOfInputs'];$k++) {
            
           $_SESSION['slnoentered'.$k]=$_POST['SerialNo'. $k ];
            if($_POST['SerialNo'. $k ]!="")
            {
                $a=$a+1;
            }
...
/*    if(isset($_POST['Process'])){
            ...
            for($k=0;$k<12;$k++) {
            unset($_SESSION['slnoentered'.$k]);
            echo "fjjffffffffffffffffffffffffffffffffffjdjdjdjkjknsnsajsiji";
            $a=0;
           if(isset($_POST['Process'])){
 ...
 $sql_getwoitems="SELECT stockid,qtyreqd,qtyrecd FROM woitems WHERE wo='".$_POST['WO']."' AND stockid!='".$_POST['StockID']."'";
$result_getwoitems=DB_query($sql_getwoitems,$db);
$p=0;
while($row_getwo=DB_fetch_array($result_getwoitems))
{
  
            $b=$row_getwo['qtyreqd']/$WORow['qtyreqd'];
               $c=$_POST['comp_name'.$p];
            if(isset($_POST['subassprocess'])){
             
              // echo $row_getwo['stockid']."<br>".$_POST['WO']."<br>".$c."<br>".$_POST['IntoLocation']; 
                
                recieve_sub($db,$row_getwo['stockid'],$_POST['WO'],$c,$_POST['IntoLocation']);
            }
...
if($WORow['controlled']==1){ //controlled
	...
	if ($WORow['serialised']==1){ //serialised
		...
		if ($_SESSION['DefineControlledOnWOEntry']==1){ //then potentially serial numbers already set up
			...
			if (DB_num_rows($WOSNResult)==0){
				...
				while ($WOSNRow = DB_fetch_row($WOSNResult)){
                    ...
                    <input type="hidden" name="SerialNo' . $i . '" id="SerialNo' . $i . '" value="' . $WOSNRow[0] . '" onchange="duplicate(this.value)">
        ...
        $tb=number_format($WORow['qtyreqd'],$WORow['decimalplaces'])-number_format($WORow['qtyrecd'],$WORow['decimalplaces']);
			for ($i=0;$i<$tb;$i++){
				if (($i/5 -intval($i/5))==0){
					echo '</tr><tr>';
				}
				...
				echo '<td><input type="textbox" name="SerialNo' . $i . '"  id="SerialNo' . $i . '" ';
                if(isset($_SESSION['slnoentered'.$i]))
                {
                   echo 'value="' . $_SESSION['slnoentered'.$i] . '"'; 
                }/*else
                {      unset($_SESSION['slnoentered'.$i]);
                    if ($i==0){
				...
				echo '" onchange="duplicate(this.value)"></td>';

			}
		}
		echo '</tr>';
         echo"<input type='hidden' name='stop' id='stop' value='ok'>";  
    ...
    echo '<br /><div class=centre><input type=submit name="plantslno" value="' . _('Submit Serial/Batch No. For Plant') . '"></div>'; 
  
   $sql_getwoitems="SELECT woitems.stockid,woitems.qtyreqd,woitems.qtyrecd,stockmaster.description FROM woitems,stockmaster WHERE woitems.stockid=stockmaster.stockid and woitems.wo='".$_POST['WO']."' AND woitems.stockid!='".$_POST['StockID']."'";
$result_getwoitems=DB_query($sql_getwoitems,$db);
  echo'<p><table width=85% class=selection>';
echo '<tr><th>' . _('Components') . '</th>
         <th>' . _('Qty Ordered') . '</th>
          <th>' . _('Already Recieved') . '</th>
          <th>' . _('Remaining To Recieve') . '</th>
          <th>' . _('Qty This Time') . '</th>
          </tr>';
          $j=0;
          $s=0;
...
while($row_getwo=DB_fetch_array($result_getwoitems))
{
    if ($j==1) {
            ...
            $cn=$now;
            $cn=$cn+$a;
           $rr= $row_getwo['qtyrecd'];
            echo "<td>".$row_getwo['description']."</td>";
              echo "<td>".$row_getwo['qtyreqd']."</td>";//TOTAL OREAD
            echo "<td>".$row_getwo['qtyrecd']."</td>";//RECICE
            $remaining_torec=$row_getwo['qtyreqd']-$row_getwo['qtyrecd'];
            echo "<td>".$remaining_torec."</td>";
            $b=$row_getwo['qtyreqd']/$WORow['qtyreqd'];//ONE PLANT ITEM
            $m=$cn*$b;
            $con=$m-$rr;
            if ($remaining_torec==0){echo "<td><input type=text readonly name='comp_name".$s."' value=0></td>"; }
            else if($a==0){echo "<td><input type=text  name='comp_name".$s."' value=0></td>"; }
            else if($con<1){echo "<td><input type=text readonly name='comp_name".$s."' value=0></td>"; }
            else { echo "<td><input type=text name='comp_name".$s."' value='".$con."'></td>";}
            
    /*          if($a==0)
            {
               
               echo "<td><input type=text name='comp_name".$s."' value=0></td>";
            }
            ...
            else
            {
                    $cno=$cno*$b-$rr;
                     ...
                     if($cno<1 )
            {
               echo "<td><input type=text readonly name='comp_name".$s."' value=0></td>"; 
            }
            else{
                echo "<td><input type=text name='comp_name".$s."' value='".$cno."'></td>";
            }
            }
...
echo '<br /><div class=centre><input type=submit name="subassprocess" value="' . _('Recieve Subassembly Items') . '"></div>';
    echo '<br /><div class=centre><input type=submit name="Process" value="' . _('Process Manufactured Items Received') . '" onclick="if(log_in()==1)return false;"></div>';
} else { //not controlled - an easy one!
...
var f=0;
if(f==0){f=common_error('SerialNo'+0,'Enter the Serial No');  if(f==1){return f; } } 
var y=document.getElementById('stop').value;// alert(y);
if(y==3)
{
...
xmlhttp.open("GET","bio_duplicate_Serial.php?Serial=" + str,true);
xmlhttp.send(); 
           
 

   
  // var y=document.getElementById(str).value; alert(y);
}
</script>