...
<ul>
   ...
   <li><a href='bio_amcreg.php'><span>AMC / Warrenty</span></a></li>
   <li ><a href='bio_paypending.php'><span>Payment pending</span></a></li>
   <li><a href='bio_cdmsurvey.php'><span>CDM survey</span></a></li>
   <li class='active '><a href='bio_complaintfollow.php'><span>Complaint followup</span></a></li>
</ul>
</div>";
...
$sql = "SELECT distinct bio_incidents.ticketno ,www_users.realname,bio_complainttypes.complaint ,bio_incident_cust.area1,bio_incident_cust.landline,bio_incident_cust.LSG_type,bio_incident_cust.block_name, bio_incidenttype.type , bio_priority.priority , bio_incident_cust.custname , bio_incident_cust.custphone , bio_district.district , bio_incidentsource.`source` , bio_leadteams.teamname , bio_incidentstatus.status , stockmaster.description , bio_incidents.createdon , bio_enquirytypes.enquirytype, 
...
LEFT JOIN bio_district ON bio_district.did = bio_incident_cust.district AND bio_incident_cust.state = bio_district.stateid AND bio_incident_cust.nationality = bio_district.cid 
...
LEFT JOIN bio_oldorders ON bio_oldorders.debtorno = bio_incident_cust.debtorno 
...
;
                                //  AND bio_incidents.status='closed'
...
echo"</table>";
 ...
 if($_POST['byoffice']==1) {
   ...
   if($_POST['type']!=0){
       ...
       $result=DB_query("SELECT * FROM bio_incidentstatus where id=3",$db);
       $myrow1=DB_fetch_array($result);
   ...
   echo "<tr><td colspan='8'><font size='-1'>".$title."</font></td></tr><tr><td>"; 
                  echo "<div style='height:400px;width:1200px; overflow:auto;'>";
...
echo "<font size='-1' style='margin-left:5%'><b>Total Count(".$a.")<b/></font>";     
        ...
        echo '<tr>  <th>' . _('Slno') . '</th> <th>' . _('Ticket No') . '</th> <th>' . _('Customer Name') . '</th>  <th>' . _('Contact No') . '</th>
                    <th>' . _('Place') . '</th><th class="lsg">' . _('LSG') . '</th><th class=district>' . _('District') . '</th><th>' . _('Complaint') . '</th><th>' . _('Created date') . '</th><th class="type">' . _ ('Type') . '</th>   
...
//  $result1=DB_query($sql1, $db);           
  ...
  if ($k==1){
        ...
        $k=1;
               ...
               <td>$custname</td><td>$ph</td><td>$place</td><td class='lsg'>$LSG_name</td><td class=district>$district</td><td>$complaint</td><td>$dat</td><td class='type'>$myrow3[type]</td>  
