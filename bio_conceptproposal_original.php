...
$sql="SELECT sum(netprice) FROM bio_temppropitems WHERE leadid=$leadid";
$result=DB_query($sql,$db);
...
echo "<tr><td colspan=2>
          <input type='button' id=\"".$lead."\" value='Add Item'  onclick='selectplant(this.id,2);'>
       </td>";
echo "<td colspan=3>
...
echo"<fieldset style='width:820px;height:190px'><legend>Customer Details</legend>";
...
echo "<td id='plantname'><input type='hidden' name='Plantid' id='plantid'>
  ...
  echo"<tr><td width=50%>Tender Details</td>";
echo "<td id='tenderdetails'><input type='hidden' name='Tender' id='tender'>
      <a onclick='cptender($leadid)'>Select</a></td></tr>";      
echo"</table>";
echo"</td></tr>";
echo"</fieldset>"; 
echo"</div>";
...
printf("<td cellpading=2>%s</td>
        ...
        <td><a  style='cursor:pointer;'  id='$leadid' onclick='passid(this.id,$status)'>" . _('Select') . "</a></td>  
        </tr>",
...
{
 var f=0;  
 if(f==0){f=common_error('expectdate','Please Select an Expected date');  if(f==1) { return f; }}
...
//   alert(str1);
 if(str1==""){
 alert("Select Feedstock"); document.getElementById("feedstock").focus();  return false;  }
 if (str1=="")
  {
