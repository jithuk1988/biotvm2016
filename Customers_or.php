...
$Errors = array();
 ...
 $incidentflag=$_GET['incident']; 
     ...
     elseif($custid!="")
     {
         $sql1="SELECT custname,custphone,custmob,custmail,houseno,housename,area1,area2,pin,nationality,state,district FROM bio_cust WHERE cust_id=$custid";
     }
...
/* First check that all the necessary items have been setup */
    ...
    echo'</select></td></tr>'; 
	//if ($_SESSION['AutoDebtorNo']==0)  { 
    	echo '<tr><td>' . _('Customer Code') . ':</td><td id=autocode><input tabindex=1 type="text" name="DebtorNo" id="DebtorNo" size=11 maxlength=10 style="width:100px"></td></tr>';
	//}
	...
	echo '<tr><td>' . _('Customer Name') . ':</td>
		<td><input tabindex=2 type="Text" name="CustName" id="CustName" size=42 maxlength=100 value="'.$custname.'" onchange=duplicatename();></td></tr>';
	...
	echo '<tr><td>' . _('House / Org Area') . ':</td>
		<td><input tabindex=4 type="Text" name="Address2" size=42 maxlength=200 value="'.$area1.'"></td></tr>';
       ...
       echo"</tr>";  
        ...
        echo"<tr id='showtaluk'></tr>";  
              ...
              echo '<tr><td>' . _('Village Name'). ':</td>
                  <td><input tabindex=9 type="Text" name="village" id="village" style=width:190px maxlength=100 value=""></td></tr>';  
...
// Show Sales Type drop down list
	...
	echo '</table></td>
			<td><table class="selection" style="display:none">';                                            //display none
